...
public:
    ...
    double train_loss(Algorithm* algorithm, Data &data){
        return (data.y-data.x * algorithm->get_beta()).squaredNorm()/double(data.get_n());
    }
...
//                std::cout<<"test size"<< group_list[k].size()<<endl;
                ...
                loss_list(k) =  (test_y-test_x * algorithm->get_beta()).squaredNorm()/double(group_list[k].size());
            }
