...
class Model:
    ...
    def __init__(self, name, args, model_instances=1):
        ...
        if self._args.custom_model_directory:
            ...
            self._model_dir = os.path.join(os.path.dirname(__file__), name.split('PreProd')[0])
        elif self._args.mrm and 'Challenger' in self.name:
            self._model_dir = os.path.join(os.path.dirname(__file__), name.split('Challenger')[0])
        else:
        ...
        if 'training_data_reference' in self.configuration_data:
            ...
            if self.configuration_data['asset_metadata']['input_data_type'] == 'UNSTRUCTURED_TEXT':
                ...
                with open(self._get_file_path(Model.TOKENIZER_PICKLE_FILENAME), 'rb') as handle:
        ...
        if feedback_file:
            if self.is_unstructured_text:
                ...
                if self._args.mrm and ('preprod' in self.name.lower() or 'challenger' in self.name.lower()):
                    with open(feedback_file) as file:
                        f = file.read()
                        self._feedback_csv = bytes(bytearray(f, 'utf-8'))
    ...
    def _get_file_path(self, filename):
        ...
        if self._args.mrm and 'Challenger' in self.name:
            engine_specific_path = os.path.join(self._model_dir, self._args.ml_engine_type.name.lower(), 'mrm', filename)
            if Path(engine_specific_path).is_file():
                return engine_specific_path
            else:
                return None

        engine_specific_path = os.path.join(self._model_dir, self._args.ml_engine_type.name.lower())
        if self._args.ml_engine_type is MLEngineType.WML:
            api_version = 'v4' if self._args.v4 else 'v3'
            engine_specific_path = os.path.join(engine_specific_path, api_version, filename)
        if Path(engine_specific_path).is_file():
            return engine_specific_path
        else:
    ...
    def get_score_input(self, num_values=1):
        ...
        values = []
        if self.is_unstructured_text:
            num_rows = len(self.training_data[0])
            random_row = random.randint(0, num_rows - 1)
            values = self.training_data[0][random_row]
