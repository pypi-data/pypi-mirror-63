...
class RepositoryRunner(ServiceRunner):
    ...
    def _register_remote_repository(self, param, domain):
        ...
        token_name = param['credential_id']
        token = param[token_name]
        token['domain_id'] = domain.domain_id

        credential_info = self.secret.Credential.create(token, metadata=self.get_meta())
        credential_id = credential_info.credential_id
        self.append_terminator(
            self.secret.Credential.delete,
            {'credential_id': credential_id, 'domain_id': domain.domain_id}
        )
