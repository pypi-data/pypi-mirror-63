...
class S3Util:
    ...
    def upload(self, file_name, s3_path):
        ...
        if platform == 'win32':  # windows
            ...
            cmd_str = 'aws s3 cp {} {} --profile {}'.format(file_name, s3_path, profile)

        elif platform in ['linux2', 'darwin']:  # ubuntu
            aws_path = '/usr/local/bin/'
            cmd_str = '{}aws s3 cp {} {} --profile {}'.format(aws_path, file_name, s3_path, profile)
        else:
    ...
    def download(self, file_name, s3_path):
        profile = s3_path.replace('s3://', '').split('/')[0]
        if platform == 'win32':  # windows
            os.chdir("C:\Python27\Scripts")
            cmd_str = 'aws s3 cp {} {} --profile {}'.format(s3_path, file_name, profile)
            self.log.info("change to scripts directory")
        elif platform in ['linux2', 'darwin']:  # ubuntu
            aws_path = '/usr/local/bin/'
            cmd_str = '{}aws s3 cp {} {} --profile {}'.format(aws_path, s3_path, file_name, profile)
        else:
            self.log.error('unknown platform', str(platform))
            return
