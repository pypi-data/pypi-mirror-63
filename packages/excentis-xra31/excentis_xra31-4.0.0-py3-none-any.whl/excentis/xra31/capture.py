    ...
    def __update(self) -> None:
        ...
        channels = sorted(
            ...
            key=lambda channel_parameters: channel_parameters[
                "actualPlcFrequency"] or channel_parameters["plcFrequency"])
        self._ofdm.clear()
        for channel_parameters in channels:
            self._ofdm.append(
                core.OfdmChannel(channel_parameters["id"],
                                 channel_parameters["actualPlcFrequency"]
                                 or channel_parameters["plcFrequency"],
...
class Output:
    ...
    def filename(self, filename: str) -> None:
        filename = re.sub("^(.*).pcap$", "\\1", filename)
    ...
    def describe(self) -> dict:
        ...
        description = {}
        self.__update()
        description["directory"] = self._parameters["directory"]
        description["filename"] = self._parameters["fileName"]
    ...
    def apply(self, description: dict) -> None:
        ...
        if "filename" in description:
            parameters["fileName"] = description["filename"]
...
class Capture:
    ...
    @trace
    def wait_for_end(self, timeout: float = None) -> str:
        ...
        :return: The filename of the most recent capture if successful,
                 ``None`` otherwise.
        :rtype: str
        """
        ...
        while capture_active and (stop == 0 or time.monotonic() < stop):
            time.sleep(.5)
            capture_active = self.active
        return self.filename if not capture_active else None

    @trace
    def wait_for_file_end(self, timeout: float = None) -> str:
        ...
        stop = 0 if timeout is None else round(time.monotonic()) + timeout
        capture_active = self.active
        while capture_active and self.filename == filename and (
                stop == 0 or time.monotonic() < stop):
            time.sleep(.5)
            capture_active = self.active
        return (filename
                if not capture_active or self.filename != filename else None)
