...
class Client:
    ...
    def __init__(self, address: str = "localhost"):
        ...
        #: The minimal XRA-31 server release.
        ...
        if Version(self.server_version) < Version(self.min_server_version):
            self.logger.error(
                "This API client ({}) requires an XRA-31 release >={},"
                " but found {}.".format(self.version, self.min_server_version,
                                        self.server_version))
            raise exceptions.Xra31VersionException(
                "This API client requires an XRA-31 release >=" +
                self.min_server_version)

        if Version(self.version) < Version(self.min_version):
            self.logger.error("The XRA-31's release ({})"
                              " requires an API client version >={},"
                              " but found {}".format(self.server_version,
                                                     self.min_version,
                                                     self.version))
            raise exceptions.Xra31VersionException(
                "The XRA-31's release requires an API client version >=" +
                self.min_version)
