...
`;var Ht=(t,e,i,r,s)=>{let o;switch(r.action){case"more-info":(o=new Event("hass-more-info",{composed:!0})).detail={entityId:r.entity||s},t.dispatchEvent(o);break;case"navigate":if(!r.navigation_path)return;window.history.pushState(null,"",r.navigation_path),(o=new Event("location-changed",{composed:!0})).detail={replace:!1},window.dispatchEvent(o);break;case"call-service":{if(!r.service)return;const[t,i]=r.service.split(".",2),s=Object.assign({},r.service_data);e.callService(t,i,s);break}case"url":if(!r.url)return;window.location.href=r.url}};customElements.define("mmp-group-item",class extends ct{static get properties(){return{item:{},checked:Boolean,disabled:Boolean,master:Boolean}}render(){return z`
      <paper-checkbox
        ...
        ${this.item.name}
        ${this.master?z`<span>(master)</span>`:""}
      </paper-checkbox>
    ...
    `}}),customElements.define("mmp-group-list",class extends ct{static get properties(){return{entities:{},player:{},visible:Boolean}}get group(){return this.player.group}get master(){return this.player.master}get isMaster(){return this.player.isMaster}get isGrouped(){return this.player.isGrouped}handleGroupChange(t){const{entity:e,checked:i}=t.detail;this.player.handleGroupChange(t,e,i)}render(){if(!this.visible)return z``;const{group:t,isMaster:e,isGrouped:i}=this,{id:r}=this.player;return z`
    ...
    `}renderItem(t,e){const i=t.entity_id;return z`
      <mmp-group-item
        ...
        `:z`
          <mmp-button class='mmp-dropdown__button' slot='dropdown-trigger'>
            <div>
              ...
              <iron-icon class='mmp-dropdown__icon' .icon=${Ft.DROPDOWN}></iron-icon>
            </div>
          </mmp-button>
        `}
        <paper-listbox slot="dropdown-content" .selected=${this.selectedId} @iron-select=${this.onChange}>
          ${this.items.map(t=>z`
            <paper-item value=${t.id||t.name}>
              ${t.icon?z`<iron-icon .icon=${t.icon}></iron-icon>`:""}
              ${t.name?z`<span class='mmp-dropdown__item__label'>${t.name}</span>`:""}
            </paper-item>`)}
        </paper-listbox>
      </paper-menu-button>
    `}static get styles(){return[Gt,nt`
        :host {
          display: block;
        }
        :host([faded]) {
        ...
        ${this.buttons.map(e=>z`
          <mmp-button
            ...
            <div align=${this.shortcuts.align_text}>
              ${e.icon?z`<iron-icon .icon=${e.icon}></iron-icon>`:""}
              ${e.image?z`<img src=${e.image}>`:""}
              ${e.name?z`<span class="ellipsis">${e.name}</span>`:""}
            </div>
          </mmp-button>`)}
      </div>
    `:"";return z`
      ...
      <div class='mmp-progress'
        ...
        ${this.showTime?z`
          <div class='mmp-progress__duration'>
            <span>${Xt(this.seekProgress||this.progress)}</span>
    ...
    `}renderVolControls(t){return this.config.volume_stateless?this.renderVolButtons(t):this.renderVolSlider(t)}renderVolSlider(t){return z`
      ...
      </div>`}renderVolButtons(t){return z`
      <div class='mmp-media-controls__volume --buttons flex'>
      ...
      </div>`}renderMuteButton(t){if(!this.config.hide.mute)switch(this.config.replace_mute){case"play":case"play_pause":return z`
          <paper-icon-button
            @click=${t=>this.player.playPause(t)}
            .icon=${Ft.PLAY[this.player.isPlaying]}>
          </paper-icon-button>
        `;case"stop":return z`
          <paper-icon-button
        ...
        `;case"play_stop":return z`
          <paper-icon-button
            ...
            .icon=${Ft.STOP[this.player.isPlaying]}>
          </paper-icon-button>
        `;case"next":return z`
          <paper-icon-button
            @click=${t=>this.player.next(t)}
      ...
      ${t.play_stop?z``:z`
        <paper-icon-button
          ...
          .icon=${t.play_pause?Ft.STOP[this.player.isPlaying]:Ft.STOP.true}>
        </paper-icon-button>
      `}
    `}handleStop(t){return this.config.hide.play_pause?this.player.playStop(t):this.player.stop(t)}handleVolumeChange(t){const e=parseFloat(t.target.value)/100;this.player.setVolume(t,e)}static get styles(){return[Gt,nt`
        :host {
          display: flex;
          width: 100%;
          justify-content: space-between;
    ...
    `}renderArtwork(t){if(!this.thumbnail&&!this.config.background)return;const e=!this.config.background||t&&"default"!==this.config.artwork?this.thumbnail:`url(${this.config.background})`;return z`<div class='cover' style='background-image: ${e};'></div>`}handlePopup(t){t.stopPropagation(),Ht(this,this._hass,this.config,this.config.tap_action,this.player.id)}renderIcon(t){if(this.config.hide.icon)return;if(this.player.active&&t&&"default"===this.config.artwork)return z`
        <div class='entity__artwork'
          style='background-image: ${this.thumbnail};'
          ?border=${!this.config.hide.artwork_border}
          state=${this.player.state}>
        </div>`;const e=!this.config.hide.icon_state&&this.player.isActive;return z`
      <div class='entity__icon' ?color=${e}>
        <ha-icon .icon=${this.computeIcon()} ></ha-icon>
      </div>`}renderEntityName(){if(!this.config.hide.name)return z`
      <div class='entity__info__name'>
        ${this.name} ${this.speakerCount()}
      </div>`}renderMediaInfo(){if(this.config.hide.info)return;const t=this.player.mediaInfo;return z`
