import opentracing
from flask import _request_ctx_stack as stack, Flask
from opentracing.ext import tags

from biz_monitor.biz_metrics import KafkaMetricsCollector
from biz_monitor.biz_tracing.requests_opentracing import monkeypatch_requests
...
class FlaskTracing(opentracing.Tracer):
    ...
    def __init__(self, tracer=None, trace_all_requests=None, app=None,
                 traced_attributes=[], start_span_cb=None, metrics_collector: KafkaMetricsCollector = None):
    ...
    def get_span(self, request=None):
        """
        Returns the span biz_monitor `request`, or the current request if
        `request==None`.
    ...
    def _before_request_fn(self, attributes):
        ...
        span.set_tag(HTTP_URI, request.path)
    ...
    def _after_request_fn(self, response=None, error=None):
        ...
        if response is not None and response.is_json:
            biz_code = response.get_json().get("code", "")
        else:
            biz_code = ""
        self.metrics_collector.report_span(scope.span, biz_code)
        scope.close()
...
def init_tracing(service_name, tracing_uri, kafka_uri, idc_name, set_name, instance_id, app: Flask):
    tracer = init_tracer(tracing_uri, service_name)
    metrics_collector = KafkaMetricsCollector(kafka_uri, service_name, idc_name, set_name, instance_id)
    FlaskTracing(tracer=tracer, app=app, metrics_collector=metrics_collector)
