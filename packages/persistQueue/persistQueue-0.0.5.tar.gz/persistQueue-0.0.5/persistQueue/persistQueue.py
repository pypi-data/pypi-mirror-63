...
class PersistQueue(object):
    ...
    def __init__(self, path_file, maxsize=500, logger_name=None):
        ...
        self.logger.info("PersistQueue version 0.0.4 Started")
    ...
    def put_queue(self, data):
        ...
        self.logger.info("put queue")
        self.logger.info("q.size == %d" % self.q.qsize())
        if self.q.qsize() >= self.maxsize:
            old_data = self.get_queue()
            self.discard(old_data)
            self.logger.info('old data deleted: %s' % str(old_data))
        self.logger.info("insert data in put-queue: %s with maxsize %d" % (data, self.maxsize))
        self.q.put(data)
