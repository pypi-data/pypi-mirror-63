...
class showPILandSelect(object):
    ...
    def __init__(self, master, pilImage, quitOnSelect=False):
        self.masterroot = master

        # Create a Toplevel() dialog window, we can create multiple ones
        root = tkinter.Toplevel(self.masterroot)  # use Toplevel() instead of Tk() to reopen the same dialog multiple times, see also: https://github.com/dangillet/PythonFaqFr/blob/master/doc/tkinter.md and https://stackoverflow.com/questions/39458318/how-to-allow-a-tkinter-window-to-be-opened-multiple-times
        self.root = root
...
def selectRegion(master, sct, config, configFile, quitOnSelect=False):
    ...
    appregionselect = showPILandSelect(master, img, quitOnSelect=quitOnSelect)
...
def translateRegion(master, sct, config, configFile):
    class TranslationBox():
        def __init__(self, master, ocrtext, transtext):
        ...
        def run(self):
            root = tkinter.Toplevel(self.masterroot)
            self.root = root
    ...
    if 'INTERNAL' not in config or 'region' not in config['INTERNAL'] or ast.literal_eval(config['INTERNAL']['region']) is None:
        show_errorbox("Error: please first select a region to capture from (use hotkey %s)" % config['DEFAULT']['hotkey_set_region_capture'], master)
        return
    ...
    if not ocrtext.strip():
        show_errorbox('No text found by OCR! Make sure your capture region is properly set!', master)
        return
    if config['DEFAULT']['debug'] == 'True':
        print('OCR\'ed text:')
        print(ocrtext)
    ...
    twindow = TranslationBox(master, ocrtext, transtext)
    prev_root = twindow


def selectAndTranslateRegion(master, sct, config, configFile):
    """Wrapper to select a region and translate it directly after, this streamlines the process"""
    selectRegion(master, sct, config, configFile, quitOnSelect=True)
    translateRegion(master, sct, config, configFile)

def show_errorbox(msg, master=None):
    """Show an error box"""
    if master is None:
        master = tkinter.Tk()
...
def show_errorbox_exception(msg, master=None):
    """Show both an error box and raise an Exception"""
    show_errorbox(msg, master)
    raise Exception(msg)
...
def main():
    ...
    # Launch Tkinter root Tk() from main thread
    tkroot = tkinter.Tk()
    tkroot.overrideredirect(1)  # hide main (blank) window
    tkroot.withdraw()  # hide main window
    
    # Set global hotkeys, loading from config file
    keyboard.add_hotkey(config['DEFAULT']['hotkey_set_region_capture'], selectRegion, args=(tkroot, sct, config, configFile), suppress=True)
    print('Hit %s to set the region to capture.' % config['DEFAULT']['hotkey_set_region_capture'])
    keyboard.add_hotkey(config['DEFAULT']['hotkey_translate_region_capture'], translateRegion, args=(tkroot, sct, config, configFile), suppress=True)
    ...
    keyboard.add_hotkey(config['DEFAULT']['hotkey_set_and_translate_region_capture'], selectAndTranslateRegion, args=(tkroot, sct, config, configFile), suppress=True)
    ...
    while 1:
        # Update GUI
        tkroot.update()
        # Leave some sleep time for the keyboard hotkeys to work
        time.sleep(0.5)
