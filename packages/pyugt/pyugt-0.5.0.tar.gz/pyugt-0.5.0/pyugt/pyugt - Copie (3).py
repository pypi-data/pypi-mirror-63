...
class showPILandSelect(object):
    ...
    def __init__(self, masterroot, pilImage, quitOnSelect=False):
        # Create a Toplevel() dialog window, we can create multiple ones
        root = tkinter.Toplevel(masterroot)  # use Toplevel() instead of Tk() to reopen the same dialog multiple times, see also: https://github.com/dangillet/PythonFaqFr/blob/master/doc/tkinter.md and https://stackoverflow.com/questions/39458318/how-to-allow-a-tkinter-window-to-be-opened-multiple-times
        self.root = root
        # hide root dialog
...
def selectRegion(sct, masterroot, config, configFile, quitOnSelect=False):
    ...
    appregionselect = showPILandSelect(masterroot, img, quitOnSelect=quitOnSelect)
...
def selectAndTranslateRegion(sct, TBox, config, configFile):
...
def main():
    ...
    # Create GUI
    masterroot = tkinter.Tk()
    masterroot.overrideredirect(1)
    masterroot.withdraw()
    TBox = TranslationBox(config, configFile)

    # Set global hotkeys, loading from config file
    keyboard.add_hotkey(config['DEFAULT']['hotkey_set_region_capture'], selectRegion, args=(sct, masterroot, config, configFile))  # Do NOT set suppress=True, else this may raise exceptions!
    ...
    keyboard.add_hotkey(config['DEFAULT']['hotkey_set_and_translate_region_capture'], selectAndTranslateRegion, args=(sct, TBox, config, configFile))
