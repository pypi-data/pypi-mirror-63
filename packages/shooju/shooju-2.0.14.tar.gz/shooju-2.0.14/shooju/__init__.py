...
class Client(object):
    ...
    def __init__(self, server, user=None, password=None, base_path='/api/1',
                 retries=None, retry_delay=None, location=None, hosts=None, proxies=None):
        """
        REST Client
        ...
        :param proxies: `requests` compatible proxy information to pass API requests through
        """
        if not hosts:
            hosts = [server]
    ...
    def _call(self, method_name, path=None, ignore_exceptions=False, params=None,
        ...
        try:
            r = method(url, params=params or None, data=payload, headers=headers,
                       files=files, auth=(self._user, self._password), timeout=timeout, proxies=self._proxies)
