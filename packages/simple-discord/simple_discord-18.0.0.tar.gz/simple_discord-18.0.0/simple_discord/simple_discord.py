...
async def on_ready():
    ...
    try:
        if command_dict["config_options"]["status"] != "online":
            if command_dict["config_options"]["activity"] == None:
                if command_dict["config_options"]["status"] == "idle":
                    await client.change_presence(status=discord.Status.idle)
                    return
                if command_dict["config_options"]["status"] == "do not disturb":
                    await client.change_presence(status=discord.Status.dnd)
                    return
                if command_dict["config_options"]["status"] == "offline":
                    await client.change_presence(status=discord.Status.offline)
                    return
            if command_dict["config_options"]["status"] == "idle":
                await client.change_presence(status=discord.Status.idle, activity=discord.Game(command_dict["config_options"]["activity"]))
            if command_dict["config_options"]["status"] == "do not disturb":
                await client.change_presence(status=discord.Status.dnd, activity=discord.Game(command_dict["config_options"]["activity"]))
            if command_dict["config_options"]["status"] == "offline":
                await client.change_presence(status=discord.Status.offline)
    except KeyError:
        pass
