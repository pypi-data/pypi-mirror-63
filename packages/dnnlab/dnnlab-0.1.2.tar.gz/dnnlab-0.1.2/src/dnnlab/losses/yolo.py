...
def yolo_loss(y_true,
    ...
    y_true_p = tf.reshape(
        y_true[..., :4], shape=(batch_size, 1, 1, 1, num_true_labels, 4))
...
def yolo_grid(batch_size, grid_h, grid_w, boxes):
    ...
    cell_grid = tf.tile(
        tf.concat([cell_x, cell_y], -1), [batch_size, 1, 1, boxes, 1])
...
def extract_model_output(y_pred, grid_coord, anchors):
    ...
    box_wh = tf.cast(
        tf.exp(box_wh), dtype=tf.float32) * tf.cast(
...
def _loss_xywh(true_box_xy, pred_box_xy, true_box_wh, pred_box_wh,
    ...
    loss_xy = tf.reduce_sum(
        tf.square(
            ...
            tf.cast(pred_box_xy, dtype=tf.float32)) * tf.cast(
                coord_mask, dtype=tf.float32))
    ...
    loss_xywh = (loss_xy + tf.cast(loss_wh, dtype=tf.float32)) / 2.0
    return loss_xywh
...
def _loss_class(
    ...
    loss_class = tf.reduce_sum(
        tf.cast(loss_class, dtype=tf.float32) * tf.cast(
            class_mask, dtype=tf.float32))
    return loss_class
