...
def mdk(*args, **kwargs):
    ...
    def mdk_bash():
        ...
        backend.cmd('exec', '-it', '@CONTAINER@', 'bash')
    ...
    def mdk_exec(command, interactive, tty):
        args = (['--interactive'] if interactive else []) + (['--tty'] if tty else [])
        backend.start(implicit=True)
    ...
    def mdk_sh():
        backend.start(implicit=True)
        backend.cmd('exec', '-it', '@CONTAINER@', 'sh')
    ...
    def mdk_zsh():
        backend.start(implicit=True)
        backend.cmd('exec', '-it', '@CONTAINER@', 'zsh')
