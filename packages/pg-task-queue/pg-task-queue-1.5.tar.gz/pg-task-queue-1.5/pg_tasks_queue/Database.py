...
class PgDatabase:
    ...
    def connect(self, dbname=None, close_connection=False):
    ...
    def _execute(self, sql_string, sql_params=None, get_result=True):
        ...
        if self._connection is not None:
            try:
                ...
                if get_result:
                    self._result = cursor.fetchall() or []
                return True
            except Exception as e:
                print(f'{err_str}: {type(e)}: {str(e)}; traceback: {traceback.print_exc()}')
                self._connection.rollback()
    ...
    def is_in_transaction(self):
        ...
        if self._connection.status == psycopg2.extensions.STATUS_READY \
                and self._connection.isolation_level == psycopg2.extensions.ISOLATION_LEVEL_READ_COMMITTED:
            return True
        return self._connection.status == psycopg2.extensions.STATUS_IN_TRANSACTION
...
class TasksDatabase:
    ...
    def test_tables(cls, create=False):
        ...
        if not TasksDatabase.pg_database._test_table(table_name, table_columns, create):
    ...
    def add_task(cls, task_dict):
        ...
        res = TasksDatabase.pg_database.insert(table=f"{schema}.tasks", values=task_dict)
        if not isinstance(res, psycopg2.extras.RealDictRow):
            print(f'Error in {func_name}: isinstance(res, psycopg2.extras.RealDictRow)...')
            return None
        res_dict = dict(res)
    ...
    def get_new_task(cls, sql_string, disconnect=True):
        ...
        update_values = {'started_time': datetime.datetime.now(), 'status': 'working'}
        result = TasksDatabase.pg_database.update(table=f'{schema}.tasks', values=update_values,
                                                  where={'task_id': task_dict.get('task_id')})
        if not isinstance(result, psycopg2.extras.RealDictRow):
            print(f'Error in {func_name}: isinstance(res, psycopg2.extras.RealDictRow)...')
            TasksDatabase.pg_database.disconnect()
            return None
