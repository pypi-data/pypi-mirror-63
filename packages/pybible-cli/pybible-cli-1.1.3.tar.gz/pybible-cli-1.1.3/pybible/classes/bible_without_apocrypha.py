...
class BibleWithoutApocrypha(Bible):
    """
    A class used to represent a real world bible (not including apocrypha books).
    """
    __ot_last_book = 39
    """Last old testament book position in the bible (not including apocrypha books)."""
    __book_names = dict([(book, pos) for pos, book in enumerate(("genesis", "exodus", "leviticus", "numbers",
                                                                 "deuteronomy", "joshua", "judges", "ruth", "1samuel",
                                                                 "2samuel", "1kings", "2kings", "1chronicles",
                                                                 "2chronicles", "ezra", "nehemiah", "esther", "job",
                                                                 "psalms", "proverbs", "ecclesiastes", "song of songs",
                                                                 "isaiah", "jeremiah", "lamentations", "ezekiel",
                                                                 "daniel", "hosea", "joel", "amos", "obadiah", "jonah",
                                                                 "micah", "nahum", "habakkuk", "zephaniah", "haggai",
                                                                 "zachariah", "malachi", "matthew", "mark", "luke",
                                                                 "john", "acts", "romans", "1corinthians",
    ...
    """Dictionary containing the book names in the bible (not including apocrypha books)."""
    ...
    def __init__(self, name: str, language: str, books: tuple):
        ...
        :Parameters:
            ...
            - `books`: tuple of books contained bible (not including apocrypha books).
        """
    ...
    def __getitem__(self, key):
        try:
            ...
            print(f"Book \"{key}\" not found in {self.name}.\nAvailable book names for index: "
                  f"{list(self.__book_names.keys())}")
            sys.exit(4)

    def __repr__(self):
        return f'BibleWithoutApocrypha(\"{self.name}\", \"{self.language}\", {self.books})'
    ...
    def books_names(self) -> tuple:
        """Tuple of book names inside the bible (not including apocrypha books)."""
