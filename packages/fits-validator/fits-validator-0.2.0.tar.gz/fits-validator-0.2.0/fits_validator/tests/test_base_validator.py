...
def valid_definition_format_params(tmpdir_factory):
    ...
    """
    dict1 = {
        "key_001": {"required": False, "comment": "comment 1", "type": "float",},
        "key_002": {"required": True, "comment": "comment 2", "type": "str",},
    }

    dict2 = {
        "key_003": {"required": True, "comment": "comment 3", "type": "int",},
        "key_004": {"required": False, "comment": "comment 4", "type": "bool",},
    }

    dict3 = {**dict1, **dict2}

    temp_dir = tmpdir_factory.mktemp("valid_definitions_temp")
    print("Temp dir = ", str(temp_dir))
    file_name = temp_dir.join("tmp_file.yml")
    file_name.write(yaml.dump(dict3))

    test_params = {
        "valid Path to dir": Path(str(temp_dir)),
        "valid Path to file": Path(str(file_name)),
        "valid dict": {**dict1, **dict2},
...
@pytest.fixture(
    scope="function",
    params=["valid Path to dir", "valid Path to file", "valid dict", "valid list[dicts]"],
)
...
def test_parse_spec_schema_definitions(valid_definition_format):
    ...
    Given: A valid schema definition, can be in one of the following formats:
        Path to dir, Path to file, dict, list[dicts]
...
def invalid_definition_format_params(tmpdir_factory):
    ...
    file_name.write("this is not a valid yaml file")

    test_params = {
        "invalid Path to dir": Path(str(temp_dir) + "/foo/"),
        "invalid Path to file": Path(str(temp_dir.join("foo.yml"))),
        "invalid yaml file": Path(str(file_name)),
        "invalid Dict": dict(),
        "invalid List[Dict]": [dict(), dict()],
        "invalid schema type1": "non valid schema type",
        "invalid schema type2": 3.14159,
    }
...
def invalid_schema_definition_params():
    ...
    """
    test_params = {
        "missing required": {"keyword": {"type": "int"}},
        "missing type": {"keyword": {"required": True}},
        "missing both, no optional": {"keyword": {}},
        "missing both, with optional": {"keyword": {"optional": "value"}},
    }
...
def test_validate_spec_schema_definitions_key_collision_fail():
    """
    Given: Schema definitions that have key collisions
    When: Validating the schema
    Then: SpecSchema(definition) raises a SpecSchemaDefinitionException
    """
    with pytest.raises(SpecSchemaDefinitionException):
        # Create a schema definition with a deliberate key collision
        SpecSchema(
            [
                {"keyword_name": {"required": True, "type": "int"}},
                {"keyword_name": {"required": False, "type": "bool"}},
            ]
        )
...
def test_validate_spec_schema_definitions_values_list_fail(invalid_values_list):
    ...
    Then: SpecSchema(definition) raises a SpecSchemaDefinitionException
    """
    test_schema = {"keyword_name": {"required": True, "type": "int", "values": invalid_values_list}}
    with pytest.raises(SpecSchemaDefinitionException):
        SpecSchema(test_schema)
...
def test_create_spec_schema(define_test_schema_definition):
    ...
    assert not DeepDiff(input_dict, schema(input_dict), ignore_order=True)
...
def spec_validator():
    ...
    """
    test_schema = {
        "SIMPLE": {"required": True, "type": "bool", "default_value": "T"},
        "BITPIX": {"required": True, "type": "int", "values": [8, 16, 32, 64, -32, -64]},
        "NAXIS": {"required": True, "type": "int", "values": [3]},
        "NAXIS1": {"required": True, "type": "int"},
        "NAXIS2": {"required": True, "type": "int"},
        "NAXIS3": {"required": True, "type": "int", "default_value": 1},
    }
...
@pytest.fixture(scope="function", params=["valid_hdu_list", "valid_fits_header", "valid_fits_file"])
...
def test_spec_validator(spec_validator, valid_test_header):
    ...
    """
    spec_validator(valid_test_header)
...
def invalid_test_header_params(tmpdir_factory):
    ...
    invalid_hdu = fits.PrimaryHDU(invalid_array)
...
def test_spec_validator_fail(spec_validator, invalid_test_header):
    ...
    with pytest.raises(SpecValidationException):
        spec_validator(invalid_test_header)
