...
class AlertHandler(object):
    ...
    def _send_message(self, alert_id, _status_dict):
        ...
        _status_dict["meta"].update(dict(sender=self._name))
    ...
    def send_with_alert_id(self, status, alert_id, meta, kwargs):
        ...
        if (isinstance(status, str) and status == Status.OK) or (isinstance(status, bool) and status):
            return self.send_ok_with_alert_id(alert_id, meta, kwargs)
        else:
            return self.send_error_with_alert_id(alert_id, meta, kwargs)

    def send_ok_with_alert_id(self, alert_id, meta, kwargs):
        """ Send an ok message on the given alert with the corresponding meta/kwarg arguments
            Returns True if alert was send else False"""

        _status_dict = self._status_dict(Status.OK, **(meta or {}))
        _status_dict.update(kwargs or {})
        return self._send_message(alert_id, _status_dict)

    def send_error_with_alert_id(self, alert_id, meta=None, kwargs=None):
    ...
    def send_offline_with_alert_id(self, alert_id, meta=None, kwargs=None):
        """ Send an offline message on the given alert with the corresponding meta/kwarg arguments
            Returns True if alert was send else False"""

        _status_dict = self._status_dict(Status.OFFLINE, **(meta or {}))
        _status_dict.update(kwargs or {})
