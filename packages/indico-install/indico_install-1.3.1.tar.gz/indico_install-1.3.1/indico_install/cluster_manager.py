...
def _list_versions(grep):
    grep = f" | grep {grep}" if grep else ""
    out = run_cmd(f"gsutil ls -d {UPDRAFT_BUCKET}{grep}", silent=True).splitlines()
    return [i[len(UPDRAFT_BUCKET):].replace('/', '') for i in out]


def _list_versions_ordered(grep):
    out = run_cmd(f"gsutil ls -l {UPDRAFT_BUCKET}/*{grep}*/services.yaml", silent=True).splitlines()[:-1]
    return [
        f"{i[0]} " + i[1][len(UPDRAFT_BUCKET):-len("services.yaml")].replace('/', '') for i in
        sorted([i.split()[1:] for i in out], key=lambda x: x[0], reverse=True)
    ]
...
class ClusterManager(object):
        ...
        if "services" in self.cluster_config:
            ...
            for svc, val in self.cluster_config["services"].items():
                if "values" not in val:
                    continue
                keys_to_migrate = [k for k in ("image", "updated_by", "updated_at") if k in val["values"]]
                if keys_to_migrate:
                    services[svc] = {"values": {k: val["values"].pop(k) for k in keys_to_migrate}}
    ...
    @classmethod
    def get_updraft_versions(cls, regex, number, ordered):
        regex = string_to_tag(regex)
        res_list = _list_versions_ordered(regex) if ordered else _list_versions(regex)
