...
def catch_recursion(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if key in catch_recursion.func_list:
            raise Exception("Tags create a creates a recursive dependency loop.")
...
class RosterInventory:
    ...
    def validate_schema(self):
        ...
        def visit(_path, key, _value):
            # drop all items that start with a dot ('.') or with X- / x-
            if isinstance(key, str) and (key[0] == "." or key[0:2].lower() in ["x-"]):
                return False
            return True
        ...
        self._data = remap(self._data, visit=visit)
    ...
    def parse(self, inventory, display):
        ...
        hosts_method = {
            "funcs": [self._add_item, self._add_item_tags, self._add_item_subgroups, self._add_item_vars],
    ...
    @catch_recursion
    def _add_item_tag(self, name, tag_name):
        if not self._roster.get("tags") or not self._roster["tags"].get(tag_name):
            self.display.warning("Tag named '%s' for host '%s' is not declared in root 'tags'" % (tag_name, name))
            return
        tag = self._roster["tags"][tag_name]
        for tag_name in tag.get("inherit") or []:
            self._add_item_tag(name, tag_name)
        # add the tags's variables to this host
        self._add_item_vars(name, content=tag)

    def _add_item_tags(self, name, content, subtags_name="tags", **_kwargs):
        if not content:
            return
        for tag_name in content.get(subtags_name) or []:
            self._add_item_tag(name, tag_name)
