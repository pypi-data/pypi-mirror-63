...
# ----------------------------------------------------------------------------------------------------------------------
def set_project_id(name: str):
    # Sets global private variable __project_id. Also updates global firestore client.
    global __project_id, __firestore_client
    __project_id = name
    __firestore_client = create_firestore_client(__project_id)  # Recreate firestore client


def set_firestore_collection_source(name: str):
    # Sets global private variable __firestore_collection_source.
    global __firestore_collection_source
    __firestore_collection_source = name


def set_firestore_collection_destination(name: str):
    # Sets global private variable __firestore_collection_destination.
    global __firestore_collection_destination
    __firestore_collection_destination = name


def create_firestore_client(project_id: str):
...
# ----------------------------------------------------------------------------------------------------------------------
__project_id = 'vaknldev'
__firestore_collection_source = u'in_website_clickstream'
__firestore_collection_destination = u'dmp_users_TEST'
__firestore_client = create_firestore_client(__project_id)
...
class User(object):
    ...
    def create_event(clickstream_event_json):
        ...
        # --------------------------------------------------------------------------------------------------------------
        event_value_type = clickstream_event_json['eventValueType']
        event_trigger = clickstream_event_json['info']['eventTrigger']
        standard_event_arguments = {
            'event_id': clickstream_event_json['eventId'],
            'timestamp': clickstream_event_json['timestamp'],
            'dmp_session_id': clickstream_event_json['info']['dmpSessionId']
        }
        if event_value_type == 'undefined':
            pass
        else:
            event_value_json = json.loads(clickstream_event_json['eventValueJson'])
            if event_value_type == 'session':
                event = EventDataclass.Session(
                    ip=clickstream_event_json['remoteIp'],
                    time_zone=clickstream_event_json['timeZone'],
                    continent=clickstream_event_json['continent'],
                    country=clickstream_event_json['country'],
                    region=clickstream_event_json['mostSpecificSubdivision'],
                    device=event_value_json[event_value_type]['userAgent']['uaDeviceType'],
                    **standard_event_arguments
                )
            elif event_value_type == 'pageview':
                page_type = event_value_json[event_value_type]['pageType']
                ...
                reservation_status = event_value_json[event_value_type]['reservationStatus']
                ...
                if event_trigger == 'ibe-extras':
                    ...
                    event = EventDataclass.ReservationBooked(
                        giataid=giataid,
                        reservation_id=event_value_json[event_value_type]['reservationId'],
                        **standard_event_arguments
                    )
                else:
                ...
                event = EventDataclass.ProductService(
                    giataid=event_value_json['package']['productCode'],
                    **standard_event_arguments
                )
    ...
    def to_firestore(self):
        ...
        doc_ref = __firestore_client.collection(u'dmp_users_TEST').document(self.dmp_user_id)
        doc_ref.set({
    ...
    def create_user_from_clickstream(self):
        ...
        # --------------------------------------------------------------------------------------------------------------
        global __firestore_client
        doc_ref = __firestore_client.collection(u'in_website_clickstream').document(self.dmp_user_id) \
            .collection(u'sessions').stream()
        sessions = [doc.id for doc in doc_ref]

        event_list = []
        for session in sessions:
            event_list += __firestore_client.collection(u'in_website_clickstream').document(self.dmp_user_id) \
