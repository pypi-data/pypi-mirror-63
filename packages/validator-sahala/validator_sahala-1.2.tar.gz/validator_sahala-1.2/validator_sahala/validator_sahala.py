...
import re
from validator_sahala.log_common import Log
...
class Validator_Sahala():
    ...
    def assert_re_Expression_data(self, key, expect_Expression, resp_value, error_msg):
        ...
        :param response: value正则匹配必须是字符串
        """
        if expect_Expression not in (int, str, float):  # 先过滤type校验，跳出本方法后下一步校验type类型
            if 're$' in str(expect_Expression) and type(expect_Expression) is str:  # 只保留包含re$的字符串
                ...
                if re.match(re_data, str(resp_value)):  # 正则表达式匹配字符串
                    log.info('26 pass 正则匹配校验成功 expect_Expression中 %s == response中 %s' % (re_data, resp_value))
                    return True  # 正则校验成功后就不去继续校验相等，否正会比对失败，抛出不必要的异常
                else:
                    log.error('29 fail 正则匹配校验失败 expect_Expression中 %s != response中 %s' % (re_data, resp_value))
                    log.error(error_msg)
                    raise Exception(error_msg)  # 失败后抛异常
            elif expect_Expression == resp_value:
                log.info('32 pass: %s 值校验：expect中 %s == response中 %s' % (key, expect_Expression, resp_value))
    ...
    def is_dict_or_None_dict(self, expect, response, error_msg):
        ...
        if isinstance(expect, dict) and isinstance(response, dict):  # 非字典进入主程序下一步
            if dict(expect) == {} and dict(response) == {}:  # 校验返回空
                log.info("43 pass expect: %s == response: %s" % (expect, response))
            elif dict(expect) != {} and dict(response) == {} or dict(expect) == {} and dict(
                    response) != {}:  # 预期不为空但返回空 or #预期空但返回不为空
                log.error("46 fail expect: %s != response: %s" % (expect, response))
                log.error(error_msg)
    ...
    def __list_value_assert(self, e_k, r_k, e_v, r_v, error_msg):
        ...
        if e_v == type(r_v):  # list中的type校验
            log.info('75 pass: value list type 校验: expect中 %s: %s == response中 %s: %s' % (e_k, e_v, r_k, r_v))
        elif e_v == r_v:  # list中值判断
            log.info('77 pass: value list 值校验: expect中 %s: %s == response中 %s: %s' % (e_k, e_v, r_k, r_v))
        ...
        if e_v not in (int, str, float) and not isinstance(e_v, int):
            if 're$' not in str(e_v) and type(e_v) != type(r_v) != list:
                log.error('81 fail: value list expect中 %s: %s != response中 %s: %s' % (e_k, e_v, r_k, r_v))
                log.error(error_msg)
                raise Exception(error_msg)
        elif e_v != type(r_v) and e_v != r_v:
            log.error('84 fail: value list type 校验: expect中 %s: %s != response中 %s: %s' % (e_k, e_v, r_k, r_v))
            log.error(error_msg)
            raise Exception(error_msg)
        ...
        for e_key, e_valuetype in e_dic.items():
            for r_k, r_v in r_dic.items():
                ...
                if e_key in r_dic.keys():
                    ...
                    if e_key == r_k:
                        log.info('102 pass: key值校验：expect中 %s == response中 %s' % (e_key, r_k))
                        if self.assert_re_Expression_data(e_key, e_valuetype, r_v,
                                                          error_msg):  # 正则校验或值比对e_valuetype == r_v
                            pass
                        elif e_valuetype == r_type:
                            log.info('106 pass: value type校验: expect中 %s: %s == response中 %s: %s' % (
                            ...
                            log.info('111 pass: list type 校验: expect中 %s: %s == response中 %s: %s' % (
                                e_key, e_valuetype, r_k, r_type))
                            ...
                            log.info('116 pass: value dict type 校验: expect中 %s: %s == response中 %s: %s' % (
                                e_key, e_valuetype, r_k, r_type))
                        else:
                            log.error('119 fail: expect中 %s: %s != response中 %s: %s' % (e_key, e_valuetype, r_k, r_v))
                            log.error(error_msg)
                            raise Exception(error_msg)
                else:
                    log.error('122 fail: expect中的：{ %s: %s } ； 在response中未找到: %s' % (e_key, e_valuetype, response))
                    log.error(error_msg)
                    raise Exception(error_msg)
    ...
    def __Validator(self, expect_, response_, error_msg):
        ...
        if type(expect) in types or type(response) in types or None in (expect, response):  # 只要有一个不是字典就直接比对
            if expect == response:
                log.info('163 pass: expect中 %s == response中 %s' % (expect, response))
            else:
                log.error('165 fail: expect中 %s != response中 %s' % (expect, response))
                log.error(error_msg)
                raise Exception(error_msg)
        else:
...
if __name__ == '__main__':
    # 字典 多层嵌套
    # response = {"count":33,"nas":{"name":{"name1":"jod","age":{"ag":18}}}}
    # expect = {"count":int,"nas":{"name":{"name1":str,"age":{"ag":int}}}}

    # list 嵌套相同key的字典，以及list中验证str和int不能验证str和int的具体值,
    # response = {"names":["1","2","3"],"jod":[1,2,3],"age":{"agee":[{"key":1},{"key":1}]},"pople":[{"1":1}]}
    # expect = {"names":[str],"jod":[int],"age":{"agee":[{"key":int}]},"pople":[{"1":1}]}

    # 校验key对应的value实际值相等
    # response = {"name": {"n": 123}, "age": [11, 22, 1222]}
    # expect = {"name": {"n": 're$^\d[1-2]'}, "age": ['re$^\d[1-2]']}
    ...
    response = False
    expect = True
    # response = {"ss":33}  #bool校验使用字典嵌套
    # expect = {"ss":int}

    # list正则校验值或值类型校验
    # response = {"n": [123, 22, 111111, 12121212, 222, "12"]}
    # expect = {"n": ['re$\d']}
    # expect = {"n": [int]}

    s = Validator_Sahala()
    s.data_validator(expect, response, "断言失败")

# 校验结果：
#[2019-08-17 16:39:54,916] - log_common.py] - INFO:
# ================Data Validator START !================
# [2019-08-17 16:39:54,918] - log_common.py] - INFO: 163 pass: expect中 False == response中 False
