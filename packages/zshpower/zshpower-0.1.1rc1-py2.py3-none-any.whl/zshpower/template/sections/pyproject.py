...
function snakypy_pyproject () {
    ...
    snakypy__pyproject_prefix1="$(snakypy::fg red "" "$SNAKYPY__SEPARATOR" " ")"
    snakypy__pyproject_prefix2="$(snakypy::fg "$SNAKYPY__PYPROJECT_PREFIX_COLOR" "" \\
"$SNAKYPY__PYPROJECT_PREFIX_TEXT" " ")"
    [[ -f "$(pwd)/pyproject.toml" ]] && \\
    get_version=$(< pyproject.toml sed -n '/^version/p' | cut -d'"' -f2 | cut -d"'" -f2)
...
"$(snakypy::icon "\\uf8d6 " "pkg-")${get_version}" " ")"
    ...
    [[ $SNAKYPY__PYPROJECT_ENABLE == True ]]; then
        echo "${snakypy__pyproject_prefix1}${snakypy__pyproject_prefix2}${snakypy__pyproject}"
    fi
}
