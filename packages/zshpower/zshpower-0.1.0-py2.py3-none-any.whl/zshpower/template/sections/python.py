...
function snakypy_py_version () {
    ...
    local search_files
    snakypy__python_version_prefix1="$(snakypy::fg red "" "$SNAKYPY__SEPARATOR" " ")"
    snakypy__python_version_prefix2="$(snakypy::fg "$SNAKYPY__PYTHON_PREFIX_COLOR" "" \\
"$SNAKYPY__PY_VERSION_PREFIX_TEXT" " ")"
    [[ $SNAKYPY__PY_VERSION_MINOR_ENABLE == True ]] && snakypy__python_version_minor=".{0[1]}"
    [[ $SNAKYPY__PY_VERSION_MICRO_ENABLE == True ]] && snakypy__python_version_micro=".{0[2]}"
...
"$(snakypy::icon "\\uf81f " "py-")${get_version}" " ")"
    search_files=(__pycache__ manage.py setup.py __init__.py .python-version \\
