...
class Banks(ListoAPI):
    ...
    def upload_bank_files(self, filename, bank=None, account_number=None, currency=None, rfc_id=None):
        return self.make_request(method="POST", path="banks/upload_bank_files", file=open(filename, 'rb'),
          data= {"bank": bank,"account_number": account_number,"currency": currency,"rfc_id": rfc_id})\
               .json()['status']
    
    def get_bank_transaction(self, **kwargs):
