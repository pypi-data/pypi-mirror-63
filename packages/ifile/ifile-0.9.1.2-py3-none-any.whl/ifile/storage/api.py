...
class File(object):
    ...
    def add_from_ftp(self, file_uuid, md5=None):
        if md5 is None:
            values = {"uuid": file_uuid}
        else:
            values = {"uuid": file_uuid, "md5": md5}
        _file = db_api.update_file(self._id, values)
        return _file

    def update(self, **kwargs):
        _file = db_api.update_file(self._id, kwargs)
        return _file
...
class Files(object):
    ...
    def add_from_ftp(self, name, file_uuid, md5=None):
        file_db = db_api.add_file(name)
        _file = File(file_db.id)
    ...
    def update_file(self, file_uuid, **kwargs):
        file_db = db_api.get_file_by_uuid(file_uuid)

        _file = File(file_db["id"])
        _file.update(**kwargs)

        self._refresh()
        return self[file_db["id"]]
