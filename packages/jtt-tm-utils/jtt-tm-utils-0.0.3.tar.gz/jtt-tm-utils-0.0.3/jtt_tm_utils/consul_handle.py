...
class ConsulReader():
    ...
    def read_services(self,service_name,tag,raise_error=False,**args):
        try:
    ...
    def read_service(self,service_name,tag,raise_error=False,**args):
        srvs = self.read_services(service_name,tag,raise_error,**args)
        if len(srvs) <= 0:
