...
from linkkt_end.utils.trades.trade import Trade, DynamicTrade, TradeType
...
if __name__ == "__main__":
    simulation_creator = SimulationCreator(length=2000, ecount=4)
            ...
            while True:
                ...
                if current_portfolio.is_user_asset(active):
                    ...
                    if random.uniform(0, 1) < 0.3:
                        ...
                        # logger.warning(f"Making a trade for the asset: {symbol}")
                ...
                if current_requirements.is_valid:
                    ...
                    process_rebalance(current_portfolio, current_requirements)
                    # executor.submit(process_rebalance, current_portfolio, current_requirements)
                    # Put this in some background process
                    break
        print()
        print()
    ...
    # current_active = copy.copy(active_assets)
    # current_portfolio = simulation_creator.portfolio
    # logger.info(current_portfolio.users_assets)
    # random.shuffle(current_active)
    # logger.debug(current_portfolio['episode'])
    # simulation_creator.requirements

    # current_requirements = simulation_creator.requirements.unique()

    # # current_requirements.reset()
    # logger.info(current_portfolio.users_assets) 
    # print(user_assets)
    # logger.info(magenta(current_portfolio.pricing.data))
    # logger.info(magenta(current_portfolio.pricing._query))
    # logger.info(user_assets)
    # count = 0
    # while True:
    #     logger.info(magenta(current_portfolio.users_assets))
    #     # logger.info(magenta(current_portfolio.pricing._query))
    #     if count == 10:
    #         break
    #     count += 1
    #     # current_asset = current_active.pop()
    #     # current_asset['episode'] = simulation_creator.current_episode
    #     # current_requirements.report(current_asset)
    #     # # Create function to determine asset in list
    #     # if current_portfolio.is_user_asset(current_asset):
    #     #     logger.warning("Asset is is in user's portfolio")
    #     #     symbol = current_asset['name']
    #     #     if random.uniform(0, 1) < 0.3:
    #     #         # Get the user's assets
