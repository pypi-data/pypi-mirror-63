...
import dask
import numpy as np
from sympy import diff, solveset, Reals, Interval
...
def candidate_thresholds(image):
    ...
    img = utils.svs2dask_array(image)[10000:12000, 10000:12000]
    ...
    def get_inflection_points(pwm_curve):
        ...
        inflection_points = solveset(diff(diff(function)), domain=Reals)
        return inflection_points
    inflection_points = get_inflection_points(pwm_curve)

    @dask.delayed(pure=True)
    def get_valid_inflection_points(inflection_points, N):
        return list(inflection_points.intersect(Interval(0, N)))
    valid_thresholds = get_valid_inflection_points(inflection_points, N)
