...
class PluginManager(BaseManager):
    ...
    def _get_installed_plugin(self, plugin_id):
        return self._installed_plugin_model.get(plugin_id=plugin_id)
    ...
    def update_plugin(self, params):
        ...
        plugin_vo: InstalledPlugin = self._get_installed_plugin(params['plugin_id'])
    ...
    def activate_plugin(self, params):
        ...
        plugin_vo: InstalledPlugin = self._get_installed_plugin(plugin_id=params['plugin_id'])
        self.transaction.add_rollback(_rollback, plugin_vo.to_dict())
    ...
    def make_reprovision(self, plugin_id):
        def _rollback(old_data: dict):
            plugin_vo.update(old_data)

        plugin_vo: InstalledPlugin = self._get_installed_plugin(plugin_id=plugin_id)
        self.transaction.add_rollback(_rollback, plugin_vo.to_dict())
    ...
    def get_credentials(self, credential_id, domain_id):
        ...
        return connector.issue_credentials(credential_id, domain_id)

    def call_verify_plugin(self, plugin_endpoint, options, credentials):
