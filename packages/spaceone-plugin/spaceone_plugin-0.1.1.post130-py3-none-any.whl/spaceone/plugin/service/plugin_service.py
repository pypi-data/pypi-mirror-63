...
from spaceone.plugin.model import SupervisorRef, InstalledPlugin
...
class PluginService(BaseService):
    ...
    def verify(self, params: dict):
        """ Verify options and credential is correct information for specific plugin
        ...
        if 'credential_id' in params:
            # Patch credentials from secret
            credentials = self.plugin_mgr.get_credentials(params['credential_id'])
        else:
            # Empty credentials
            credentials = {}
        ...
        return self.plugin_mgr.call_verify_plugin(plugin_endpoint, options, credentials)

    def _create_supervisor_reference(self, supvr, domain_id) -> SupervisorRef:
        params = {
