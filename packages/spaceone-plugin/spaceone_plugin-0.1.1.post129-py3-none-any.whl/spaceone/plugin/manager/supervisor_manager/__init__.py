...
from spaceone.plugin.error.custom import ERROR_SUPERVISOR_UPDATE
from spaceone.plugin.manager.supervisor_manager.supervisor_state import SupvrStateMachine
from spaceone.plugin.model import InstalledPlugin
from spaceone.plugin.model import Supervisor
...
class SupervisorManager(BaseManager):
    def __init__(self, transaction):
        ...
        self._supervisor_model: Supervisor = self.locator.get_model('Supervisor')
...
#    def create_or_update_supervisor(self, params):
#        self._update_domain_id()
#        print(f'[create_or_update_supervisor] params: {params}')
#        # find supervisor from Supervisor.
#        try:
#            supervisor = self.get_by_hostname(params['hostname'], self._domain_id)
#        except ERROR_NOT_FOUND as e:
#            print('not exists')
#            supervisor = None
#
#        if supervisor is None:
#            supervisor = self.create(params)
#        else:
#            supervisor = supervisor.update(params)
#
#        return supervisor

#    def get_public_or_domain_supervisors(self, domain_id):
#        query = _query_domain_id_or_public(domain_id)
#        return self.list_supervisors(query)
