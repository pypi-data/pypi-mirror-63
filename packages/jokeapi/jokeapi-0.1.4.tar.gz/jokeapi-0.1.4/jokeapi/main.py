...
class Jokes:
    ...
    def build_request(
        ...
        if len(blacklist) > 0:
            for b in blacklist:
                if b not in ["nsfw", "religious", "political", "racist", "sexist"]:
                    raise ValueError(
                        """\n\n
        ...
        if response_format not in ["json", "xml", "yaml"]:
            raise ValueError("Response format must be either json, xml or yaml.")
        if type:
        ...
        if id_range:
            ...
            if len(id_range) > 2:
                ...
                raise ValueError("id_range[0] must be greater than or equal to 0.")
