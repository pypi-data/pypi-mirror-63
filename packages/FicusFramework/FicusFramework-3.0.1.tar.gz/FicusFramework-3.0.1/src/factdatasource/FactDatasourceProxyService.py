...
class FactDatasourceProxy(object):
    ...
    @abstractmethod
    def collect_conditions(self, fd_code: str, size: int, condition_groups: list):
        """
        返回指定查询条件的数据
        :param fd_code:
        :param size: 返回的条数
...
class CentralizedFactDatasourceProxy(FactDatasourceProxy, Singleton):
    ...
    def collect_conditions(self, fd_code: str, size: int, condition_groups: list):
        ...
        try:
            r = do_service(f"/remote/fd-service/{fd_code}/conditions", method="post", data=condition_groups, params={'size': size})
            if r is not None:
                if isinstance(r, list):
                    return [Munch(x) for x in r]
                else:
