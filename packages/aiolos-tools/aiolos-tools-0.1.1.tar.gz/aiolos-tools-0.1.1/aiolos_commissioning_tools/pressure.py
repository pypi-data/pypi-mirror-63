...
def process_boundary_layer_data(run_id, scanner_data, tunnel_data, scanner_channels, rake, probe) :
    ...
    Ue= wind_speed_from_pitot (pt, ps, pabs, probe.correction, total_temperature, dew_point_temperature)

    # calculate boundary layer parameters
    boundary_layer_thickness    = fluids.boundary_layer_thickness(rake.z, u, Ue)
    displacement_thickness      = fluids.dispacement_thickness(rake.z, u, Ue)
    momentum_thickness          = fluids.momentum_thickness(rake.z, u, Ue)
    shape_factor = displacement_thickness / momentum_thickness

    result = BoundaryLayerData(run_id=run_id, z=z, po=po, ps=ps, u=u, Ue=Ue, boundary_layer_thickness=boundary_layer_thickness,
                               displacement_thickness=displacement_thickness,  momentum_thickness=momentum_thickness, shape_factor=shape_factor)
    log.debug(result.__repr__())
    return result
