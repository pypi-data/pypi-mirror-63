...
class BlobAccessor(object):
    ...
    def get_file_dataset(
            self,
            **kwargs) -> FileDataset:
        ds = FileDataset._create(self.get_file_dataflow(**kwargs))
    ...
    def get_file_dataflow(self, **kwargs) -> dprep.Dataflow:
        ...
        # dflow = self._convert_cdn_dataflow(dflow) skip for now and wait for DataPrep Official release to studio
        return dflow
    ...
    def get_tabular_dataset(
        ...
        ds = TabularDataset._create(dflow)\
    ...
    def _get_cdn_base_url(self) -> str:
        return 'https://%s.azureedge.net/%s/' % (
            self._blob_account_name,
            self._blob_container_name)
    ...
    def _get_relative_path(self, url: str) -> str:
        if "blob.core.windows.net" in url:
            ...
            return url.replace(self._get_cdn_base_url(), "")
