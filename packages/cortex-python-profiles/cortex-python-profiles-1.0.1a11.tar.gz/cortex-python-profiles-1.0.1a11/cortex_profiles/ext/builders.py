...
class BulkProfilesBuilder(AbstractProfilesBuilder):
    ...
    def __init__(self, cortex_client: CortexClient, schemaId: Optional[str] = None, db_uri: Optional[str] = None):
        ...
        log.info("Done Instantiating BulkProfilesBuilder")
    ...
    def with_events(self, event_chunk: Iterable[EntityEvent], **ee_to_attr_convertor_kwargs):
        ...
        :return:
        """
        log.info("Casting Events")
        casted_events = (self.cast_event(ee) for ee in event_chunk)
        log.info("Teeing Casted Events into separate streams")
        event_chunk_for_events, event_chunk_for_invalid_events, event_chunk_for_attrs = tee(casted_events, 3)
        # Add chunk of raw events to save ...
        log.info("Adding stream of valid events")
        self._events.append(self.stream_valid_events(event_chunk_for_events))
        # Add chunk of invalid events
        log.info("Adding stream of valid invalid events")
        self._invalid_events.append(self.stream_errors(event_chunk_for_invalid_events))
        # Add chunk of attributes to save ...
        log.info("Adding stream of attributes")
    ...
    def build(self, verbose=False, chunk_size=10_000) -> Tuple[List,List]:
        ...
        * This method logs progress as chunks are incrementally saved ...
        ...
        invalid_event_chunk_stream = enumerate(chunk_iterable(invalid_event_stream, chunk_size))

        log.info('Initializing Counters / Output')
        ...
        log.info('Consuming Initial Chunks')
        (next_attr_chunk, next_event_chunk, next_invalid_event_chunk) = (
            next(attribute_chunk_stream, None),
            next(event_chunk_stream, None),
            next(invalid_event_chunk_stream, None),
        )

        while (next_attr_chunk is not None) or (next_event_chunk is not None) or (next_invalid_event_chunk is not None):
            profile_ids_to_flush = set([])
            # Insert Each Chunk of attributes
            if next_attr_chunk:
                log.info('Handling Chunk of Attributes')
                (
                    new_profile_ids,
                    new_insert_records,
            ...
            if next_event_chunk:
                log.info('Handling Chunk of Events')
                (
                    new_profile_ids,
                    new_insert_records,
            ...
            if next_invalid_event_chunk is not None:
                log.info('Handling Chunk of Invalid Events')
            ...
            log.info('Consuming Next Chunks')
            (next_attr_chunk, next_event_chunk, next_invalid_event_chunk) = (
                next(attribute_chunk_stream, None) if next_attr_chunk is not None else None,
                next(event_chunk_stream, None) if next_event_chunk is not None else None,
                next(invalid_event_chunk_stream, None) if next_invalid_event_chunk is not None else None
            )
