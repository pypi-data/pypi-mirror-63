    ...
    def build(self, verbose=False, chunk_size=10_000) -> Tuple[List,List]:
        ...
        * This method logs progress as chunks are incrementally saved ...
        ...
        (next_attr_chunk, next_event_chunk, next_invalid_event_chunk) = (
            next(enumerate(chunk_iterable(attribute_stream, chunk_size)), None),
            next(enumerate(chunk_iterable(event_stream, chunk_size)), None),
            next(enumerate(chunk_iterable(invalid_event_stream, chunk_size)), None),
        )
        while (next_attr_chunk is not None) or (next_event_chunk is not None) or (next_invalid_event_chunk is not None):
            ...
            (next_attr_chunk, next_event_chunk, next_invalid_event_chunk) = (
                (
                    next(enumerate(chunk_iterable(attribute_stream, chunk_size)), None)
                    if next_attr_chunk is not None else None
                ),
                (
                    next(enumerate(chunk_iterable(event_stream, chunk_size)), None)
                    if next_event_chunk is not None else None
                ),
                (
                    next(enumerate(chunk_iterable(invalid_event_stream, chunk_size)), [])
                    if next_invalid_event_chunk is not None else None
                )
            )
