    ...
    def download(
        ...
        with tempfile.TemporaryDirectory() as snapshot_dir:
            ...
            for additional_file in additional_files:
                shutil.move(
                    os.path.join(snapshot_dir, additional_file[1:]), target_directory
                )
...
def create_snapshot(
    ...
    with open(os.path.join(project_dir, project_definition.cluster_yaml())) as f:
...
def download_snapshot(
    name: str, credentials: Dict[str, str], target_directory: Optional[str] = None
) -> str:
