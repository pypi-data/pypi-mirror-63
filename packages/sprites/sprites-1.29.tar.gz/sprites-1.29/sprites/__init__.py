"""
   ...
   49、ishide：返回角色是否是隐藏的。
   50、write2：用来写有阴影的字，有前景bg和fg参数，默认为中间对齐，宋体16号字。
   51、find_overlapping：查找重叠命令。本命令用来查找有无和角色的矩形重叠的项目(包括画笔线条，圆点，多边形，圆弧，填充区域，图章，其它角色)，可以有一个参数，叫排除参数。本命令返回找到的项目编号集合。
   52、overlap_with：重叠命令。本命令查找有无和角色最小矩形重叠的项目（包括线条，圆点，多边形，圆弧，图章，填充区域，其它角色)。它的参数可以是序列、整数与角色或字符串，返回重叠的所有项目编号。
   53、topleft：到左上角，返回坐标。
   54、topright：到右上角，返回坐标。
   55、bottomleft：到左下角，返回坐标。
   56、bottomright：到右下角，返回坐标。
   57、arc：画圆弧，实际上是一个派形图。参数为radius半径，start起始角，extent结束角，width线宽，fill填充颜色与outline边框颜色 ，返回项目编号。
   58、polygon：画多边形，以角色所在坐标为第一个点画多边形，参数为其它坐标列表，width线宽，fill填充颜色与outline边框颜色 ，返回项目编号。
...
class Sprite(Turtle):
    ...
    def arc(self,radius,start=0,extent=45,width=0,fill=None,outline=None):
        """画弧
           radius：弧所在圆形的半径
           start：起始角
           extent：结束角
        ...
        """
        if fill == None:fill = self.fillcolor()
        if outline == None:outline = self.pencolor()
        x,y = self.pos()
        x0,y0 = x - radius,-(y + radius)
        x1,y1 = x + radius, radius - y
        coord = (x0,y0,x1,y1)
    ...
    def polygon(self,coords,width=0,fill=None,outline=None):
        """连点画多边形
           coords：多边形每个顶点坐标
           width：边框值
           fill：填充颜色，如果为空字符串，则是没有颜色
    ...
    def dot(self,size=None,*color):
        ...
        color -- 颜色字符串或三元组，a colorstring or a numeric color tuple       
...
if __name__ == "__main__":  
  ...
  blackborder = w.polygon(cors,fill="")
  
  cat1 = Sprite(3,tag='cc')
  cat2 = Sprite(3,tag='cc',pos=(100,0))
                
  s = Sprite(pos=(-250,-50))
  s.fillcolor("red")
  hu = s.arc(100,10,100,40,'','brown')
  dots = []
  for x in range(10):
    dots.append(s.dot(10))
    s.fd(50)
  while True:
    s.goto(mouse_pos())
    r = s.overlap_with(dots)
    if r:print(r)
    c = s.overlap_with('cc')
    if c:print('碰到猫',c)
    x = s.overlap_with(hu)
    if x : print('碰到饼')
    y = s.overlap_with(blackborder)
    if y :print('碰到黑边')
    s.update()
