...
class DEWAKSS(_BaseEstimator, _TransformerMixin):
    ...
    def __init__(self,
        ...
        Params:
        -------
        :param data: an object containing the data to be denoised [AnnData object, numpy.ndarray, scipy.csr_matrix].
        ...
        :param n_neighbors: number of k nearest neighbors to use and iterate over.
        :param n_pcs: number of PCs to use and iterate through.
        :param mode: [*] 'distances' or 'connectivities'. If data is AnnData will select the appropriate measure for the Affinity graph. Default: 'distances'.
        :param use_global_err: Boolean. If global neighbors should be used or local cell vise error. Default True.
        :param iterations: Number of diffusion steps to perform if run2best is False. Parameter will be ignored if other value is supplied in the fit function. Default is None (1) step, i.e. the weighted kNN is used without diffusion.
        :param weighted: [*] If False will use the initial Affintiy matrix with equally weighted neighbour influences. This will change when diffusing.
        ...
        :param set_diag: Explicitly set the diagonal entries of the affinity matrix at each step to this value before transforming to right stochastic matrix. Default 0
        :param symmetrize: Transform the initial affinity matrix to a symmetric matrix by summing M + M.T. Default True
        :param denoise_type: what operation should be performed to denoise, ['mean', 'median']. Default 'mean'. (median is very slow!)
        ...
        :param settype: type that the graph should be converted too. Default numpy.float32
        :returns: self
        :rtype: DEWAKSS
        ...
        self.neighbour_args = neighbour_args
        self._copy_input = copy

        self._parse_input_data(data)
        # if (data is None) and (graph is None):
        #     raise ValueError(f"Either input argument 'data' or 'graph' needs to be given a value")
        # elif data is not None:
        #     self._data_is_graph = False
        #     self._parse_input_data(data)
        # elif graph is not None:
        #     self._data_is_graph = True
        #     self._graph = graph
        #     self._data = None
    ...
    def _iterate_pcs(self, X, iterations=None):
        ...
        for pc in PCs:
            ...
            if self.verbose:
                print(f'Computing {N_tmp} neighbours with nPCs = {pc}')
            _sc.pp.neighbors(self._data, n_neighbors=N_tmp, n_pcs=pc, **self.neighbour_args)
            graph = self._data.uns['neighbors'][mode].copy().astype(self._settype)
            self._iterate_neighbours(X, graph, pcs=pc, iterations=iterations)

    def _iterate_neighbours(self, X, graph, pcs=None, iterations=None):

        Ns = self.n_neighbors if hasattr(self.n_neighbors, '__iter__') else [self.n_neighbors]
        for N in Ns:
            if self.verbose:
                print(f'Compute objective for PCs={pcs} N={N}')
    ...
    def plot_global_performance(self, ax=None, metric='mse', verbose=None, skipfirst=True, plot_style='seaborn-poster'):
        ...
        :param skipfirst: Should the first unfit MSE be shown.
    ...
    def _get_optimal_connectivities(self):
        ...
        if self.verbose:
            print(f'Computing {N_tmp} neighbours with nPCs = {PCs}')
        _sc.pp.neighbors(self._data, n_neighbors=N_tmp, n_pcs=PCs, **self.neighbour_args)
        if self.verbose:
            print('Done!\nProceeding to compute local neighbour optima.')
        graph = self._data.uns['neighbors'][mode].copy().astype(self._settype)
        M, J = graph.shape
        opt_connectivities = _csr.csr_matrix((M, J)).astype(self._settype)
        for pcs, N, It in self._run_configs_:

            if not _np.any(self._cell_optimal_config_.index.isin([(pcs, N, It)])):
                continue
    ...
    def _find_local_denoising(self):
        ...
        if (not self.use_global_err) or (not self._local_optima_fit):
    ...
    def SNR(self, data, Xmean=None, copy=True):
        ...
        if isinstance(data, _AnnData):
            ...
            adata.var['SNR'] = feature_variance
        ...
        for var, vec in zip(feature_variance, Xmean.T):
            feature_SNR.append(_np.linalg.norm(vec) / _np.sqrt(chi2inv * var))

        feature_SNR = _np.array(feature_SNR)

        data_SNR = _np.linalg.norm(Xmean) / _np.sqrt(_chi2.ppf(1 - self._alpha, _np.prod(Xmean.shape) - 1) * data_variance)
    ...
    def _compute_cell_performance(self, optkey='MSE'):
        ...
        for mse, df in grp:
            ...
            self._cell_optimal_config_.index.names = ('PCs', 'neighbours', 'i')
    ...
    def transform(self, data, copy=True, transformtype=None):
        ...
        if isinstance(data, _AnnData):
            ...
            adata.uns['dewakss'] = {self.create_layer: {'dewakss_connectivities': self.opt_connectivities, "opt_iterations": self.opt_iterations, "mode": self.mode, 'layer': self.use_layer}}
