...
def iter_devices(
    ...
    if device:
        ...
        devices = (
            d
            for d in devices
            if fnmatch.fnmatch(d.name.lower(), device)
            or (d.alias and fnmatch.fnmatch(d.alias.lower(), device))
        )
    if klass:
    ...
    if host:
        host = host.lower()
        devices = (
            d for d in devices if fnmatch.fnmatch(servers[d.server].host.lower(), host)
        )
    return devices
...
def device_attributes(dev_name, attribute=None):
    ...
    if attribute:
        ...
        def filt(attr):
            fnmatch.fnmatch(attr.name.lower(), attribute)
...
def device_commands(dev_name, command=None):
    ...
    if command:
        ...
        def filt(cmd):
            fnmatch.fnmatch(cmd.cmd_name.lower(), command)
...
def iter_starter_servers(
    ...
    if server_name:
        servers = (s for s in servers if fnmatch.fnmatch(s["info"].name, server_name))
    if server_type:
        servers = (s for s in servers if fnmatch.fnmatch(s["info"].type, server_type))
    if server_instance:
        servers = (
            s for s in servers if fnmatch.fnmatch(s["info"].instance, server_instance)
        )
    return servers
