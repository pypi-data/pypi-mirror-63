...
pair_class_device = click.option(
    '-d', '--device', required=True, type=(str, str), multiple=True,
...
verbose = click.option(
    '-v', '--verbose', default=False, show_default=True, is_flag=True,
    help='verbose output')

color = click.option(
    '--color/--no-color', default=True, show_default=True, is_flag=True,
    help='colored output')

timeout = click.option(
    '--timeout', type=float, default=None, show_default=True,
    help='operation timeout (s)')

stop_timeout = click.option(
    '--stop-timeout', type=float, default=5, show_default=True,
    help='timeout for the stop operation. If this timeout expires a kill is done')

dserver = click.option(
    '--exclude-dserver/--include-dserver', default=True,
    show_default=True, is_flag=True, help='exclude/include DServer devices')

server_type = click.option(
    '-t', '--server-type', default=None, required=True,
    help='server type')

server_type_filter = click.option(
    '-t', '--server-type', default=None,
...
attr_filter = click.option('-a', '--attribute', default=None, prompt=True,
    help='attribute name (supports pattern matching *,?,[])')

attrs_filter = click.option('-a', '--attribute', default=None, multiple=True,
    help='attribute name(s) (supports pattern matching *,?,[])')

attr = click.option('-a', '--attribute', required=True, prompt=True,
    help='attribute name')

attrs = click.option('-a', '--attribute', required=True,
    multiple=True, help='attribute name(s)')

attrs_arg = click.argument('attributes', required=True, nargs=-1)

cmd = click.option('-c', '--command', required=True, prompt=True,
    help='command name')

prop = click.option('-p', '--property', required=True, prompt=True,
    help='property name')

props_filter = click.option('-p', '--property', required=True, multiple=True,
    help='property name')

value = click.option('-v', '--value', required=True, prompt=True,
    help='value')

#: server option
server = click.option('-s', '--server', required=True, prompt=True,
    help='server name (<type>/<instance>)')

# list of server options
servers = click.option('-s', '--servers', required=True,
    multiple=True, help='server names (<type>/<instance>)')

#: device option
device = click.option('-d', '--device', required=True, prompt=True,
    help='device name')

# list of server options
devices = click.option('-d', '--devices', required=True,
    multiple=True, help='device names')

dry_run = click.option('--dry-run', default=False, show_default=True,
                       is_flag=True, help='dry run (=simulation)')
...
def _attribute_info_table(device, attribute):
    ...
    return [
        ...
        ['Range', '({}, {})'.format(tangoctl.value_str(info.min_value),
                                    tangoctl.value_str(info.max_value))],
        ['Alarms', '({}, {})'.format(tangoctl.value_str(info.alarms.min_alarm),
                                     tangoctl.value_str(info.alarms.max_alarm))],
        ['Warnings', '({}, {})'.format(tangoctl.value_str(info.alarms.min_warning),
        ...
        ['Arch. abs. change', tangoctl.value_str(info.events.arch_event.archive_abs_change)],
        ['Arch. rel. change', tangoctl.value_str(info.events.arch_event.archive_rel_change)],
...
def device_list(device, class_, server, host, nb_cols, exclude_dserver):
    ...
    with error():
        devices = tangoctl.iter_devices(device=device, klass=class_, server=server,
                            host=host, exclude_dserver=exclude_dserver)
...
def device_table(device, class_, server, host, color, exclude_dserver):
    ...
    with error():
        ...
        devices = tangoctl.iter_devices(db=db, device=device, klass=class_,
                                        server=server, host=host,
                                        exclude_dserver=exclude_dserver)
        table = []
        for dev in devices:
            srv = db_info.servers[dev.server]
            row = [_device_str(dev, color), _alias_str(dev, color),
                   _device_class_str(dev, color), _server_str(srv, color),
...
def device_tree(device, class_, server, host, color, verbose, exclude_dserver):
    ...
    with error():
        ...
        devices = tangoctl.iter_devices(device=device, klass=class_,
            server=server, host=host, exclude_dserver=exclude_dserver, db=db)
...
@device_group.command('ping',
    help='ping device(s) (supports pattern matching *,?,[])')
@devices
@color
@dserver
def device_ping(devices, color, exclude_dserver):
    ...
    """
    with error():
        devices = tuple(tangoctl.expr_to_idevices(devices,
            exclude_dserver=exclude_dserver))
        if not devices:
...
def device_command_list(device, nb_cols):
    ...
    with error():
        ...
        cmds_names = [cmd.cmd_name for cmd in cmds]
...
def device_command_table(device):
    ...
    with error():
        for cmd in tangoctl.device_commands(device):
            arg = type_str(cmd.in_type)
            res = type_str(cmd.out_type)
    ...
    with error():
        ...
        for dev_name, values in tangoctl.attributes_read({device:attribute}):
            if isinstance(values, Exception):
                ...
                click.secho(msg, fg='bright_red', color=color)
            else:
                for value in values:
...
def device_info(device):
    ...
    with error():
        ...
        lines = [templ.format(name, r[name]) for name in sorted(dinfo)]
        click.echo('\n'.join(lines))
...
def server_instance_list(server_type, server_instance, nb_cols):
    ...
    """
    with error():
        servers = list(tangoctl.iter_servers(server_type=server_type,
    ...
    with error():
        for server in servers:
            ...
            try:
                ...
                click.secho('[ERROR]', fg='bright_red', color=color, nl=False)
                click.echo(msg)
...
def server_info(server):
    ...
    with error():
        ...
        lines = [templ.format(name, r[name]) for name in sorted(sinfo)]
        click.echo('\n'.join(lines))
...
@server_instance_filter
@dserver
def server_tree(compact, server_only, server, server_type, server_instance, exclude_dserver):
    """
    ...
    servers = tangoctl.iter_servers(server=server, server_type=server_type,
                                    server_instance=server_instance, db=db)
        ...
        for serv in servers:
            ...
            if not server_only:
                for device in sorted(serv.devices):
                    if exclude_dserver and device.lower().startswith('dserver/'):
                        continue
...
@server_type_filter
@server_instance_filter
@click.option('--all', default=False, show_default=True, is_flag=True,
...
def starter_tree(starter, server, server_type, server_instance, all):
    ...
    tree = treelib.Tree()
    db_node = tree.create_node(db_name)
    starters = list(tangoctl.iter_starters(starter=starter, db=db))
    ...
    for starter in starters:
        ...
        servers = tangoctl.iter_starter_servers(servers, server, server_type, server_instance)
...
@stop_timeout
@dry_run
def starter_stop(starter, server, server_type, server_instance, timeout,
                 stop_timeout, dry_run):
    ...
    try:
        tangoctl.starter_stop_servers(
            starter_name=starter, server_name=server, server_type=server_type,
            server_instance=server_instance, timeout=timeout,
            stop_timeout=stop_timeout, dry_run=dry_run)
...
def starter_start(starter, server, server_type, server_instance, timeout, dry_run):
    ...
    try:
        tangoctl.starter_start_servers(
            starter_name=starter, server_name=server, server_type=server_type,
            server_instance=server_instance, timeout=timeout, dry_run=dry_run)
    except gevent.Timeout:
        click.secho('Operation timeout ({} s)'.format(timeout), fg='yellow')
...
@timeout
@dry_run
def starter_restart(starter, server, server_type, server_instance, timeout,
                    dry_run):
