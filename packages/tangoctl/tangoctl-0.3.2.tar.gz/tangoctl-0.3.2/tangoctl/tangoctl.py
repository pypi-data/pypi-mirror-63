...
class Starter:
    ...
    def kill_server(self, name, dry_run=False):
        ...
        if not dry_run:
            ...
            state = self.wait_for_state(
                name, lambda s: s in ('MOVING', 'FAULT'))
            if not state:
    ...
    def stop_kill_server(self, name, timeout=5, dry_run=False):
        ...
        try:
            ...
            self._log.warning(
                'timeout trying to stop %s. Performing a kill', name)
            stopped = False
            task.kill()
        if not stopped:
...
def starter_stop_servers(
        starter_name=None, server_name=None, server_type=None,
        server_instance=None, timeout=None, stop_timeout=5,
    ...
    with gevent.Timeout(timeout):
        ...
        for level in sorted(levels, reverse=True):
            ...
            for starter, servers in levels[level].items():
                ...
                tasks = [gevent.spawn(stop, server['name'],
                                      timeout=stop_timeout, dry_run=dry_run)
                         for server in servers]
...
def starter_start_servers(
        starter_name=None, server_name=None, server_type=None,
        server_instance=None, timeout=None, dry_run=False, db=None):
