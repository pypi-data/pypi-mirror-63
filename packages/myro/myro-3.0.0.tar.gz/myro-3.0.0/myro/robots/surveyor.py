...
class CameraWindow(tkinter.Toplevel):
    def __init__(self, robot):
        ...
        self.canvas.bind(
            "<B1-Motion>",
            func=lambda event=self: self.dispatch_event(event, "motion", 0),
        )
        self.canvas.bind(
            "<Button-1>", func=lambda event=self: self.dispatch_event(event, "down", 0)
        )
        self.canvas.bind(
            "<ButtonRelease-1>",
            func=lambda event=self: self.dispatch_event(event, "up", 0),
        )
        self.canvas.bind(
            "<B2-Motion>",
            func=lambda event=self: self.dispatch_event(event, "motion", 1),
        )
        self.canvas.bind(
            "<Button-2>", func=lambda event=self: self.dispatch_event(event, "down", 1)
        )
        self.canvas.bind(
            "<ButtonRelease-2>",
            func=lambda event=self: self.dispatch_event(event, "up", 1),
        )
        self.canvas.bind(
            "<B3-Motion>",
            func=lambda event=self: self.dispatch_event(event, "motion", 2),
        )
        self.canvas.bind(
            "<Button-3>", func=lambda event=self: self.dispatch_event(event, "down", 2)
        )
        self.canvas.bind(
            "<ButtonRelease-3>",
            func=lambda event=self: self.dispatch_event(event, "up", 2),
        )
    ...
    def dispatch_event(self, event, action, bin):
        if action == "down":
            ...
            self.canvas.create_rectangle(
                x1 + 1, y1 + 1, x2 + 1, y2 + 1, outline="yellow", tag="box"
            )
            self.canvas.create_rectangle(x1, y1, x2, y2, outline="blue", tag="box")
        elif action == "motion":
    ...
    def update(self, image=None):
        ...
        for bin in self.blob:
            ...
            self.canvas.create_rectangle(x1, y1, x2, y2, outline=color, tag="box")
