...
class Fluke:
    ...
    def setForwardness(self, direction):
        if direction in ["fluke-forward", 1]:
            ...
            raise AttributeError(
                "unknown direction: '%s': should be 'fluke-forward' or 'fluke-forward'"
                % direction
            )
        try:
            self.lock.acquire()
    ...
    def darkenCamera(self, level=0):
        if self.debug:
            print(
                "Turning off White Balance, Gain Control, and Exposure Control", level
            )
    ...
    def setEchoMode(self, value):
        if isTrue(value):
            self._set(Fluke.SET_ECHO_MODE, 1)
        else:
            self._set(Fluke.SET_ECHO_MODE, 0)
        time.sleep(0.25)
        self.ser.flushInput()
        self.ser.flushOutput()
        return
    ...
    def set(self, item, position, value=None):
        ...
        if item == "led":
            if type(position) in [int, float]:
                if position == 0:
                    if isTrue(value):
                        return self._set(Fluke.SET_LED_LEFT_ON)
                    else:
                        return self._set(Fluke.SET_LED_LEFT_OFF)
                elif position == 1:
                    if isTrue(value):
                        return self._set(Fluke.SET_LED_CENTER_ON)
                    else:
                        return self._set(Fluke.SET_LED_CENTER_OFF)
                elif position == 2:
                    if isTrue(value):
                        return self._set(Fluke.SET_LED_RIGHT_ON)
                    else:
                        return self._set(Fluke.SET_LED_RIGHT_OFF)
                else:
                ...
                if position == "center":
                    ...
                    if isTrue(value):
                        return self._set(Fluke.SET_LED_ALL_ON)
                    else:
                        return self._set(Fluke.SET_LED_ALL_OFF)
                else:
                    raise AttributeError("no such LED: '%s'" % position)
