...
def _getFrequency(s, line, text):
    ...
    if len(s) > 0 and s[0].isalpha():
        if s.lower() in _frequency:
            ...
            raise ValueError(
                "invalid note name/frequency '%s' on line %d: %s" % (s, line, text)
            )
    else:
        return int(float(s))
...
def _getDuration(v, line, text):
    ...
    if "/" in v:
        try:
            ...
            raise ValueError(
                "invalid duration value '%s' on line %d: %s" % (v, line, text)
            )
    return float(v)
...
def song2text(song):
    ...
    for tup in song:
        if len(tup) == 2:
            ...
            f1, f2, d = tup
            text += "%s %s %s; " % (
                _getNoteFromFrequency(f1),
                _getNoteFromFrequency(f2),
                d,
            )
    return text
    ...
    if type(song) in [list]:
        for tup in song:
            if len(tup) == 2:
                ...
                fp.write(
                    "%s %s %s\n"
                    % (_getNoteFromFrequency(f), _getNoteFromFrequency(f), d)
                )
    else:  # string
        song = song.replace("\n", ";")
        lines = song.split(";")
...
def _parseSongLine(song, line, lineNumber, filename):
    ...
    if len(lineList) == 0:
        ...
        song.append(
            (
                _getFrequency(name1, lineNumber, line),
        ...
        raise ValueError(
            "song format error in '%s' at line %d: %s" % (filename, lineNumber, line)
        )
