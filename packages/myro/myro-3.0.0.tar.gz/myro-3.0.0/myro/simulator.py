...
# What is userspace?
# TODO: Reevaluate if this function even needs to exist.
# def INIT(filename):
    ...
    def __init__(self, dimensions, offsets, scale, root=None, run=0):
        ...
        menu = [
            ...
            (
                "Clear",
                [
                    ["lights", lambda: self.simClear("lights")],
    ...
    def dispatch_event(self, event, type):
        ...
        if self.mode == "draw":  # draw line to follow
            ...
            if type == "down":
                ...
                x, y = [float(v) / self.scale for v in (x, -y)]
                self.addShape(
                    "line", x, y, self._drawX, self._drawY, fill="black", width=0.03
                )
                self.redraw()
        ...
        for shape in self.shapes:
            if shape[0] == "line":
                ...
                if "width" in list(nargs.keys()):
                    ...
                    width = 0.03 / 2.0  # minimum width of a line
                seg = Segment((x1, y1), (x2, y2))
                angle = seg.angle()
                        ...
                        if name == "line":
                            ...
                            if intersect1 != None:
                                ...
                                if "width" in nargs:
                                    ...
                                    width = 0.03 / 2.0  # radius, in meters
                                if dist <= width:
                                    d.scan[pos] = 1
                                    self.drawRay(
                                        "line", x, y, x3, y3, self.colorParts[d.type]
                                    )
                                    break
                                else:
...
class MyroIR(RangeSensor):
    def __init__(self):
        RangeSensor.__init__(
            self,
            "ir",
            geometry=((0.09, 0.05, 0), (0.09, -0.05, 0)),
            arc=5 * PIOVER180,
            maxRange=0.35,
            noise=0.0,
        )
...
class MyroLightSensors(LightSensor):
    def __init__(self):
        LightSensor.__init__(
            self,
            (
                (0.06, -0.02, 20 * PIOVER180),
                (0.07, 0.0, 0),
                (0.07, 0.02, 20 * PIOVER180),
            ),
            noise=0.0,
        )
        self.groups = {
...
if __name__ == "__main__":
    ...
    simulator = _tkCall(
        INIT, os.path.join(myro.globvars.myropath, "worlds", "MyroWorld")
    )
    for port in [60000]:
