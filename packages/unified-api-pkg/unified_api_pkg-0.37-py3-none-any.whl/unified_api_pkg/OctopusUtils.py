    ...
    if receiptType == "REDEEM_COMP" or receiptType == "ISSUE":
       ...
       #printOut.append(mix_2_column_b(bilingualDict["TID"],"Device no.",props["OCTTID"],bilingualDict["OUTLET"],"Shop no.",props["OUTLET"]))
        ...
        if "R_REDEEMED" in transData:
            if props["language"] == "C":
                printOut.append(mix_1_column_c(bilingualDict["REDEEMED"],"$"+__add_minus(transData["CMD"])+'%.1f' % transData["R_REDEEMED"]))
            else:
                printOut.append(mix_1_column("Redeem Reward$","$"+__add_minus(transData["CMD"])+'%.1f' % transData["R_REDEEMED"]))
        
        if "R_EARN" in transData:
            if props["language"] == "C":
                printOut.append(mix_1_column_c(bilingualDict["EARN"],"$"+'%.1f' % transData["R_EARN"]))
            else:
                printOut.append(mix_1_column("Earn Reward$","$"+'%.1f' % transData["R_EARN"]))

        if "R_BALANCE" in transData:
            if props["language"] == "C":
                printOut.append(mix_1_column_c(bilingualDict["R_BAL"],"$"+'%.1f' % transData["R_BALANCE"]))
            else:
                printOut.append(mix_1_column("Reward$ Balance","$"+'%.1f' % transData["R_BALANCE"]))
        ...
        if transData["CMD"] == "TOPUP" and transData["AMT"] != 0:
            ...
            if "R_REDEEMED" in transData:
                if props["language"] == "C":
                    printOut.append(mix_1_column_c(bilingualDict["REDEEMED"],"$"+'%.1f' % transData["R_REDEEMED"]+__add_minus(transData["CMD"])))
                else:
                    printOut.append(mix_1_column("Redeem Reward$",__add_minus(transData["CMD"])+"$"+'%.1f' % transData["R_REDEEMED"]))
                printOut.append("-"*40)
            if "NETAMT" in transData:
