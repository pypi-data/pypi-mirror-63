...
class GCloudStorageClient():
    ...
    def _get_content_type(self, dst_file):
        if dst_file.endswidth('/Manifest') or dst_file.endswidth('.ismc') or dst_file == 'Manifest':
            content_settings = 'text/xml'
        elif dst_file.endswidth('.m3u8'):
            content_settings = 'application/x-mpegURL'
        elif dst_file.endswidth('.mpd'):
            content_settings = 'application/dash+xml'
        else:
    ...
    def upload_file(self, src_file, dst_file, options):
        try:
            ...
            blob.upload_from_filename(filename=src_file, content_type=self._get_content_type(dst_file))
    ...
    def upload_files(self, folder_id, selected_chunks, folder_chunks, do_tar=False, do_compress=False):
        ...
        if do_tar:
            ...
            for chunk in selected_chunks:
                ...
                blob.cache_control = 'public, max-age=2'
                blob.upload_from_filename(filename=folder_chunks + '/' + chunk, content_type=self._get_content_type(chunk))
        ...
        if do_tar:
            ...
            for file in os.listdir(dir):
                ...
                if not os.path.isdir(filePath):
                    ...
                    blob.cache_control = 'public, max-age=2'
                    blob.upload_from_filename(filename=filePath, content_type=self._get_content_type(filePath))
