...
def callbackfuncCloudCtl(telemetry):
    print("{} msg rcvd".format(datetime.now().strftime("%H:%M:%S")))
    ########################################################################################
    # select one cartoon animation by assigning its string name to the variable
    ########################################################################################
    if "m_requestBleDeviceName" in telemetry:
        controller.requestBleDeviceWithName(telemetry['m_requestBleDeviceName'])
...
########################################################################################
controller = m2controller.BleCtrller(platformType = m2Const.etInternetBridgeToBle,
                                     cbFuncCloudCtl = callbackfuncCloudCtl,
...
while True:
    if requestExit:
        ...
        break
    #controller.SendCmdTransBlking(False)
    controller.update(updateNotification=False)
    time.sleep(0.01)
    if controller.isRPi():
        TimeNowS = time.time()
        for ii in range(PWMCnt):
