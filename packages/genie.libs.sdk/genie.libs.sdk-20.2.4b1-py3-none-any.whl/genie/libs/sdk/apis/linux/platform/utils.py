...
def start_routem_process(device, config, routem_executable, config_save_location="/tmp"):
    ...
    try: 
        ...
        id = uuid.uuid4().hex
        config = "{}/config_{}.conf".format(config_save_location, id)

        # Decode base 64 data into temporary file
        device.execute("CONFIG=\"{}\"".format(encoded))
        device_out = device.execute("echo $CONFIG | base64 -d > {}".format(config))

        if 'Permission denied' in device_out:
            raise Exception("Permission denied while trying to create config file. " + \
                "Make sure {} has the correct permissions!".format(config_save_location))
