...
def render(server_name, database_name, filename):
    ...
    with connection() as conn:
        curr=conn.cursor()
        enviroment_project_= "  select p.project_name, e.[environment_name]	from [elt].[project] as p inner join [elt].[project_environment] as e on p.project_id=e.project_id;"
        curr.execute(enviroment_project_)
        c=curr.fetchall()
        enviroment_project=c
