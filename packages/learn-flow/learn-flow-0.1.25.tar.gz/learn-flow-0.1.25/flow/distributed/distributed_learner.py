...
class DistributedValidator(object):
    """
    ...
    def validate_model(self, sender):
        """Model validation."""
        # if valid partition is not None, then run validation step.
        if self.dataset.partitions.get("valid", None) is not None:
...
class DistributedLearner(object):
    ...
    """
    def __init__(self, model, outputs, dataset, resume=False):
        """
