...
class mount_text_function(object):
    ...
    def __init__(self, function, host='0.0.0.0', port=5000, path='/model/predict', threaded=True):
        ...
        if check_text_function_is_valid(self.app.config['function']):
            ...
            if threaded:
                ...
                self.app.run(host=self.host, port=self.port, debug=self.app.config['debug'], threaded=False)
