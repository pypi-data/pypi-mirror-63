...
class ProspinoData:
    ...
    def pdgid(cls, prosid):
        ...
        return {1 : 1000022, 2 : 1000023, 3 : 1000025, 4 : 1000035, 5 : 1000024, 6 : -1000024, 7 : 1000037, 8 : -1000037}.get(prosid)

    @classmethod
    def prosid(cls, pdgid):
        "Translate from Prospino ID code to PDG ID code (for EWinos)"
        return {1000022 : 1, 1000023 : 2, 1000025 : 3, 1000035 : 4, 1000024 : 5, -1000024 : 6, 1000037 : 7, -1000037 : 8}.get(pdgid)
    ...
    def invalid(self, kmin=0.5, kmax=2):
        ...
        if self.xsec_lo() <= 0 or self.xsec_nlo() <= 0:
            return True
        if self.kfac() < kmin or self.kfac() >= 2:
            return True
        return False
...
class KPred:
    ...
    TODO:
    * Make collider/energy-specific
    * Pickle/dill the feature transformers together
    """
    ...
    def __init__(self, energy, process):
        ...
        ## Determine data-file paths for this energy and process
        try:
            from importlib import resources
        except ImportError:
            import importlib_resources as resources
        dirname = "E{}_P{}".format(energy, process)
        pkgname = "salami." + dirname

        ## Load scalers and model
        try:
            def getpath(fname):
                "Find package files the dirty way"
                import os.path
                p = os.path.join(os.path.dirname(__file__), dirname, fname)
            ...
            # with resources.path(pkgname, "kfactor_weights.hdf5") as modelpath:
            #     self.model = load_model(modelpath)
            # with resources.path(pkgname, "feature_scaler.pkl") as fscalepath:
            #     self.feature_scaler = load_pickle(fscalepath)
            # with resources.path(pkgname, "xsec_scaler.pkl") as xscalepath:
            #     self.loxsec_scaler = load_pickle(xscalepath)

        # TODO: use specific exception
        except Exception as e:
            #print("Error: " + str(e))
            raise Exception("Unsupported process " + self.process)


    def predict_kfac(self, slhadoc, xsec_lo, kmin=None, kmax=None):
        """\
    ...
    def predict_kfac_xsec(self, slhadoc, xsec_lo, kmin=None, kmax=None):
    ...
    def predict_xsec(self, slhadoc, xsec_lo, kmin=None, kmax=None):
...
class KPreds:
    ...
    def kpred(self, process):
        return self.kpreds.setdefault(str(process), KPred(str(self.energy), str(process)))

    def predict_kfac(self, slhadoc, process, xsec_lo, kmin=None, kmax=None):
        "Predict a LO->NLO K-factor, given an SLHA dataset and a LO cross-section"
        return self.kpred(process).predict_kfac(slhadoc, xsec_lo, kmin, kmax)

    def predict_kfac_xsec(self, slhadoc, process, xsec_lo, kmin=None, kmax=None):
        "Predict a LO->NLO K-factor and the NLO cross-section, given an SLHA dataset and a LO cross-section"
        return self.kpred(process).predict_kfac_xsec(slhadoc, xsec_lo, kmin, kmax)

    def predict_xsec(self, slhadoc, process, xsec_lo, kmin=None, kmax=None):
        "Predict the NLO cross-section, given an SLHA dataset and a LO cross-section"
