...
class TagVersion(Version):
    ...
    def __init__(self, tag_reader=None, ci_reader=environ, ci_spec=None):
        ...
        self.__tags = self.__read_tags(tag_reader)
    ...
    @staticmethod
    def __read_tags(tag_reader):
        reader = tag_reader if tag_reader else Git().get_tags_from
        return reader(pattern=r'(\d+\.){2}\d+', is_must=False)
    ...
    def __advance_version(self, version_parts):
        ...
        return self.__advance_major_minor_from_tags(version_parts, ci_build)

    def __advance_build_relative_to_base_build(self, version_parts, ci_build):
        version_parts[2] += ci_build - self.__base_build
        return version_parts

    def __advance_major_minor_from_tags(self, version_parts, ci_build):
        bumps = [tag[0].split('_')[1] for tag in self.__tags if tag and tag[0].startswith('bump_')]
        if 'major' in bumps:
