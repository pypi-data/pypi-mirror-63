...
def add_job_request(job_name, script_and_module_ids, input_files_ids, wait=False):
    machine_size = os.environ.get('DISCO_MACHINE_SIZE')
    cluster_id = os.environ.get('DISCO_CLUSTER_ID')
    job = Job.create(script_file_id=script_and_module_ids[0],
                     input_file_ids=input_files_ids,
                     constants_file_ids=[script_and_module_ids[1]],
                     job_name=job_name,
