...
class UserManager(BaseManager):
    ...
    def create_user(self, params, domain):
        ...
        if domain.plugin_info:
            found_users, count = self.find_user({'user_id': params['user_id']}, domain)
            if count == 1:
    ...
    def find_user(self, search, domain: Domain):
        """ Find User via Plugin
        Args:
            params
                - keyword
        Returns:
            users(dict), total_count
            :param params:
            :param domain:
        """
        ...
        endpoint = self._get_plugin_endpoint(domain)

        ret = self._call_find(keyword, user_id, domain, endpoint)
