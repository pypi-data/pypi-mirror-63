...
class Manager:
    ...
    def __register_client(self, connection: socket, address: tuple):
        ...
        # marks if this client replaced an old client
        replaced = False
        ...
        for index, check_client in enumerate(self.__client_pool):
            ...
            if check_client.uuid == client.uuid:
                ...
                try:
                    ...
                    # remove the client
                    self.__remove_client(check_client, False)
        ...
        # in case of a exception when executing handler
            ...
            self.logger.error(
                "(Message Handler) Exception caught when running the on_connect() handler. For the " + client.type +
                " client type. Exception: '" + str(error) + "'")

        # add the client to the pool if it did not replace an old client
        if not replaced:
    ...
    def __remove_client(self, client, delete=True):
        # catch and exceptions within the general on_connect function
        try:
            # run the on_connect function for the client
        ...
        if delete:
            ...
            del self.__client_pool[self.__client_pool.index(client)]
