...
class KafkaClient:
    ...
    def set_producer_avro(self, value_schema, **configs):
        self.producer = AvroProducer({**configs}, default_value_schema=value_schema)
            ...
            while True:
                if self.avro_consumer:
                    ...
                    if msg.error():
                        ...
                        continue
                    logger.log_info(f'Received message: {msg.value()}')
                    yield msg.value()
                else:
                    ...
                    if msg.error():
                        ...
                        continue
                    logger.log_info(f'Received message: {msg.value().decode("utf-8")}')
                    yield msg.value().decode('utf-8')
        else:
            raise ConsumerNotSetError('Set consumer before!')


class KafkaClient_old:
