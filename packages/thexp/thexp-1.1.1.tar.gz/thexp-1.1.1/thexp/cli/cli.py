...
Usage:
 ...
 thexp config --global -u <name> <value> 
...
from thexp.frame.experiment import Global
...
def update(mode,name,val):
    glob = Global()
    glob.update(mode,name,val)

def unset(mode,name):
    glob = Global()
    glob.unset(mode,name)

def list(globa=False,local=False):
    glob = Global()
    glob.list_config(globa,local)
...
if arguments["init"]:
    ...
    if arguments["--global"]:
        if arguments["-l"]:
            ...
            if arguments["-u"]:
                glob.unset("global",arguments["<name>"])
            else:
                glob.update("global",arguments["<name>"],arguments["<value>"])
    elif arguments["--local"]:
        if arguments["-l"]:
            list(local=True)
        elif arguments["<name>"]:
            if arguments["-u"]:
                glob.unset("local", arguments["<name>"])
            else:
                glob.update("local", arguments["<name>"], arguments["<value>"])
    elif arguments["-l"]:
        list(True,True)
else:
    print(arguments)
