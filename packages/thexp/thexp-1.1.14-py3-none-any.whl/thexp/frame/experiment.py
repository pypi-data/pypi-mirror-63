    ...
    def start_exp(self):
        ...
        for member in self.snap_member:
            if inspect.ismodule(object):
                if not getattr(object, '__file__', None):
                    warnings.warn('{!r} is a built-in module'.format(object))
            if inspect.isclass(object):
                if not hasattr(object, '__module__'):
                    warnings.warn('{!r} is a built-in class'.format(object))
                elif getattr(object, '__file__', None):
                    warnings.warn('{!r} is a built-in class'.format(object))
