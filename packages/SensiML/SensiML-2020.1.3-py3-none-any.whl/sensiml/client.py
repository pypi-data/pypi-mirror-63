...
class SensiML(object):
    ...
    def list_segmenters(self):
        ...
        segmenters = SegmenterSet(
            self._connection, self._project).get_set()
    ...
    def list_knowledgepacks(self, unnamed=False):
        ...
        if unnamed:
            return knowledgepack_list[
                ["Name", "Created", "Project", "Pipeline",
                    "kp_uuid", "kp_description"]
            ]
    ...
    def upload_project(self, name, dclprojpath, asynchronous=True):
        ...
        for capture in capture_files:
            ...
            try:
                ...
                if os.path.exists(
                    os.path.join(
                        basedir, "metadata", os.path.basename(capture)[
                            :-3] + "dcl"
                    )
                ):
                    with open(
