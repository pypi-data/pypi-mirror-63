...
from spcc.forms.field import TreeSelect
...
class newModelForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.change = 0
        if self.model == None:
            raise Exception('ValueError', self.model)
    ...
    def get_local_field_objects(self, model=None):
        local_field_objects = []
        if model:
            local_field_objects = [y[0] for y in get_fields_with_model(model=model) if y[0].name in self.fields.keys()]
    ...
    def check_field(self, field_name=None):
        if field_name and self.instance == None:
            ...
            if self.cleaned_data[field_name] != '' and (
    ...
    def get_save_data(self, model=None):
        # print model
        field_objects = get_model_fields(model=model)
    ...
    def save(self):
        ...
        for field in self.model._meta.fields:
            if type(field) == models.DateTimeField and field.name in self.base_fields.keys() and save_args[
                field.name] is not None and save_args[field.name] != '':
                try:
