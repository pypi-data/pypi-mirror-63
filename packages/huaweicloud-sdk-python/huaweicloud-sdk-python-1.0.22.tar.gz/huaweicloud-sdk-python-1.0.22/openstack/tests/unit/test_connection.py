...
class TestConnection(base.TestCase):
    def test_microversion(self):
        ...
        conn.set_microversion("compute", 2.26)
        self.assertEqual(conn.profile.get_filter("compute").microversion, "2.26")
        conn.unset_microversion("compute")
        self.assertEqual(conn.profile.get_filter("compute").microversion, None)
