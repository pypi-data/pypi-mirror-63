...
class Resource(resource.Resource):
    ...
    # overwrite resource2._prepare_request as rds requires header
    def get(self, session, requires_id=True):
        ...
        response = session.get(request.uri, endpoint_filter=self.service,
                               ...
                               headers=request.headers, raise_exc=False)
    ...
    def list(self, session, paginated=False, **params):
        ...
        while more_data:
            endpoint_override = self.service.get_endpoint_override() if self.service.get_endpoint_override() \
                else self.service.get_endpoint_override()
            ...
            if resp.status_code >= 400:
                yield ErrorResponse.make_response(resp)
                return
...
class ErrorResponse(resource.Resource):
    ...
    def make_response(response):
        ...
        resp._translate_response(response)
        return resp
