...
if __name__ == '__main__':
    ...
    '''
    ff = conn.bssintl.query_monthly_expenditure_summary(userDomainId, **data)
    print(ff)

    data = {
        "consume_month": "2019-06"
    }
    '''
    ...
    This API can be invoked using the customer AK/SK or token only.
    '''
    ff = conn.bssintl.query_resource_usage_details(userDomainId, **data)
    print(ff)
    ...
    This API can be invoked using the customer AK/SK or token only.
    '''
    ff = conn.bssintl.query_resource_usage_record(userDomainId, **data)
    print(ff)
