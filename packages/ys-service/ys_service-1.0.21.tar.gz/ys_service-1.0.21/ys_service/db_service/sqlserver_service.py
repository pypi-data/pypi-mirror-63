...
class SqlServerService(BaseDB):
    ...
    def connect(self, host=None, username=None, password=None, db=None):
        ...
        if not self.is_in_map(hash_value):
            pool = PooledDB(
                creator=pymssql, mincached=10, maxcached=10, maxshared=3, maxconnections=10, blocking=True,
                host=_host, user=_username, password=_password, database=_db, charset="utf8"
            )
            self.add_to_map(hash_value, pool)
    ...
    def get_cursor(self, hash_value):
        ...
        :return:
        """
        return self.connect_map[hash_value].connection().cursor()
