...
class HeaderToLabelMiddleware(object):
    ...
    def process_request(self, request):
        ...
        log_entry_debug = dict(
            ...
            method=request.method,
            body=Utils.obfuscate_body(Utils.adjust_string_length(self._initial_http_body, getattr(settings, MAX_REQUEST_RESPONSE_SIZE))),
            url=request.build_absolute_uri()
        )
    ...
    def process_response(self, request, response):
        ...
        log_entry = dict(
            ...
            request_end_time_ms=Utils.get_time_in_milliseconds(),
            method=request.method,
            body=Utils.obfuscate_body(Utils.adjust_string_length(self._initial_http_body,
                                            getattr(settings, MAX_REQUEST_RESPONSE_SIZE)) if response.status_code < 400 else self._initial_http_body),
            status_code=response.status_code,
            response=Utils.obfuscate_response(Utils.adjust_string_length(response.content,
