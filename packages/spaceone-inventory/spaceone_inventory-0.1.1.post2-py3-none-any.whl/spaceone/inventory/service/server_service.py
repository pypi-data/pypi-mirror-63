...
class ServerService(BaseService):
    ...
    def create(self, params):
        ...
        zone_vo, region_vo = self._get_zone_and_region(zone_id, domain_id)
        params['zone'] = zone_vo
        params['region'] = region_vo

        if pool_id:
            pool_vo = self._get_pool(pool_id, domain_id, zone_id)
            params['pool'] = pool_vo
    ...
    def update(self, params):
        ...
        pool_id = params.get('pool_id')
        project_id = params.get('project_id')
        release_pool = params.get('release_pool', False)
        ...
        if release_pool:
            ...
            if pool_id:
                if zone_id is None:
                    pool_vo = self._get_pool(pool_id, domain_id)
                    params['pool'] = pool_vo
    ...
    def get(self, params):
        ...
        """
        return self.server_mgr.get_server(params['server_id'], params['domain_id'])

    @transaction
    @check_required(['domain_id'])
    @append_query_filter(['server_id', 'name', 'state', 'primary_ip_address',
                          'ip_addresses', 'server_type', 'os_type', 'asset_id',
                          'region_id', 'zone_id', 'pool_id', 'project_id',
    ...
    def _get_zone_and_region(self, zone_id, domain_id):
        zone_mgr: ZoneManager = self.locator.get_manager('ZoneManager')
        zone_vo = zone_mgr.get_zone(zone_id, domain_id)

        return zone_vo, zone_vo.region

    def _get_pool(self, pool_id, domain_id, zone_id=None):
        pool_mgr: PoolManager = self.locator.get_manager('PoolManager')
        pool_vo = pool_mgr.get_pool(pool_id, domain_id)

        if zone_id:
            if zone_id != pool_vo.zone.zone_id:
                raise ERROR_POOL_NOT_EXIST_IN_ZONE(zone_id=zone_id, pool_id=pool_id)
