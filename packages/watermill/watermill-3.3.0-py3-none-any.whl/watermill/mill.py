...
class WaterMill:
    ...
    def run(self):
        ...
        while True:
            ...
            eos = False
            try:
                root_element_data = next(self._broker.get_elements(join_tree.root_node.user_type))
            ...
            if root_element_data:
                ...
                if window_func:
                    ...
                    assert new_window_key is not None
                    if root_elements and window_key is not None and window_key != new_window_key:
                        process_result = self._process_func(root_elements, *join_data_mappings, **injected_kwargs)
                        process_results, eos = _handle_result(process_result, process_results, eos)
                        root_elements.clear()
                        join_data_mappings.clear()
