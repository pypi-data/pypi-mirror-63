...
class Test_LodeStarDataResample(unittest.TestCase):
    ...
    def test_resampled_time(self):
        ...
        self.assertListEqual(list(self.resampled_lodestar.data.index.values[:3]), expected_times)
    ...
    def test_resampled_rpm(self):
        ...
        for i in [3, 6]:
            rpm_int.append(round(interp(self.resampled_lodestar.data.index.values[i], lodestar.data.index.values, lodestar.rpm), 3))
