...
from .parameters import one_twelfth_carbon, electron_mass
...
# May need additional work for linear molecules
def project_out_transl_and_rot(num_vib_modes, hess, coords):
...
def get_vib_harm_freqs_and_eigvecs(coords, charges, masses, hess, outproj, print_level):
    ...
    #amu_au_sqrt = 1.0/((one_twelfth_carbon/electron_mass)**0.5)
    amu_au_sqrt = (electron_mass/one_twelfth_carbon)**0.5
    num_vib_modes = 3 * len(coords) - 6 + mol_is_linear(coords)
    ...
    if (outproj):
        vib_hess = project_out_transl_and_rot(num_vib_modes, hess, coords)
    else:
    ...
    vib_w = get_vibrational_w(mol_is_linear(coords), 3*len(coords), w, outproj, print_level)

    return vib_w, T.real, 3*len(coords)


def get_vibrational_w(is_linear, num_coordinates, w, outproj, print_level):
    ...
    if (len(w) == vib_degrees_of_freedom):
        if (print_level > 0):
            print('Only vibrational frequencies were calcuated')
