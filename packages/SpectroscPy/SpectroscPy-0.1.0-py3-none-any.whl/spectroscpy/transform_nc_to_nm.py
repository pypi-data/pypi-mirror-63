...
def transform_cartesian_to_normal_one_rank(w, rank_geo, c, offset, offset_b, ind_w, num_coordinates, \
                                           num_vib_coordinates, num_nongeo_components, A, B, T, act):
	
	if (c > 0):
		for i in range(num_coordinates):

			if ((c <= w[0])):
				if ((i + 1) > num_vib_coordinates):
					act = 0
				if (c == min(w)):
					ind_w = i

			transform_cartesian_to_normal_one_rank(w, rank_geo, c-1, offset, offset_b, ind_w, \
                                                   num_coordinates, num_vib_coordinates, \
...
def transform_cartesian_to_normal(A, dims, rank_geo, num_coordinates, num_vib_coordinates, T):
