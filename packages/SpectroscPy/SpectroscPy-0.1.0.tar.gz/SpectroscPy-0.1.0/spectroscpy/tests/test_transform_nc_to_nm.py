...
def test_transform_cartesian_to_normal_one_rank():
    ...
    assert len(ref_B) == len(B)

    for i in range(len(B)):
        assert ref_B[i] == pytest.approx(B[i], 1.0e-8)
...
def test_transform_cartesian_to_normal():
    ...
    transformed_A = transform_cartesian_to_normal(A, dims, rank_geo, num_coordinates, \
                                                   num_vib_coordinates, T)

    assert len(ref_transformed_A) == len(transformed_A)

    for i in range(len(ref_transformed_A)):
        assert len(ref_transformed_A[0]) == len(transformed_A[i])

        for j in range(len(transformed_A[0])):
            assert len(ref_transformed_A[0][0]) == len(transformed_A[i][j])

            for k in range(len(transformed_A[0][0])):
                assert ref_transformed_A[i][j][k] == pytest.approx(transformed_A[i][j][k], 1.0e-8)
