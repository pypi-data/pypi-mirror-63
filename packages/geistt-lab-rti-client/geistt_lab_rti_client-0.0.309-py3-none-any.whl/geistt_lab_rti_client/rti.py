...
class RTI(Emitter):
    ...
    def __init__(self, application_id="python", instance_id=None, federation_id=None, secret=None, incognito=False, suppress_own_messages=False, connect=True):
        super().__init__()
        ...
        def on_federations(message: str):
            if message == "?": self.publish_text(constants.federations_channel, self.federation_id)
    ...
    def subscribe_text(self, channel_name: str, handler, data_type="text"):
        ...
        if (socket_channel_name not in self.subscriptions):
            if self.connected: self.socket.subscribe(socket_channel_name)
    ...
    def _publish_client(self):
        ...
        if self._host: message.client.host = self._host
        if self._station: message.client.station = self._station
