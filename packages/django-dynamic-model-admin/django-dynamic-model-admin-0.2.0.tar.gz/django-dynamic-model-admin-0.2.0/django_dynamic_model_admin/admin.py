...
from .models import get_field_widget_choices
from .models import get_field_widget
...
class ModelFieldInline(admin.TabularInline):
    ...
    def get_formset(self, request, obj=None, **kwargs):
        ...
        formset.form.base_fields["real_field_name"].widget = forms.Select(choices=self.data_model.get_data_field_choices())
        formset.form.base_fields["field_widget"].widget = forms.Select(choices=get_field_widget_choices())
        return formset
...
class ModelDefinitionAbstractAdmin(DraggableMPTTAdmin, admin.ModelAdmin):
    ...
    def get_form(self, *args, **kwargs):
        self.field_inline_instance = ModelFieldInline(self.field_model, self.data_model, self.model, self.admin_site, "definition", 0, classes=["model_field_inline"])
        self.fieldset_inline_instance = ModelDefinitionRelatedInline(self.fieldset_model, self.model, self.admin_site, "definition", 0, classes=["model_fieldset_inline"])
        self.inline_inline_instance = ModelDefinitionRelatedInline(self.inline_model, self.model, self.admin_site, "definition", 0, classes=["model_inline_inline"])
        result = super().get_form(*args, **kwargs)
        return result
    ...
    def get_form(self, request, obj=None, change=False, **kwargs):
        ...
        if obj:
            ...
            for field_name in field_names:
                ...
                if field_name in fields_map:
                    new_field, new_widget = get_field_widget(fields_map[field_name].field_widget, obj)
                    if new_field:
