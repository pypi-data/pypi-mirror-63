...
class AllDictData:
	...
	def __setFormatDictPath(self,keyPath):

		keyPath=keyPath.replace("[",",[")
		keyPath=keyPath.replace("[","")
		keyPath=keyPath.replace("]","")
		pathList=keyPath.split(",")
		robotDictPath=pathList
	...
	def getDictPaths(self,dataDict):
		...
		for key, value in self.__recursive_dict(dataDict):
			if type(value) in [dict]:
				...
				dictPath=self.__setFormatDictPath(keyPath)
				allKeys.append(dictPath)
				endPath=True 
	...
	def getDictValues(self,dataDict):
		keyList=self.getDictPaths(dataDict)
		allValues=[]
		for item in keyList:
			elem=dataDict
			for k in item:
