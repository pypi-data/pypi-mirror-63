...
import tlclient.trader.message_pb2 as message_pb2
...
class Client(Fist):
    ...
    def insert_order(self, tg_name, exchange, ticker, price, volume, order_type, direction, source='',
        ...
        req = message_pb2.ReqOrderInsert()
        req.basic.account_id = tg_name
        req.basic.exchange = exchange
        req.basic.symbol = ticker
        req.basic.sub_account = source
        req.basic.entrust_price = price
        req.basic.entrust_vol = volume
        req.basic.security_type = asset_type
        req.basic.order_type = order_type
        req.basic.bs_flag = direction
        req.basic.offset_flag = offset_flag
        req.basic.margin_mode = margin_mode
        req.basic.lever_rate = lever_rate
        req.basic.sender = self.fist_name
        req.parent_id = -1
        ret = self.req(self.trade_router, req, message_pb2.MsgType.MSG_TYPE_REQ_ORDER_INSERT, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
    ...
    def cancel_order(self, order_id, tg_name):
        req = message_pb2.ReqOrderCancel()
        req.order_id = order_id
        req.account_id = tg_name

        ret = self.req(self.trade_router, req, message_pb2.MsgType.MSG_TYPE_REQ_ORDER_CANCEL, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
    ...
    def req_position(self, tg_name):
        req = message_pb2.ReqPosition()
        req.basic.account_id = tg_name
        req.basic.sender = self.fist_name
        ret = self.req(self.trade_router, req, message_pb2.MsgType.MSG_TYPE_REQ_POSITION, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
    ...
    def req_account(self, tg_name):
        req = message_pb2.ReqAccount()
        req.basic.account_id = tg_name
        req.basic.sender = self.fist_name
        ret = self.req(self.trade_router, req, message_pb2.MsgType.MSG_TYPE_REQ_ACCOUNT, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
    ...
    def req_active_orders(self, tg_name):
        req = message_pb2.ReqActiveOrders()
        req.basic.account_id = tg_name
        req.basic.sender = self.fist_name
        ret = self.req(self.trade_router, req, message_pb2.MsgType.MSG_TYPE_REQ_ACTIVE_ORDERS, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
    ...
    def req_cancel_active_orders(self, tg_name, req_active_orders_id=-1):
        req = message_pb2.ReqCancelActiveOrders()
        req.active_orders_id = req_active_orders_id
        req.basic.account_id = tg_name
        req.basic.sender = self.fist_name
        ret = self.req(self.trade_router, req, message_pb2.MsgType.MSG_TYPE_REQ_CANCEL_ACTIVE_ORDERS, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
    ...
    def req_history_trades(self, tg_name):
        req = message_pb2.ReqHistoryTrades()
        req.basic.account_id = tg_name
        req.basic.sender = self.fist_name
        ret = self.req(self.trade_router, req, message_pb2.MsgType.MSG_TYPE_REQ_HISTORY_TRADES, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
    ...
    def on_pub_frame(self, f):
        ...
        if msg_callback is not None:
            ...
            if msg_type == message_pb2.MsgType.MSG_TYPE_RSP_ORDER_INSERT:
                rsp_ord_obj = f.get_obj(message_pb2.RspOrderInsert)
                self.on_rsp_order_insert(rsp_ord_obj, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RSP_ORDER_CANCEL:
                rsp_cancel_obj = f.get_obj(message_pb2.RspOrderCancel)
                self.on_rsp_order_cancel(rsp_cancel_obj, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RTN_ORDER:
                ord_object = f.get_obj(message_pb2.RtnOrder)
                self.on_rtn_order(ord_object, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RTN_TRADE:
                trd_obj = f.get_obj(message_pb2.RtnTrade)
                self.on_rtn_trade(trd_obj, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RSP_POSITION:
                pos_obj = f.get_obj(message_pb2.GatewayPosition)
                self.on_rsp_position(pos_obj, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RSP_ACCOUNT:
                acc_obj = f.get_obj(message_pb2.RspAccount)
                self.on_rsp_account(acc_obj, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RSP_ACTIVE_ORDERS:
                ods_obj = f.get_obj(message_pb2.GatewayOrders)
                self.on_rsp_active_orders(ods_obj, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RSP_CANCEL_ACTIVE_ORDERS:
                rsp_obj = f.get_obj(message_pb2.RspCancelActiveOrders)
                self.on_rsp_cancel_active_orders(rsp_obj, frame_nano)

            elif msg_type == message_pb2.MsgType.MSG_TYPE_RSP_HISTORY_TRADES:
                rsp_trades = f.get_obj(message_pb2.RspHistoryTrades(rsp_trades))
