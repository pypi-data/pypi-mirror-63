...
from tlclient.trader.message_pb2 import Exchange, BsFlag, OrderType, OffsetFlag
...
exchange_id_to_exchange = {ExchangeID.SSE: Exchange.EXCHANGE_SSE, ExchangeID.SZE: Exchange.EXCHANGE_SZE,
                           ExchangeID.HK: Exchange.EXCHANGE_HK, ExchangeID.CFFEX: Exchange.EXCHANGE_CFFEX,
                           ExchangeID.DCE: Exchange.EXCHANGE_DCE, ExchangeID.SHFE: Exchange.EXCHANGE_SHFE,
                           ExchangeID.CZCE: Exchange.EXCHANGE_CZCE }
...
class ConsoleClient:
    ...
    def process_trade_buy_or_sell_command(self, parts):
        ...
        if parts[part_index] == 'buy':
            direction = BsFlag.BS_FLAG_BUY
        else:
            direction = BsFlag.BS_FLAG_SELL
        ...
        if not silently:
            with print_lock:
                print(insert_order_confirm_template.format(trade_gateway, BsFlag.Name(direction), ticker,
                            Exchange.Name(exchange_id_to_exchange[exchange]), volumn, price, OrderType.Name(order_type), OffsetFlag.Name(offset_flag)))
                confirm = self.get_user_input(confirm_prompt)
                if confirm == 'Y' or confirm == 'y':
                    order_id = self.sc.insert_order(tg_name = trade_gateway, exchange = exchange_id_to_exchange[exchange], ticker = ticker, price = price,
