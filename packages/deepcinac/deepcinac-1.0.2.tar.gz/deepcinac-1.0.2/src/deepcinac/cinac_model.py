...
class CinacModel:
    ...
    def add_input_data(self, cinac_file_names, session_ids=None, verbose=0, cinac_files_only_for_training=None,
        ...
        for index_file, cinac_file_name in enumerate(cinac_file_names):
            ...
            #     if not os.path.exists(dir_path):
            #         os.mkdir(dir_path)
            #     # creating individual cinac files, with segment of size self.window_len
            #     # return a dict with key training or validation that contains a list
            #     # CinacFileReader instances
            #     cinac_file_readers_dict = cinac_file_reader. \
            #         create_cinac_files_for_training_and_validation(dir_path=dir_path,
            #                                                        window_len=self.window_len)
            #
            #     for set_name, cinac_file_readers in cinac_file_readers_dict.items():
            #         for cinac_file_reader in cinac_file_readers:
            #             segments_list = cinac_file_reader.get_all_segments()
            #             # a segment represent a cell and given frames with their corresponding ground truth
            #             # segment is a tuple of 3 int (cell, first_frame, last_frame)
            #             for segment in segments_list:
            #                 self.cinac_file_readers_and_segments[set_name].append((cinac_file_reader, segment))
            #                 if session_ids is not None:
            #                     self.session_ids[set_name].append(session_ids[index_file])
    ...
    def _split_and_stratify_cell_type_mode_data(self, seed_value=None, verbose=0):
        """
    ...
    def _split_and_stratify_data_for_window_len_h5(self, seed_value=None, verbose=0):
        """
        ...
        """
        if self.cell_type_classifier_mode:
            self._split_and_stratify_cell_type_mode_data(seed_value=seed_value, verbose=verbose)
            return
    ...
    def _split_and_stratify_data_to_delete(self, seed_value=None, verbose=0):
        """
                Split (between validation and training) and stratify the data
                Args:
                    seed_value:
        ...
        if using_splitted_tiff_cinac_movie:
            ...
            for cinac_file_index, data in enumerate(self.cinac_file_readers_and_segments):
                ...
                if not is_created:
                    ...
                    all_already_loaded = False
            if not all_already_loaded:
                raise Exception("All calcium movie in cinac files were not available as single frame npy. "
                                "Those have been created, you to relaunch the code now. ")
            else:
                print("All calcium movie in cinac files were  available as single frame npy")
