...
def convert_wiki_to_lines_inner_generator(wiki, skip_cdata, encoding, inner_element, element,
                                          parse_special_pages, skip_templated_lines, maxlines, max_articles, job):
...
def convert_wiki_to_lines_inner(result_queue, path, *args):
...
def convert_wiki_to_lines(wiki,
                          ...
                          maxlines=9999999999999999,
                          parallel=True):
    if parallel and isinstance(fin, io.BufferedReader):
        n_workers = cpu_count()
        result_queue = Queue()
        fname = fin.name
        ...
        workers = [Process(target=convert_wiki_to_lines_inner,
                           args=(result_queue, fname, skip_cdata, encoding, inner_element, element,
        ...
        # do sequential processing
        for article, title in convert_wiki_to_lines_inner_generator(wiki, skip_cdata, encoding, inner_element, element,
