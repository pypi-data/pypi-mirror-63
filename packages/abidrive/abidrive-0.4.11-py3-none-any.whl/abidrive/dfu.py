...
class FirmwareFromGithub(Firmware):
    ...
    def get_as_hex(self):
        ...
        if self.hex is None:
            ...
            response = requests.get('https://api.github.com/repos/madcowswe/AbiDrive/releases/assets/' + str(self.github_asset_id),
...
def get_all_github_firmwares():
    response = requests.get('https://api.github.com/repos/madcowswe/AbiDrive/releases')
    if response.status_code != 200:
...
def update_device(device, firmware, logger, cancellation_token):
    ...
    device = abidrive.find_any("usb", serial_number,
                    cancellation_token, cancellation_token, timeout=30)

    if do_backup_config:
        abidrive.configuration.restore_config(device, None, logger)
        os.remove(abidrive.configuration.get_temp_config_filename(device))
...
def launch_dfu(args, logger, cancellation_token):
    ...
    devices[1] = abidrive.find_any("usb", serial_number,
        find_abidrive_cancellation_token, cancellation_token)
    find_abidrive_cancellation_token.set()
