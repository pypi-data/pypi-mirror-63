...
class AbiDriveTestContext():
    ...
    def rediscover(self):
        """
        Reconnects to the AbiDrive
        """
        self.handle = abidrive.find_any(
...
class AxisTestContext():
    def __init__(self, name: str, yaml: dict, odrv_ctx: AbiDriveTestContext):
        self.handle = None
        self.yaml = yaml
        self.name = name
...
def get_max_rpm(axis_ctx: AxisTestContext):
    ...
    # whereas the AbiDrive modulates the space vector around a circular trajectory.
    ...
    # The AbiDrive only goes to 80% modulation depth in order to save some time for the ADC measurements.
...
class AbiDriveTest(ABC):
    """
    Tests inheriting from this class get full ownership of the AbiDrive
    ...
    def check_preconditions(self, odrv_ctx: AbiDriveTestContext, logger):
        pass
    @abc.abstractmethod
    def run_test(self, odrv_ctx: AbiDriveTestContext, logger):
        pass

class AxisTest(ABC):
    """
    Tests inheriting from this class get ownership of one axis of
    an AbiDrive. If the axis is mechanically coupled to another
...
class TestDiscoverAndGotoIdle(AbiDriveTest):
    def run_test(self, odrv_ctx: AbiDriveTestContext, logger):
        odrv_ctx.rediscover()
...
class TestFlashAndErase(AbiDriveTest):
    def __init__(self):
        AbiDriveTest.__init__(self, exclusive=True)
    def run_test(self, odrv_ctx: AbiDriveTestContext, logger):
        ...
        logger.debug("waiting for AbiDrive...")
        odrv_ctx.rediscover()
        # ensure the correct abidrive is returned
...
class TestSetup(AbiDriveTest):
    """
    Preconditions: AbiDrive is unconfigured and just rebooted
    """
    def run_test(self, odrv_ctx: AbiDriveTestContext, logger):
        odrv_ctx.rediscover()

        # initial protocol tests and setup
        logger.debug("setting up AbiDrive...")
...
class TestStoreAndReboot(AbiDriveTest):
    """
...
class TestHighVelocityInViscousFluid(DualAxisTest):
    ...
    def run_test(self, axis0_ctx: AxisTestContext, axis1_ctx: AxisTestContext, logger):
        ...
        if driver_ctx.name == 'top-abidrive.black':
            # abidrive.utils.start_liveplotter(lambda: [driver_ctx.odrv_ctx.handle.vbus_voltage])
            abidrive.utils.start_liveplotter(lambda: [driver_ctx.handle.motor.current_control.Iq_measured, 
...
class TestSelfLoadedPosVelDistribution(DualAxisTest):
    """
    Uses an AbiDrive mechanically connected to itself to test a distribution of
...
class TestVelCtrlVsPosCtrl(DualAxisTest):
    """
    Uses one AbiDrive as a load operating in velocity control mode.
    The other AbiDrive tries to "fight" against the load in position mode.
    """
    def run_test(self, axis0_ctx: AxisTestContext, axis1_ctx: AxisTestContext, logger):
        load_ctx = axis0_ctx
        driver_ctx = axis1_ctx
        ...
        abidrive.utils.print_drv_regs("load motor ({})".format(load_ctx.name), load_ctx.handle.motor)
        abidrive.utils.print_drv_regs("driver motor ({})".format(driver_ctx.name), driver_ctx.handle.motor)
...
class TestAsciiProtocol(AbiDriveTest):
    def run_test(self, odrv_ctx: AbiDriveTestContext, logger):
        import abidrive.serial_transport
        port = abidrive.serial_transport.SerialStreamTransport(odrv_ctx.yaml['uart'], 115200)
