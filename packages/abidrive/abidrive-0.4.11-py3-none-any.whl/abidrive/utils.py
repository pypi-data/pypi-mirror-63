...
from abidrive.enums import errors
...
def dump_errors(abidrv, clear=False):
    axes = [(name, axis) for name, axis in abidrv._remote_attributes.items() if 'axis' in name]
    axes.sort()
...
def show_oscilloscope(abidrv):
    size = 18000
    values = []
    for i in range(size):
        values.append(abidrv.get_oscilloscope_val(i))
