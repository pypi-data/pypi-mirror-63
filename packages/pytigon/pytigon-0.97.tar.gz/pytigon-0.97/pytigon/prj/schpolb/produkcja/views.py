...
from pytigon_lib.schdjangoext.spreadsheet_render import render_to_response_odf, render_to_response_ooxml
...
(
    ...
    where nag.Data >='%s' and nag.Data< '%s' and nag.rd = 'PD' and nag.status > 0
...
where Data >='%s' and Data<= '%s' and rp.rd = 'PD' and pd_rp_h5 + pd_rp_M5 <> 0 and rp.status>0
...
select8 = """
select [Zakład] 'Zakład', 
       sum([Godzinny netto teoretyczne]) 'Godzinny netto teoretyczne', 
       sum([Godziny przepracowane]) 'Godziny przepracowane', 
       sum([Godziny obce produkcyjne]) 'Godziny obce produkcyjne', 
       sum([Suma z IloscPrzelPW]) 'Suma z IloscPrzelPW', 
       sum([Suma z IloscPrzelPW1Gat]) 'Suma z IloscPrzelPW1Gat'
from 
(
    select mag 'Zakład', 0 'Godzinny netto teoretyczne', 0 'Godziny przepracowane', 0 'Godziny obce produkcyjne', sum(IloscMGPrzel) 'Suma z IloscPrzelPW', 0 'Suma z IloscPrzelPW1Gat'
    from
    (
    select nag.mag, REPLACE(left(OpiKar4,1), 'P', '1') Kategoria, sum(PD_IloscMGPrzel) as IloscMGPrzel from 
    mg_Nag nag(NOLOCK)
    left join mg_Lin lin(NOLOCK) on lin.NagId = nag.NagId
    left join mg_kar kar(NOLOCK) on kar.symkar = lin.symkar 
    where nag.Data >='%s' and nag.Data< '%s' and nag.RD = 'PW' and OpiKar4 like '[1,2,3,P]%%' and nag.status > 0
    group by nag.mag, REPLACE(left(OpiKar4, 1),'P','1')
    ) tab
    group by mag
    union all
    select mag, 0, 0, 0, 0, sum(IloscMGPrzel) IloscMGPrzel
    from
    (
    select nag.mag, REPLACE(left(OpiKar4,1), 'P', '1') Kategoria, sum(PD_IloscMGPrzel) as IloscMGPrzel from 
    mg_Nag nag(NOLOCK)
    ...
    select substring(mag,2,100), sum(CzasTeoretyczny)/60, 0, 0, 0, 0 from (
    select nag.mag,
    sum(nag.CzasNormatywny_HH) *60 + sum(nag.CzasNormatywny_MM) as CzasTeoretyczny
...
group by tab2.[Zakład]
order by tab2.[Zakład]
"""
...
class Prac():
    def __init__(self, row=None, oddz=None, year=None, month=None):
        self.oddz = oddz
        if row:
            ...
            x = models.Cuw.objects.filter(year=year, month=month, sap_id=self.nr_sap)
            if len(x)>0:
                obj = x[0]
            else:
                obj = models.Cuw()
            ...
            obj.month = month
            obj.mag = self.oddz
            obj.sap_id = self.nr_sap
            obj.name = self.nazwisko
            obj.stawka = self.stawka
            obj.czas = self.czas
            obj.czas_noc = self.czas_noc
            obj.czas_nadl50 = self.czas_nadl50
            obj.czas_nadl100 = self.czas_nadl100
...
class DataOddz():
    def __init__(self, oddz=None, year=None, month=None):
        self.oddz = oddz
        self.year = year
        self.month = month
        self.godz_obce = 0
    ...
    def save(self):
        x = models.Oddz.objects.filter(year=self.year, month=self.month, mag = self.oddz)
        if len(x)>0:
            obj = x[0]
        else:
            obj = models.Oddz()
        ...
        obj.mag = self.oddz
        obj.godz_obce = self.godz_obce
        obj.naj_wyn = self.naj_wyn
        obj.il_rbg = self.il_rbg
        obj.save()
...
class ProdukcjaForm(forms.Form):
    ...
    def process(self, request, queryset=None):
        ...
        d1 = pendulum.datetime(int(rok), int(mies), 1)
        d2 = d1.add(months=1)
        d3 = d2.add(days=-1)
        
        data_start = d1.isoformat()[:10].replace('-','')
        data_stop = d2.isoformat()[:10].replace('-','')
        data_start_str = d1.isoformat()[:10]
        data_stop_str = d3.isoformat()[:10]
        ...
        if proc_id == 0:
            ...
            file_name = os.path.join(tmp_dir,'podsumowanie.xlsx')
            wb = openpyxl.Workbook()
            ws1 = wb.active
            ws1.title = "podsumowanie"
        
            with settings.DB as db:
                db.execute(select8 % (data_start, data_stop, data_start, data_stop, data_start, data_stop))        
                ...
                for row in db.fetchall():         
                    ...
                    o = models.Oddz.objects.filter(year=int(rok), month=int(mies), mag=row2[0])
                    if len(o)>0:
                        row2[3] = o[0].godz_obce
                    
                    czas = 0
                    c = models.Cuw.objects.filter(year=int(rok), month=int(mies), mag=row2[0])
                    for pos in c:
                        czas += float(pos.czas)
                    row2[2] = czas
                    ws1.append(row2)
            ...
            with open(file_name,"rb") as f:
                ...
                mail = EmailMessage("Plik premiowy - podsumowanie", MAIL_CONTENT, to=mail_to)    
                mail.attach("podsumowanie.xlsx", f.read(), "application/vnd.ms-excel")
                mail.send()                
            ...
            #return { 'status':  'etap', 'proc_id': proc_id, 'OK': True,  }
        ...
        #    test = self.cleaned_data['test']
            ...
            data = DataOddz(str(proc_id), int(rok), int(mies) )
            ...
            with settings.DB as db:
                ...
                for row in worksheet.rows:
                    ...
                    if not int(prac[0]) in cuw:
                        cuw[int(prac[0])] = {}
                    cuw[int(prac[0])][sap_id] = Prac(row, prac[0], int(rok), int(mies))
            ...
            dfile = os.path.join(settings.DATA_PATH, settings.PRJ_NAME)
...
class LabelForm(forms.Form):
    search = forms.CharField(label=_('Search'), required=True, max_length=None, min_length=None)
    ...
    def process(self, request, queryset=None):
        ...
        if search:
            symkar = search
            return { 'object_list': [ [str(symkar), 'TEST'], ], }
        else:
            return { 'object_list': []   }
    
    def process_empty(self, request, param=None):
        return { 'object_list': [ ['12345', 'TEST0'],],  }
    

def view_labelform(request, *argi, **argv):
    return PFORM(request, LabelForm, 'produkcja/formlabelform.html', {})



@dict_to_template('produkcja/v_genlabel.html')




def genlabel(request, **argv):
    
    return { 'doc_type': 'pdf' , 'width': int(70*72/25.4), 'height': int(120*72/25.4), }
