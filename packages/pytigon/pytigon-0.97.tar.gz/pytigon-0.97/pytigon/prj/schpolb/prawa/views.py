...
sel100 = """
select COALESCE(prawa_dzial.opis, tab3.dz) Stanowisko, tab3.nazwa  from (
    select tab.*, tab2.oper_count from
    (
        select COALESCE(prawa_dzial.profil, prawa_dzial.nazwa) dz, prawa_grupapraw.nazwa, count(*) grupa_count from prawa_operator
        ...
        where prawa_operator.aktywny = TRUE
        group by COALESCE(prawa_dzial.profil, prawa_dzial.nazwa), prawa_grupapraw.nazwa
    ) tab
    left join (
        select COALESCE(prawa_dzial.profil, prawa_dzial.nazwa) dz, count(*) oper_count from prawa_operator
        left join prawa_dzial on prawa_dzial.id = prawa_operator.dzial_id
        where prawa_operator.aktywny = TRUE
        group by COALESCE(prawa_dzial.profil, prawa_dzial.nazwa)
    ) tab2 on tab2.dz = tab.dz
) tab3
left join prawa_dzial on prawa_dzial.nazwa = tab3.dz
where grupa_count = oper_count and grupa_count > 1
order by COALESCE(prawa_dzial.opis, tab3.dz), tab3.nazwa
"""
...
sel201 = """
select distinct RightsOprName FROM dbo.rgt_vv_OprBro as s(NOLOCK) where Enabled=1 and exists (select top 1 * from dbo.rgt_vv_OprOperator as ss(NOLOCK) where ss.RightsOprName = s.RightsOprName)
"""

sel202 = """
select FunkcjaPraw, Opis FROM dbo.[w_funkcjePraw_opis] s WITH (NOLOCK) where Opis like '!%'
"""
...
class SOD(forms.Form):
    pdf = forms.NullBooleanField(label=_('Do pdf'), required=True, initial=False,)
    ...
    def process(self, request, queryset=None):
        ...
        with settings.DB as db:
            db.execute(sel201)
            actions = list([ row[0] for row in db.fetchall() ])
            db.execute(sel202)
            sod =db.fetchall()
            keys = {}
            for pos in sod:
        ...
        return { 'object_list': object_list, 'actions': actions, 'doc_type': doc_type, }
    

def view_sod(request, *argi, **argv):
    return PFORM(request, SOD, 'prawa/formsod.html', {})
