...
SEL1 = """
    select logo, platnik, nazwa2 from kli_kli kl(NOLOCK)
""" 
...
@dict_to_template('sprzedaz/v_customer_sync.html')
...
def customer_sync(request):
    ...
    object_list = []  
    with settings.DB as db:        
        #models.ErpCustomer.objects.all().update(active=False)
        db.execute(SEL1)
        tab = db.fetchall()        
        for pos in tab:        
            if pos[0] and pos[2]:
                objects = models.ErpCustomer.objects.filter(logo=pos[0])
                if len(objects)>0:
                    obj = objects[0]
                    continue
                else:
                    obj = models.ErpCustomer()
