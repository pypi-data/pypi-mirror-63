...
class ClientHandler:
    ...
    def _OnResourceResponse(
        ...
        if request.GetUrl().startswith("file://") and request.GetUrl().endswith(
            "example.html"
        ):
            data = (
                "<b style='color: green;'>This text was inserted through "
                + "_OnResourceResponse()</b><br>"
                + data
            )
        ...
        if FIX_ENCODING_BUG:
            ...
            if (
                ...
                if not re.search(r"<meta[^<>]+charset\s*=", data, re.IGNORECASE):
...
class ResourceHandler:
    ...
    def ProcessRequest(self, request, callback):
        ...
        request.SetFlags(
            cefpython.Request.Flags["AllowCachedCredentials"]
            | cefpython.Request.Flags["AllowCookies"]
        )
        # A strong reference to the WebRequest object must kept.
        self._webRequest = cefpython.WebRequest.Create(request, self._webRequestClient)
        return True
...
class MyApp(wx.App):
    ...
    def CreateTimer(self, frame):
        ...
        frame.Bind(wx.EVT_TIMER, self.OnTimer, id=self.timerID)
