...
class HardwareInfo:
    def __init__(self, dic):
        ...
        for k, v in dic.items():
            if k in ['cpu_id', 'cpu_name', 'arch_name', 'chip_name', 'vendor_name', 'board_name', 'ld_script', 'flash_program'] and v:
                self.__dict__[k] = v
            else:
                if type(v) == dict:
                    self.cpu_list[k] = v
    ...
    def out(self):
        ...
        if len(self.cpu_list) > 0:
            ...
            for k in ['cpu_id', 'arch_name', 'vendor_name', 'chip_name', 'cpu_name', 'board_name', 'ld_script']:
                if self.__dict__[k]:
...
{tag}{keywords}{homepage}{yoc_version}{license}
## 第二部分：依赖信息
...
{export}'''
        ...
        text = text.format(
            name=self.text_out('name', self.name, '# <必选项> 包名称 （符合C语言变量命名规则），长度少于等于64字节'),
            version=self.text_out('version', self.version),
            description=self.text_out('description', self.description, '# <必选项> 建议至少20字以上'),
            ...
            license=self.text_out('license', self.license, '# <可选项> 源代码的许可证，要确保所有代码、文件的许可证不冲突。如：MIT，Apache license v2.0，BSD'),
            homepage=self.text_out('homepage', self.homepage),
            type=self.text_out('type', self.type,'# <可选项> 组件类型，为：solution, chip, board, common，缺省值 common'),
        ...
        try:
            ...
            print("Generate %s file failed." % self.filename)
    ...
    def text_out(self, k, v, desc=''):
        ...
        v_len = string_len(v)
        if desc:
            desc = ' ' * (desc_width - k_len - v_len) + desc
        if v:
            return '{}: {}{}\n'.format(k, v, desc)
        return ''
    ...
    def author_out(self):
        ...
        if len(self.author) > 0:
            if type(self.author) == str:
                ...
                text = 'author:' + ' ' * (desc_width - 7) + '# <可选项> 原作者信息\n'
    ...
    def keywords_out(self):
        ...
        if len(self.keywords) > 0:
            text = 'keywords:' + ' '*(desc_width - 7) + '# <可选项> 标签，会影响到组件被搜索的效果，合理的标签很重要\n'
