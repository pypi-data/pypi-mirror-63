...
class DatasetAnnotationSetImageViewSet(
    ...
    def _order_images(self, order, queryset, limit=32, gallery=[]):
        ...
        not_annotated = self._query_images(dataset_id, gallery + skipped + annotated, limit)
    ...
    def _query_images(self, dataset_id, not_in, limit):
        skip_ids = ""
        if len(not_in):
        ...
        query = """
                SELECT "id"
                FROM "dataset_images"
                WHERE "dataset_id" = {dataset_id} {skip_ids}
                ORDER BY "id" ASC
                LIMIT {limit}
                """.format(
            dataset_id=dataset_id,
    ...
    def get_annotated_data(self, request, *args, **kwargs):
        ...
        if qs:
            ...
            return Response({
                'error': False,
                'annotation_info': serializer_class(
                    annotation, context=self.get_serializer_context()).data['annotation_info'],
    ...
    def save_annotation(self, request, *args, **kwargs):
        ...
        annotation_set = self.get_parent_object_or_404()
        annotation_objects = [{**obj_data, 'position_number': i} for i, obj_data in
                              enumerate(self.request.data.get('objects', []), start=1)]
        if len(annotation_objects) and 'coordinates' in annotation_objects[0]:
            for i in range(len(annotation_objects)):
                coordinates = annotation_objects[i].get('coordinates', [])
                if len(coordinates):
                    annotation_objects[i]['coordinates'] = list(
                        map(lambda val: {'x': int(val['x']), 'y': int(val['y'])}, coordinates))
