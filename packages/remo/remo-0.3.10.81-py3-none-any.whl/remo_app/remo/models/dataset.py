...
class Image(models.Model):
    ...
    """
    original = models.FileField(upload_to=get_original_file_path, null=True)
    image = models.FileField(upload_to=get_image_file_path, null=True)
    local_image = models.CharField(max_length=1000, null=True)
    thumbnail = models.FileField(upload_to=get_thumbnail_file_path, null=True)
    view = models.FileField(upload_to=get_view_file_path, null=True)
    preview = models.FileField(upload_to=get_preview_file_path, null=True)
...
class Dataset(models.Model):
    ...
    def clone_relations_from(self, instance):
        ...
        for dataset_annotation_set in instance.annotation_sets.all():
            old_obj = dataset_annotation_set.__class__.objects.get(pk=dataset_annotation_set.pk)
            dataset_annotation_set.pk = None
            dataset_annotation_set.dataset = self

            # Clear release date for new private dataset cause this
            # field has sense only for public ones, DEV-246
            dataset_annotation_set.released_at = None

            dataset_annotation_set.save()
            dataset_annotation_set.clone_relations_from(old_obj)
            dataset_annotation_set.save()

            annotation_set_ids[old_obj.pk] = dataset_annotation_set.pk

        for dataset_img in instance.dataset_images.all():
            old_obj = dataset_img.__class__.objects.get(pk=dataset_img.pk)
            dataset_img.pk = None
            dataset_img.dataset = self
            dataset_img.save()
            dataset_img.clone_relations_from(old_obj, annotation_set_ids)
            dataset_img.save()

        self.users_shared = instance.users_shared.all()
