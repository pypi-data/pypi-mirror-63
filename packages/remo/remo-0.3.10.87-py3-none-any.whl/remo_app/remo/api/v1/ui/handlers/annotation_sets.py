...
from remo_app.remo.api.constants import TaskType, AnnotationSetType, AnnotationSetStrictMode
...
class AnnotationSets(mixins.RetrieveModelMixin,
    ...
    def create(self, request, *args, **kwargs):
        ...
        if len(classes):
            strict_mode = AnnotationSetStrictMode.strict.value
    ...
    def export(self, request, *args, **kwargs):
        ...
        if annotation_format not in ['coco', 'json', 'csv']:
            return Response({'error': f'Exporter parameter annotation_format: {annotation_format}, not valid.'},
                            status=status.HTTP_400_BAD_REQUEST)

        full_path = request.query_params.get('full_path', 'false')
        if full_path not in ['false', 'true']:
            return Response({'error': f'Exporter parameter full_path: {full_path}, not valid.'},
                            status=status.HTTP_400_BAD_REQUEST)
        full_path = True if full_path == 'true' else False

        export_classes = request.query_params.get('export_classes', 'false')
        if export_classes not in ['false', 'true']:
            return Response({'error': f'Exporter parameter export_classes: {export_classes}, not valid.'},
                            status=status.HTTP_400_BAD_REQUEST)
        export_classes = True if full_path == 'true' else False
        # TODO: add export classes

        export_coordinates = request.query_params.get('export_coordinates', 'pixel')
        if export_coordinates not in ['pixel', 'percent']:
            return Response({'error': f'Exporter parameter export_coordinates: {export_coordinates}, not valid.'},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if annotation_format == 'csv':
            ...
            return Response(output, headers=headers, content_type=content_type)
