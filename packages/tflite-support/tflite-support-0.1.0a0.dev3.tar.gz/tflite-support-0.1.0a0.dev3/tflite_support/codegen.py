...
from tflite_support import _pywrap_codegen

FLAGS = flags.FLAGS

flags.DEFINE_string('model', '', 'Path to model (.tflite) flatbuffer file.')
flags.DEFINE_string('destination', '', 'Path of destination of generation.')
...
flags.DEFINE_string(
    'model_asset_path', '',
    '(Optional) Path to the model in generated assets/ dir. If not set, generator will use base name of input model.'
)


def get_model_buffer(path):
  if not os.path.isfile(path):
    logging.fatal('Cannot find model at path %s.', path)
...
def prepare_directory_for_file(file_path):
  ...
  if not os.path.isdir(target_dir):
    logging.fatal('Cannot write to %s', target_dir)
