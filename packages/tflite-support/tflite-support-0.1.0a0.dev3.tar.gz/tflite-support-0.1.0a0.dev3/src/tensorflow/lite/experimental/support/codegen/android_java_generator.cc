...
TensorInfo CreateTensorInfo(const TensorMetadata* metadata,
  ...
  FindAssociatedFile(metadata, tensor_identifier,
                     &(tensor_info.associated_axis_label),
                     &(tensor_info.associated_axis_label_index), err);
  if (is_input && !tensor_info.associated_axis_label.empty()) {
    err->Warning(
        "Found axis label file on input tensor (%s). Label file for input "
  ...
  switch (metadata->content()->content_properties_type()) {
...
void SetCodeWriterWithTensorInfo(CodeWriter* code_writer,
  ...
  code_writer->SetTokenValue("ASSOCIATED_AXIS_LABEL",
                             tensor_info.associated_axis_label);
...
private static final String MODEL_NAME = "{{MODEL_PATH}}";)");
  for (const auto& tensor : model.outputs) {
    if (!tensor.associated_axis_label.empty()) {
...
private final QuantizationParams {{NAME}}QuantizationParams;)");
  ...
  code_writer->NewLine();
  {
    const auto ctor_block = AsBlock(
        code_writer, "public Metadata(ByteBuffer buffer) throws IOException");
    code_writer->Append(
...
NormalizationOptions {{NAME}}NormalizationOptions =
    (NormalizationOptions) extractor.getInputTensorMetadata({{ID}}).processUnits(0).options(new NormalizationOptions());
...
public {{MODEL_CLASS_NAME}}(Context context, String modelPath, Device device, int numThreads) throws IOException {
  ...
  metadata = new Metadata(model.getData());)");
  for (const auto& tensor : model.inputs) {
    SetCodeWriterWithTensorInfo(code_writer, tensor);
    code_writer->Append(R"(
...
dependencies {
    ...
    api files('libs/tensorflow-lite-support.aar')
    api files('libs/tensorflow-lite-support-metadata.aar')
    api files('libs/metadata_schema_fbs_android.aar')
    api files('libs/schema_fbs_android.aar')
