...
/// Finds an associated file from a TensorMetadata. The output will be written
/// to "file_path" and "file_index".  If there're multiple files meet the
/// criteria, only the first one is used. If there's no file meets the criteria,
/// "file_path" will set as empty and "file_index" will set as -1.
/// Note: Currently we only find "TENSOR_AXIS_LABEL" file, as it's the only
/// one supported by codegen.
