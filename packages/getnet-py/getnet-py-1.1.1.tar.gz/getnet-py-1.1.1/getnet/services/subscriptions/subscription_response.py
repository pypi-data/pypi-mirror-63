...
class SubscriptionCreditResponse(BaseCreditResponse):
    authorization_timestamp: datetime
    soft_descriptor: str

    def __init__(self, **kwargs):
        if len(kwargs) > 3:
            self.authorization_timestamp = kwargs.pop("authorization_timestamp")
            self.soft_descriptor = kwargs.pop("soft_descriptor")
            kwargs['authorized_at'] = None
            super(SubscriptionCreditResponse, self).__init__(**kwargs)

        self.delayed = kwargs.get('delayed')
        self.reason_code = kwargs.get('reason_code')
        self.reason_message = kwargs.get('reason_message')


class PaymentResponse(BasePaymentResponse):
    credit: BaseCreditResponse
    payment_received_timestamp: datetime

    def __init__(self, credit: dict, payment_received_timestamp: str = None, **kwargs):
        self.payment_received_timestamp = (
            payment_received_timestamp
            if isinstance(payment_received_timestamp, datetime) or payment_received_timestamp is None
            else parser.isoparse(payment_received_timestamp)
        )
        super(PaymentResponse, self).__init__(**kwargs)
        self.credit = SubscriptionCreditResponse(**credit)
