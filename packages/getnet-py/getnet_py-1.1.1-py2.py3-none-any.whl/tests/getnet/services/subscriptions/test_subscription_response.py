...
from getnet.services.subscriptions.subscription_response import PaymentResponse
...
sample_error.update({
    "payment": {
        'seller_id': '75c5adac-bfb2-4726-81fd-2e68b044c669',
        'order_id': 'test-28593325',
        'payment_id': '0597ce89-d715-4230-b87f-1c1710263f7c',
        'amount': 1990,
        'currency': 'BRL',
        'status': 'denied',
        'credit': {
            'delayed': False,
            'reason_code': 'PAYMENTS-116',
            'reason_message': 'Erro ao efetuar transação de crédito'
        }
    }
})
...
class PaymentErrorResponseTest(unittest.TestCase):
    def testInit(self):
        error = PaymentResponse(**sample_error.get('payment'))

        self.assertEqual('denied', error.status)
        self.assertEqual('PAYMENTS-116', error.credit.reason_code)
