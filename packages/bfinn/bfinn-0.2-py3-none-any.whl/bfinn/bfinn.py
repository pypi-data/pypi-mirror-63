"""Base for BFINN sentiment analysis."""
...
LANGUAGE_TO_FILENAME = {
    'da': 'BFINN-da-32.txt',
    'en': 'BFINN-en-165.txt',
    'emoticons': 'BFINN-emoticon-8.txt',
    'fr': 'BFINN-fr-165.txt',
    'sv': 'BFINN-sv-165.txt',
    }


class BfinnException(Exception):
    """Base for exceptions raised in this module."""

    pass


class WordListReadingError(BfinnException):
...
class Bfinn(object):
    ...
    --------
    >>> bfinn = Bfinn()
    >>> bfinn.score('This is oh so bad.') < 0
    True

    >>> bfinn = Bfinn(language='da')
    >>> bfinn.score('Det er bare vidunderlig!!!') > 0
    True

    >>> bfinn = Bfinn(emoticons=True)
    >>> bfinn.score('My reaction: :-)') > 0
    True

    >>> bfinn = Bfinn(language='da', emoticons=True)
    >>> bfinn.score('Det er bare :-)))') > 0
    True
    ...
    def full_filename(self, filename):
        ...
        --------
        >>> bfinn = Bfinn()
        >>> filename = bfinn.full_filename('BFINN-111.txt')
        >>> from os.path import split
        >>> split(filename)[-1]
        'BFINN-111.txt'
    ...
    def regex_from_tokens(tokens, word_boundary=True, capture=True):
        ...
        --------
        >>> bfinn = Bfinn()
        >>> bfinn.regex_from_tokens(['good', 'bad'])
        '(\\b(?:good|bad)\\b)'

        >>> bfinn.regex_from_tokens(['good', 'bad'], word_boundary=False,
        ...     capture=False)
        '(?:good|bad)'
    ...
    def find_all(self, text, clean_whitespace=True):
        ...
        --------
        >>> bfinn = Bfinn()
        >>> bfinn.find_all('It is wonderful!')
        ['wonderful']
    ...
    def scores_with_pattern(self, text):
        ...
        --------
        >>> bfinn = Bfinn()
        >>> bfinn.scores_with_pattern('Good and bad')
        [3, -3]

        >>> bfinn.scores_with_pattern('some kind of idiot')
        [0, -3]
