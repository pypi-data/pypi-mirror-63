...
class Socket():
    ...
    def listenToStream(self):
        try:
            ...
            self.log.exception(f"Suppressing a caught an exception, will attempt to reconnect to recover the connection and continue without raising. Details below\n{type(e)}: {traceback.format_exc()}")
            self.reconnect()
    ...
    def health_check(self):
        if (datetime.now() - self.last_ping).seconds > 360:
            self.log.error(f"extended delay detected between pings, manually calling reconnect.[PING timeout: {(datetime.now() - self.last_ping).seconds} sec]")
            self.reconnect()
