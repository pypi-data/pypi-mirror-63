...
def __parse_usernotice(line: str) -> UserNotice:
    ...
    if notice_type in ("sub", "resub"):
        ...
        months = __get_single(line, "msg-param-cumulative-months")
        if months is None:
            msg.cumulative_months = 0
        else:
            msg.cumulative_months = int(months)
        months = __get_single(line, "msg-param-streak-months")
        if months is None:
            msg.streak_months = 0
        else:
            msg.streak_months = int(months)
        ...
        viewers = __get_single(line, "msg-param-viewerCount")
        if viewers is None:
            msg.viewer_count = 0
        else:
