...
def macd(df, price="close", col="macd", fast_period, slow_period, signal_period):
    """
    ...
    df = ema(df, price, col + "_fast_ema", fast_period)
    df = ema(df, price, col + "_slow_ema", slow_period)
    df[col] = df[col + "_fast_ema"] - df[col + "_slow_ema"]
    df = ema(df[slow_period - 1 :], macd, macd + "_signal", signal_period)
    df[col + "_hist"] = df[col] - df[col + "_signal"]
    df.drop([col + "_fast_ema", col + "_slow_ema"], axis=1, inplace=True)
