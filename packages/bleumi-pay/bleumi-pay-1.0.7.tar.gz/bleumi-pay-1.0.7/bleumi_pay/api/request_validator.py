...
class RequestValidator(object):
    ...
    def is_eth_addr(self, addr):
        ...
        return ((match != None)) or (input == 'ETH') or (input == 'XDAI') or (input == 'XDAIT')
    ...
    def validate_create_payment_request(self, params, chain=None):
        ...
        msg = self.check_network_addr('BuyerAddress', params.buyer_address, chain, True, False)   
        if msg is not None:
            return msg

        # check if transfer_address is valid address in the network  
        msg = self.check_network_addr('TransferAddress', params.transfer_address, chain, True, False)   
        if msg is not None:
            return msg

        # check if token is valid address in the network (if provided)
        if params.token is not None:
            msg = self.check_network_addr('Token', params.token, chain, False, True)   
            if msg is not None:
                return msg
    ...
    def validate_create_checkout_url_request(self, params):
        ...
        if params.token is not None:  
            ...
            if params.buyer_address is not None:  
                msg = self.check_network_addr('BuyerAddress', params.buyer_address, params.chain, False, False)   
                if msg is not None:
                    return msg
