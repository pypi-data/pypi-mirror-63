...
def get_data_progress(progress: float, msg: str, value: Dict) -> int:
    """
...
class CdasWs:
    ...
    def get_data_result(self, data_request: Dict) -> Tuple[int, Dict]:
        """
        ...
        ----------
        data_request
            data request represented by a dictionary mirroring a
            DataRequest element in
            <https://cdaweb.sci.gsfc.nasa.gov/WebServices/REST/CDAS.xsd>.
        Returns
        -------
        Tuple
    ...
    def download(self, url: str, size: int, **keywords) -> str:
        """
    ...
    def get_datetime(value: Union[datetime, str]) -> datetime:
        """
        Gets a datetime object from the given value.

        Parameters
        ----------
        value
            datetime object or string value.
        Returns
        -------
        datetime
        ...
        ------
        ValueError
            If the given value is not a valid datetime.
        """
    ...
    @staticmethod
    def create_cdf_request(dataset: str,
                           variables: List[str],
                           start: datetime,
                           end: datetime,
                           bin_data: Dict) -> Dict:
        """
        Creates a dictionary representation of a DataRequest for
        a CDF.
        ...
        bin_data
            bin_data dictionary or None.
        Returns
        -------
        Dict
            dictionary representation of a CDF DataRequest with the
            given values.
        """
    ...
    def get_data(self, dataset: str, variables: List[str],
        ...
        try:
            start_datetime = CdasWs.get_datetime(start)
        except ValueError:
            raise ValueError('unrecognized start datetime value')

        try:
            end_datetime = CdasWs.get_datetime(end)
        except ValueError:
            raise ValueError('unrecognized end datetime value')

        data_request = CdasWs.create_cdf_request(dataset,
                                                 variables,
                                                 start_datetime,
                                                 end_datetime,
                                                 keywords.get('binData', None))
        ...
        tmp_filename = self.download(data_url, data_length,
                                     progressCallback=get_data_progress,
