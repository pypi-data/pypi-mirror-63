...
from cdasws.datarequest import DataRequest, CdfFormat, CdfRequest, ImageFormat, GraphOptions, GraphRequest
...
class CdasWs:
    ...
    def get_data_result(self,
                        data_request: DataRequest) -> Tuple[int, Dict]:
        """
    ...
    def get_graph(self, 
        ...
        """
        Gets the a graphical representation of the spedified data from the
        server.
        ...
        if 'Status' in result:
            status['cdas']['status'] = result['Status']
        if 'Message' in result:
            status['cdas']['message'] = result['Message']
        if 'Warning' in result:
            status['cdas']['warning'] = result['Warning']
        if 'Error' in result:
            status['cdas']['error'] = result['Error']
