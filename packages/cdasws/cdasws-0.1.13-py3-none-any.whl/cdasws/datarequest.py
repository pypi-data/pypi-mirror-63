...
sub-classes from
<https://cdaweb.sci.gsfc.nasa.gov/WebServices/REST/CDAS.xsd>.<br>

Copyright &copy; 2019 United States Government as represented by the
...
class DataRequest(metaclass=ABCMeta): # pylint: disable=too-few-public-methods
    ...
    def __init__(self,
        ...
        keywords
            ...
            binInterval - Binning interval (seconds).
            binInterpolateMissingValues - Whether to interpolate
                mission values.  If false, a fill value will be used.
            binSigmaMultiplier - Standard deviation multiplier used for
                rejecting data.  If less than 1, spike removal is skipped.
                4 is less aggressive.  6 is more aggressive.
        """
        ...
        if 'binInterval' in keywords or \
           'binInterpolateMissingValues' in keywords or \
           'binSigmaMultiplier' in keywords:
            self._data_request[request_name]['BinData'] = {}
            if 'binInterval' in keywords:
                self._data_request[request_name]['BinData']['Interval'] = \
                    keywords['binInterval']
            if 'binInterpolateMissingValues' in keywords:
                self._data_request[request_name]['BinData']['InterpolateMissingValues'] = \
                    keywords['binInterpolateMissingValues']
            if 'binSigmaMultiplier' in keywords:
                self._data_request[request_name]['BinData']['SigmaMultiplier'] = \
                    keywords['binSigmaMultiplier']
...
class AudioRequest(DataRequest): # pylint: disable=too-few-public-methods
    """
    Class representing a AudioRequest from
    <https://cdaweb.sci.gsfc.nasa.gov/WebServices/REST/CDAS.xsd>.
...
def main():
    ...
    request = CdfRequest('AC_H1_MFI',
                         ...
                         #3, CdfFormat.BINARY,
                         binInterval=60.0,
                         binInterpolateMissingValues=True,
                         binSigmaMultiplier=4
                         )
    print(request.json(indent=4, sort_keys=True))
