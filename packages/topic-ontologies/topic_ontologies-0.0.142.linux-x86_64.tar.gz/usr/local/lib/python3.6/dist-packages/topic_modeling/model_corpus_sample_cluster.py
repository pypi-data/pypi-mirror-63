...
fs = FileSystem.get(URI("hdfs://betaweb020.medien.uni-weimKar.de:8020"), spark_context._jsc.hadoopConfiguration())
...
def project_documents(topic_ontology):
    def project_document(argument):
        ...
        vector = argument_esa_model.esa_all_terms.model_topic(path_topic_model,path_word2vec_model,path_word2vec_vocab,'max',argument)
    ...
    documents_df  = spark.read.format("csv").option("header", "true").option("delimiter", "|", ).option('quote', '"').load(path_documents).na.drop().sample(0.2,False)
    documents = documents_df.select("text").rdd.map(lambda r: r[0]).repartition(100)
