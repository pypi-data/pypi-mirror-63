...
def project_arguments(topic_ontology,model,vocab_size,is_cluster_corpora,is_testing):
    ...
    for corpus in corpora:
        ...
        if granularity == 'argument':
            ...
            documents_df  = spark.read.format("csv").option("header", "true").option("delimiter", "|", ).option('quote', '"').load(path_arguments)
...
project_arguments('debatepedia','esa',1000,False,True)
#project_arguments('wikipedia','esa',1000,True,True)
#project_arguments('strategic-intelligence','esa',1000,True,True)
#project_arguments('strategic-intelligence-sub-topics','esa',1000,True,True)