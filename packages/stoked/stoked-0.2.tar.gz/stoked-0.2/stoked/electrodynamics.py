...
class point_dipole_electrodynamics(interactions):
    ...
    def solve(self):
        ...
        A = np.einsum('ixjy,jyz->ixjz', self.G, self.alpha_t)

        if self.interactions is None:
            self.sol = np.linalg.tensorsolve(identity - A, Einc)
            self.prev_sol = np.copy(self.sol)
        else:
            N = int(np.floor(self.interactions))
            sol = np.copy(Einc)
            self.prev_sol = np.copy(sol)

            for i in range(N):
                self.prev_sol = np.copy(sol)
                sol = Einc + np.einsum('NaMb,Mb->Na', A, sol)
    ...
    def force(self):
        ...
        def Efield_without(r, i):
            ...
            if self.interactions is None:
                sol = self.sol
            elif self.interactions // 2 != self.interactions / 2:
                sol = self.prev_sol
            else:
                sol = self.sol
            ...
            for j in range(self.Nparticles):
                ...
                E += np.einsum('xy,yz,z', G, self.alpha_t[j], sol[j]) 
            return E
        ...
        eps = 1e-12
        for i in range(self.Nparticles):
            E0  = Efield_without(self.position[i], i)
            ...
            dE = (np.array([Edx,Edy,Edz]) - E0)/eps
