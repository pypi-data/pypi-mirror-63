...
class SQLMap:
	...
	def update (self, table, alias = None):		
		return sql.SQLComposer ("UPDATE " + self.__resolve (self._render_table (table), alias) + " SET {this.pairs}", self._engine, self._conn, True)
	...
	def delete (self, table, alias = None):
		return sql.SQLComposer ("DELETE FROM " + self.__resolve (self._render_table (table), alias), self._engine, self._conn, True)
