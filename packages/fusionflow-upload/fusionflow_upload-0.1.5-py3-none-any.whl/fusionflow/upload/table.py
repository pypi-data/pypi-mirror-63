...
from .sql import Sql
from .utils import get_tz, to_callable
...
def table(uri: str, *args, **kwargs):
    ...
    * db
    ...
    # file://path/?increment=Full&timezone="GMT+08"
    service = Table.from_uri(uri)
    return service
...
class Table(metaclass=TableMeta):
    ...
    @classmethod
    def from_uri(cls: Type['Table'], uri: str) -> Type['Table']:
        return cls
    ...
    def registe_validator(self, field: Union[None, str, Field], func:
        ...
        **Caution**: if last validator modify field type, it must contain a
        ...
        validator = to_callable(func) if not callable(func) else func
        if field is None:
    ...
    def __copy__(self):
        """support copy incase not all parameters
        passed to ``__init__`` so default copy will
        return a ``PartialTable`` and can not go on
        """
        paras, attrs = self.__get_parameters_and_attributes()
        obj = type(self)(**paras)
    ...
    def __deepcopy__(self, memo):
        """
        deep copy all atrributes
        :return: [description]
        :rtype: [type]
        """
        paras, attrs = map(copy.deepcopy,
                           self.__get_parameters_and_attributes())
        obj = type(self)(**paras)
        for key, val in attrs.items():
            setattr(obj, key, val)
        return obj
    ...
    def __getnewargs_ex__(self):
        """support parameters set in script, these values will saved in
        _parameter_values, like ``__copy__``, if not all required parameters
        passed into ``__new__``, a PartialTable will return, and that will
        cause :py:class:`TypeError`_ invalid partial state.

        do not use ``__reduce__``, reduce worked when instance already create,
        not in this context.

        **Caution**:
        currently before pickle, do not access any properties about field info,
        or extract any data. because of generator can not pickled or copied,
        and field info extract from data now.
        """
        params, attrs = self.__get_parameters_and_attributes()
        return (), params

    def __get_parameters_and_attributes(
        self
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """return parameters and instance private attribtues
        used for copy and pickle

        parameters: all required ``Parameter`` values to return the
        right instance, otherwise
        a ``PartialTable`` will return, and cause type error
        """
        parameters = self._parameter_values
        attributes = {}
        for key, val in self.__dict__.items():
            if key != '_parameter_values':
                attributes[key] = val
