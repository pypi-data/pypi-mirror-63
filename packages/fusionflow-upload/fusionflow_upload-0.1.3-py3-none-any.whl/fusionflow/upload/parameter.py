...
class Parameter:
    ...
    :param type_:
        parameter type, use for format value, use this callable,
        otherwise return directly
    :param required: ``bool``
        must provide this parameter, otherwise default value must set
    :param default:
        default value for no required parameter, call be callable,
        like :method:`datetime.now`_
    ...
    def __init__(self, type_=None, *,
                 required=True, default=None, positional=False, help=None):

        if isinstance(default, (MutableMapping, MutableSequence, MutableSet)):
            warnings.warn("""{} is mutable contain, subclass which inheriate
            this paramater will shared this parameter
            both manipulate it will lead unexpected result,
            use `lambda : {}()` instead
    ...
    def __get__(self, instance, cls):
        ...
        if self.name not in instance._parameter_values:
    ...
    def __repr__(self):
        ...
        """.format(
            parametertype=type(self).__name__, type=self.type_, name=self.name,
