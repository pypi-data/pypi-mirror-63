...
def general_outtype_type_cast(
    cursor, name, defaultType, size, precision, scale
):
    """change oracle cursor default return value type
    `cx_Oracle types mapping `ref <https://cx-oracle.readthedocs.io/en/latest/user_guide/sql_execution.html#fetch-data-types`_>`_
...
class OracleDatabaseTable(DatabaseTable):
    ...
    def extract(self):
        with connect(self.username, self.password, self.dsn) as connection:
            # use utc internally
            # if datetime_tz set to not utc, formanize native time using this
                ...
                yield [
                    {
                        ...
                        'value':col, 'type': self.type_covert[field_types[idx]]
                    }
