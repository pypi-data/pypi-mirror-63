...
def regularize_proxies(proxies):
    if isinstance(proxies, str):
        proxies = {
            'http': proxies,
            'https': proxies,
        }
    elif proxies is None or isinstance(proxies, dict):
        proxies = proxies
    else:
        raise ValueError(f'proxies: {proxies} invalid')
    return proxies
...
def base_convert(read_obj, read_index: int = -1, read_format=None,
    ...
    default_url = CONF.get("default", "server")
    url = os.environ.get("CHEMIO_SERVER_URL", default_url)
    proxies = regularize_proxies(proxies)
    logger.debug(f"url: {url}, files: {files}, payload: {payload}, proxies: {proxies}")
    response = requests.post(url, files=files, data=payload, proxies=proxies)
