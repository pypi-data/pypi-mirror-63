...
import cryoloBM.boxmanager as boxmanager

def config_boxmanager_parser(parser_config):
    config_required_group = parser_config.add_argument_group(
        "General arguments",
        "Options to start the box manager.",
    )

    config_required_group.add_argument("-i", "--image_dir", help="Path to image directory.", widget="DirChooser")

    config_required_group.add_argument("-b", "--box_dir", help="Path to box directory.", widget="DirChooser")
...
def config_configuration_parser(parser_config):
    ...
    config_other_group = parser_config.add_argument_group(
        "Other option",
        "Other options that does not fit anywhere else",
    )
...
def main():
    ...
    if "config" in sys.argv[1]:
        ...
        boxmanager.start_boxmanager(args.image_dir, args.box_dir)
