...
jup = np.array([np.sum(np.array(state['Jupiter'])**2) for state in solar_system.history])
jup = np.sqrt(jup)

SUNNY = np.array([np.sum(np.array(state['Sun'])**2) for state in solar_system.history])
SUNNY = np.sqrt(SUNNY)
