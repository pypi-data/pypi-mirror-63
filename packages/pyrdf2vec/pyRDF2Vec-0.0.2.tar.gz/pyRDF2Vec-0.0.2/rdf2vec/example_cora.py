...
g = rdflib.Graph()
g.parse('../data/CORA/cora.ttl', format='turtle')
print('OK')

# Load our train & test instances and labels

labels = pd.read_csv('../data/CORA/label.txt', sep='\t', header=None, index_col=0)
train_ids = [x.strip() for x in open('../data/CORA/train.txt', 'r').readlines()]
test_ids = [x.strip() for x in open('../data/CORA/test.txt', 'r').readlines()]
val_ids = [x.strip() for x in open('../data/CORA/dev.txt', 'r').readlines()]
...
ano_walker = AnonymousWalker(4, float('inf'))
walklet_walker = WalkletWalker(4, float('inf'))
ngram_walker = NGramWalker(4, float('inf'), n_wildcards=1)
wl_walker = WeisfeilerLehmanWalker(4, float('inf'))
com_walker = CommunityWalker(4, float('inf'))
halk_walker = HalkWalker(4, float('inf'), freq_threshold=0.1)

# Create embeddings using Weisfeiler-Lehman
combined_walk_strats = [random_walker, com_walker, halk_walker, ano_walker, walklet_walker]
embeddings = []
for strat in combined_walk_strats:
	transformer = RDF2VecTransformer(walkers=[strat], sg=1)
	embeddings.append(transformer.fit_transform(kg, train_entities + test_entities))
wl_embeddings = np.hstack(embeddings)
...
print(np.array(walk_embeddings).shape, np.array(wl_embeddings).shape)

# transformer = RDF2VecTransformer(walkers=[com_walker, halk_walker, ano_walker, walklet_walker], sg=1)
