...
class TradingEnv(gym.Env):
    ...
    def __init__(self, df, window_size, max_loss=None, hold_penalty_ticks=None, penalty_size=0.1):
        assert df.ndim == 2
        self._start_position = Positions.Out
        self._fill_action = Actions.Out
        self._penalty_size = penalty_size
        self.df = df
        ...
        self.observation_space = spaces.Box(low=-250, high=250, shape=self.shape, dtype=np.float32)
    ...
    def step(self, action):
        ...
        if state.is_trade_end or self._done:
            ...
            if state.penalty is not None and self._current_tick - self._last_trade_tick > state.penalty:
                step_reward = -self._penalty_size
