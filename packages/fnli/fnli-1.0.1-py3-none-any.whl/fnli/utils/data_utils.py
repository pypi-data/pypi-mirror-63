...
def get_file(fname,
    ...
    `example.txt` would therefore be `~/.Fnli/datasets/example.txt`.
    ...
    if not os.access(datadir_base, os.W_OK):
        datadir_base = os.path.join('/tmp', '.fnli')
        ...
        def dl_progress(count, block_size, total_size):
            if ProgressTracker.progbar is None:
                ...
                if count * block_size > total_size:
                    ProgressTracker.progbar.update(total_size)
                else:
...
def _hash_file(fpath, algorithm='sha256', chunk_size=65535):
    ...
    ```python
        >>> from Fnli.utils.data_utils import _hash_file
...
def validate_file(fpath, file_hash, algorithm='auto', chunk_size=65535):
    ...
    if str(_hash_file(fpath, hasher, chunk_size)) == str(file_hash):
        ...
        print("1- ", str(_hash_file(fpath, hasher, chunk_size)))
        print("2- ", str(file_hash))
        return False
...
class OrderedEnqueuer(SequenceEnqueuer):
    ...
    # Arguments
        sequence: A `Fnli.utils.data_utils.Sequence` object.
