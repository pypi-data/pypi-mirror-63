...
class Processor:
    ...
    def sanity_checks(self):
        if not self.input or not self.input_exists:
            return (
                False,
                f"The input specified '{self.input}' did not exist. Input must be an existing directory or file.",
            )
        if not self.overwrite and self.on_change:
            return (
                False,
                f"On-change will not have an effect so long as overwrite is not enabled.",
            )
        if not self.output and not self.overwrite and not self.format:
            return (
                False,
                f"Only input '{self.input}' was specified. Without a setting for 'output', 'overwrite' and/or 'format' all processing will fail.",
            )
        if path_is_in_path(self.input, self.output):
            return (
                False,
                f"The output '{self.output}' is a subpath of input '{self.input}'.",
            )
        if path_is_in_path(self.output, self.input):
            return (
                False,
                f"The input '{self.input}' is a subpath of output '{self.output}'.",
            )
        return True, None
    ...
    def process_file(self, input_path: str, output_path: str):
        ...
        if not input_path:
            logger.warning(f"No input path specified")
            return False
        if not output_path:
            logger.warning(f"No output path specified")
            return False
        if os.path.isfile(output_path):
            if not self.overwrite:
                logger.warning(f"Trying to overwrite without overwrite enabled")
                return False
            if self.on_change:
                ...
                if input_mtime <= output_mtime:
                    if self.verbose:
                        logger.info(f"File not changed, skipping")
                    return True
        else:
            ...
            if not os.path.isdir(output_dir):
                logger.warning(
                    f"Directory of '{output_path}' did not exist or could not be created."
                )
                return False
        extension = determine_file_extension(input_path)
        if not extension:
            logger.warning(
                f"Unknown extension '{extension}' for input file: '{input_path}'."
            )
            return False
        ...
        try:
            with open(input_path, "rb") if is_binary else open(
                input_path, "r", encoding="utf-8"
            ) as file:
                original_content = file.read()
        except Exception as err:
            logger.warning(
                f"Could not load data from input file '{input_path}': {err}."
            )
            return False
        ...
        if extension in [
            ...
            "png",
            "jpeg",
        ] and self.settings.get(f"disable_{extension}", False):
            # Perform copy
        ...
        if None == processed_content:
            ...
            # return False # This is only a warning and not an error
        try:
            with open(output_path, "wb") if is_binary else open(
                output_path, "w", encoding="utf-8"
            ) as file:
                written = file.write(processed_content)
                if written != len(processed_content):
                    logger.warning(
                        f"Partially written output ({written} of {len(processed_content)} bytes)."
                    )
                    return False
        except Exception as err:
            logger.warning(
                f"Could not write data to output file '{output_path}': {err}."
            )
            return False
    ...
    def process_files_list(self, list):
        ...
        for item in list:
            ...
            if single_interval > datetime.timedelta(seconds=1):
                logger.warning(
                    f"Processing of {input_path} was slow ({human_delta(single_interval)})"
                )
        # self.pool.close()
        # self.pool.join()
        # self.pool=None
        ...
        logger.info(
            f"Successful processing of {success_count}/{len(list)} items took {human_delta(interval)} total"
        )
        return True
    ...
    def _process_existing_dir_to_existing_dir(self):
        ...
        for input_path in input_paths:
            common = os.path.commonpath(
                (os.path.abspath(self.output), os.path.abspath(input_path))
            )
            rel = os.path.relpath(
                os.path.abspath(input_path), os.path.abspath(self.input)
            )
        ...
        if self.input_is_dir:
            ...
            if self.output:
                if self.output_is_dir:
                    if self.verbose or True:
                        logger.info(
                            f"existing-file-to-existing-dir easy peasy case NOT IMPLEMENTED YET!"
                        )
                    return False
                elif not self.output_exists:
                    if self.verbose or True:
                        logger.info(
                            f"existing-file-to-new-file easy peasy case NOT IMPLEMENTED YET!"
                        )
                    return False
                else:
                    if self.overwite:
                        if self.verbose or True:
                            logger.info(
                                f"existing-file-to-existing-file overwrite easy peasy case NOT IMPLEMENTED YET!"
                            )
                        return False
                    else:
                        logger.error(
                            f"existing-file-to-existing-file non-overwrite error"
                        )
                        return False
                return False
            elif self.overwite:
