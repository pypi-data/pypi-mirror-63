...
def _props_grouper(props, pgs):
    ...
    props_by_groups = map(
        lambda item: sorted(item, key=lambda item: item[0][0]), props_by_groups
    )
    props = []
...
def condense_std_named_colors(css):
    ...
    for color_name, color_hexa in iter(
        tuple(
            {
                ":aqua;": ":#0ff;",
                ":blue;": ":#00f;",
...
def restore_needed_space(css):
    ...
    return (
        css.replace("!important", " !important")
        .replace("@media(", "@media (")  # !important
        .replace(  # media queries # jpeg > jpg
