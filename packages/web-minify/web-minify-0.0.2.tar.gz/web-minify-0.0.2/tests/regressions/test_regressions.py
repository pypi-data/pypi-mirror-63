...
def test_regressions():
    ...
    for expected_file in expected_files:
        ...
        rel = expected_file[
            expected_file.startswith(expected_path) and len(expected_path) + 1 :
        ]
        ...
        output_content = ""
        with open(output_file, "rb") if output_is_binary else open(
            output_file, "r", encoding="utf-8"
        ) as output_handle:
            output_content = output_handle.read()
        expected_content = ""
        with open(expected_file, "rb") if expected_is_binary else open(
        ...
        if output_content != expected_content:
            ...
            diff = difflib.ndiff(
                output_content.splitlines(1), expected_content.splitlines(1)
            )
