...
def add_timestamp(js):
    return f"{js}\n/* {format(datetime.now().isoformat()[:-7].lower())} */\n"
...
def js_minify(js, settings: dict):
    ...
    """
    if not settings.get("comments", False):
        js = remove_commented_lines(js)
    js = js_minify_keep_comments(js)
    js = add_timestamp(js) if settings.get("timestamp", False) else js
    return js.strip()
