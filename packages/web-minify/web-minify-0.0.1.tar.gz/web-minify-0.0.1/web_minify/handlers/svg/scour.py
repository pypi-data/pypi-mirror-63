...
from __future__ import (
    division,
)  # use "true" division instead of integer division in Python 2 (see PEP 238)
from __future__ import (
    print_function,
)  # use print() as a function in Python 2 (see PEP 3105)
from __future__ import (
    absolute_import,
)  # use absolute imports by default in Python 2 (see PEP 328)
...
def removeUnusedDefs(doc, defElem, elemsToRemove=None, referencedIDs=None):
    ...
    for elem in defElem.childNodes:
        ...
        if elem.nodeType == Node.ELEMENT_NODE and (
            elem.getAttribute("id") == ""
            or elem.getAttribute("id") not in referencedIDs
        ):
...
def unprotected_ids(doc, options):
    ...
    identifiedElements = findElementsWithId(doc.documentElement)
    if not (
        options.protect_ids_noninkscape
        or options.protect_ids_list
        or options.protect_ids_prefix
    ):
            ...
            if (
                ...
                for grandchild in child.childNodes:
                    if (
                        ...
                        and grandchild.namespaceURI == NS["SVG"]
                        and grandchild.nodeName in ["title", "desc"]
                    ):
                        break
                else:
...
default_attributes_per_element = defaultdict(
    list
        ...
        # convert Bézier curve segments into s where possible
            ...
            while i < len(data):
                ...
                if bez_ctl_pt[0] == data[i] and bez_ctl_pt[1] == data[i + 1]:
                    ...
                    newPath.append(
                        ("s", [data[i + 2], data[i + 3], data[i + 4], data[i + 5]])
                    )
                    _num_path_segments_removed += 1
                else:
                    j = 0
...
def scourCoordinates(
    data, options, force_whitespace=False, control_points=[], flags=[]
):
    """
...
def serializeXML(element, options, indent_depth=0, preserveWhitespace=False):
    ...
    attrName2Index = dict(
        [(attrList.item(i).nodeName, i) for i in range(attrList.length)]
    )
    ...
    if children.length == 0:
        ...
        for child in element.childNodes:
            ...
            if child.nodeType == Node.ELEMENT_NODE:
                ...
                if preserveWhitespace or element.nodeName in [
                    "text",
                    "tspan",
                    "tref",
                    "textPath",
...
def scourString(in_string, options=None):
    ...
    if cnt_flowText_el:
        errmsg = (
            "SVG input document uses {} flow text elements, "
    ...
    if options.strip_xml_space_attribute and doc.documentElement.hasAttribute(
        "xml:space"
    ):
    ...
    for child in doc.documentElement.childNodes:
        _num_attributes_removed += moveCommonAttributesToParentGroup(
            child, referencedIds
        )
    ...
    if options.shorten_ids:
        _num_bytes_saved_in_ids += shortenIDs(
            doc, options.shorten_ids_prefix, unprotected_ids(doc, options)
        )
    ...
    _num_attributes_removed += removeDefaultAttributeValues(
        doc.documentElement, options
    )
...
_options_parser.add_option(
    "-p", action="store", type=int, dest="digits", help=optparse.SUPPRESS_HELP
)
...
_option_group_compatibility = optparse.OptionGroup(
    _options_parser, "SVG compatibility checks"
)
...
def parse_args(args=None, ignore_additional_args=False):
    ...
    if rargs:
        ...
        if not ignore_additional_args and rargs:
            _options_parser.error(
                "Additional arguments not handled: %r, see --help" % rargs
            )
    if options.digits < 1:
        _options_parser.error(
    ...
    if options.indent_depth < 0:
        _options_parser.error(
            "Value for --nindent should be positive (or zero), see --help"
        )
    if (
        options.infilename
        and options.outfilename
        and options.infilename == options.outfilename
    ):
...
def sanitizeOptions(options=None):
    optionsDict = dict(
        (key, getattr(options, key)) for key in dir(options) if not key.startswith("__")
    )
...
def getInOut(options):
    if options.infilename:
        ...
        if sys.stdin.isatty():
            _options_parser.error(
                "No input file specified, see --help for detailed usage information"
            )
...
def start(options, input, output):
    ...
    if not (
        (input is sys.stdin)
        or (hasattr(sys.stdin, "buffer") and input is sys.stdin.buffer)
    ):
        input.close()
    if not (
        (output is sys.stdout)
        or (hasattr(sys.stdout, "buffer") and output is sys.stdout.buffer)
    ):
        output.close()
    ...
    if not options.quiet:
        print(
            'Scour processed file "{}" in {} ms: {}/{} bytes new/orig -> {:.1f}%'.format(
