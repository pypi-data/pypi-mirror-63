...
""",
    ...
    _option_group_general = parser.add_argument_group(
        "general", "General options for this program"
    )
    _option_group_general.add_argument(
        "--version", action="version", version=__version__
    )
    _option_group_general.add_argument(
        "--format",
        type=str,
        help="Format string used to generate any output filename. (Dangerous!!)",
        default=defaults.get("format"),
    )
    _option_group_general.add_argument(
        "--overwrite",
        action="store_true",
        help="Allow overwrite files in-place. Default is skip and warn. NOTE: output fils are always overwritten. (Dangerous!!)",
        default=defaults.get("overwrite"),
    )
    _option_group_general.add_argument(
        "--on-change",
        action="store_true",
        help="Allow overwrite files only on source changed (detected by modify time).",
        default=defaults.get("on_change"),
    )
    _option_group_general.add_argument(
        "--verbose",
        action="store_true",
        help="Show output during processing.",
        default=defaults.get("verbose"),
    )
    _option_group_general.add_argument(
        "--zipy",
        action="store_true",
        help="GZIP Processed files as '*.gz'.",
        default=defaults.get("zipy"),
    )
    ...
    _option_group_general.add_argument(
        ...
        dest="disable_js",
        default=False,
        help="Copy .js files verbatim instead of processing",
    )
    ...
    _option_group_general.add_argument(
        ...
        dest="disable_css",
        default=False,
        help="Copy .css files verbatim instead of processing",
    )
    ...
    _option_group_general.add_argument(
        ...
        dest="disable_html",
        default=False,
        help="Copy .html files verbatim instead of processing",
    )
    ...
    _option_group_general.add_argument(
        ...
        dest="disable_svg",
        default=False,
        help="Copy .svg files verbatim instead of processing",
    )
    ...
    _option_group_general.add_argument(
        ...
        dest="disable_png",
        default=False,
        help="Copy .png files verbatim instead of processing",
    )
    ...
    _option_group_general.add_argument(
        ...
        dest="disable_jpeg",
        default=False,
        help="Copy .jpeg files verbatim instead of processing",
    )

    _option_group_general.add_argument(
        "--output",
        type=str,
        help="Path to local output (file or folder).",
    ...
    _option_group_general.add_argument(
        ...
        metavar="input",
        type=str,
        help="Path to local input (file or folder).",
        default=defaults.get("input"),
    )

    _option_group_common = parser.add_argument_group(
        "common", "Options common to many formats"
    )
    _option_group_common.add_argument(
        "--sort",
        action="store_true",
        help="Alphabetically sort CSS Properties (CSS).",
        default=defaults.get("sort"),
    )
    _option_group_common.add_argument(
        "--comments",
        action="store_true",
        help="Keep comments (CSS/HTML).",
        default=defaults.get("comments"),
    )
    _option_group_common.add_argument(
        "--timestamp",
        action="store_true",
        help="Add a timestamp in output files (CSS/HTML/SVG).",
        default=defaults.get("timestamp"),
    )
    _option_group_common.add_argument(
        "--wrap",
        action="store_true",
        help="Wrap output to ~80 chars per line (CSS).",
        default=defaults.get("wrap"),
    )

    _option_group_optimization = parser.add_argument_group(
        "svg optimization", "Optimization options that are only available for SVG"
    )
    _option_group_optimization.add_argument(
        "--set-precision",
        action="store",
        type=int,
        dest="digits",
    ...
    _option_group_document = parser.add_argument_group(
        "svg document", "Document options that are only available for SVG"
    )
    _option_group_document.add_argument(
        "--strip-xml-prolog",
        action="store_true",
    ...
    _option_group_document.add_argument(
        ...
        "(which may contain license or author information etc.)",
    )
    _option_group_document.add_argument(
    ...
    _option_group_document.add_argument(
        ...
        default=False,
        help="changes document width / height to 100pct / 100pct and creates viewbox coordinates",
    )

    _option_group_formatting = parser.add_argument_group(
        "svg output formatting",
        "Output formatting options that are only available for SVG",
    )
    _option_group_formatting.add_argument(
        "--indent",
        action="store",
        type=str,
        dest="indent_type",
        default="space",
        metavar="TYPE",
        help="indentation of the output: none, space, tab (default: space)",
    )
    _option_group_formatting.add_argument(
        "--nindent",
        action="store",
        ...
        default=1,
        metavar="NUM",
        help="depth of the indentation, i.e. number of spaces / tabs: (default: 1)",
    )
    _option_group_formatting.add_argument(
        "--no-line-breaks",
        action="store_false",
    ...
    _option_group_ids = parser.add_argument_group(
        "svg id attributes", "ID attribute options that are only available for SVG"
    )
    _option_group_ids.add_argument(
        "--enable-id-stripping",
        action="store_true",
        dest="strip_ids",
        default=False,
    ...
    _option_group_compatibility = parser.add_argument_group(
        "svg compatability checks",
        "Compatibility check options that are only available for SVG",
    )
    _option_group_compatibility.add_argument(
        "--error-on-flowtext",
        action="store_true",
