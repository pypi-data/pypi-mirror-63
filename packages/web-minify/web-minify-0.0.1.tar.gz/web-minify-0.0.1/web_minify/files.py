...
def generate_file_list(
    ...
    return [
        ...
        for file in files
        if not file.startswith(() if allow_hidden else ".")
        and not file.endswith(omit)
...
def path_is_in_path(path_a, path_b):
    if not path_a or not path_b:
        return False
    return os.path.abspath(path_a) == os.path.commonpath(
        [os.path.abspath(path_a), os.path.abspath(path_b)]
    )
...
def determine_file_extension(file_path: str):
    ...
    if not file_ending:
        return None
    return file_endings[file_ending] or None
