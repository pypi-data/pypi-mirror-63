...
try:
    ...
    def tabulate(nptab, columns=None, tablefmt=None):     # pylint: disable=unused-argument
        """Alt tabulate
        """
        outp = ""
        outp += " ".join(columns) + "\n"
...
T = transpose
"""Convenience alias for :mod:`nptab.transpose`."""
...
class Nptab(HashJoinMixin):
    ...
    """
    max_repr_rows = 20
    """int: Maximum number of rows to show when :func:`~nptab.Nptab.__repr__` is invoked."""
    repr_layout = 'presto'
    """string: The layout used with `tabulate` in the :func:`~nptab.Nptab.__repr__` method."""
    ...
    def __init__(self, datastruct=None, columns=None, copy=True):
        ...
        """list :
            ...
            checked with the property :func:`~nptab.Nptab.valid`.
            """
        self.columns = list()
        """list :
            ...
            can be checked with the method :func:`~nptab.Nptab.valid`.
            """
    ...
    def append(self, nptab):
        ...
        elif isinstance(nptab, Nptab):
            assert set(self.columns) == set(nptab.columns), \
                "Not the same columns in Nptab: {} {}".format(self.columns, nptab.columns)
            for col, dta in zip(nptab.columns, nptab.data):
                ci = self.columns.index(col)
                self.data[ci] = np.concatenate([self.data[ci], dta])

        elif isinstance(nptab, pd.DataFrame):
            assert set(self.columns) == set(nptab), \
                "Not the same columns in Nptab: {} {}".format(self.columns, nptab.keys())
    ...
    def __repr__(self):
        ...
        """
        nptab = tabulate([[c[r] for c in self.data]
