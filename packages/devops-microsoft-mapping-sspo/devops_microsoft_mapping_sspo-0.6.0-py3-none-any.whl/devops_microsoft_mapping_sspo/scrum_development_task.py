...
class ScrumDevelopmentTask(BaseEntity):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.scrum_development_task = None
        self.scrum_development_application = None
        self.element = None
    ...
    def create(self):
        try:
            ...
            logging.info('Scrum development Task: Name and Description: '+self.scrum_development_task.name)
    ...
    def set_name_description(self):
        self.scrum_development_task.name = self.element.fields['System.Title']
        self.scrum_development_task.description = str(self.check_value(self.element,'System.Description'))
    ...
    def retrive_team_members(self):
        try:
            ...
            if created_by is not None and created_by is not 'None':
                ...
                self.scrum_development_task.created_by = team_member.id
            ...
            if assigned_by is not None and assigned_by is not 'None':
                ...
                if team_member is not None:
                    self.scrum_development_task.assigned_by = [team_member]
    ...
    def retrive_dates(self):
        ...
        if created_data is not None and created_data is not 'None':
            self.scrum_development_task.created_date = self.validate_date_format(str(created_data)) 
                            
        if activated_date is not None and activated_date is not 'None':
            self.scrum_development_task.activated_date = self.validate_date_format(str(activated_date))
                    
        if resolved_date is not None and resolved_date is not 'None':
            self.scrum_development_task.resolved_date = self.validate_date_format(str(resolved_date)) 
                    
        if closed_date is not None and closed_date is not 'None':
            self.scrum_development_task.closed_date = self.validate_date_format(str(closed_date))
    ...
    def retrive_sprint(self):
        try:
            ...
            if sprint_backlog is not None:
                self.scrum_development_task.sprint_backlogs = [sprint_backlog]
            else:
            ...
            if sprints is not None:
                self.scrum_development_task.sprints = [sprints]
            else:
                ...
                for relation in relations:
                    ...
                    if relation_name == "Parent":
                        ...
                        if atomic_user_story:
                            self.scrum_development_task.atomic_user_story = atomic_user_story.id
