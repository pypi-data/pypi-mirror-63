...
from .extract_intensities import get_intensities, get_time_axis
...
class XPCS(object):
    ...
    def get_two_time_corr_function(self, mode: str = 'std',
                                   delta_time: int = 1) -> Tuple[np.ndarray, np.ndarray]:
        if not self.status:
        ...
        intensity, time_array = self.get_intensities(delta_time=delta_time)
        return calculate_2d_corr(intensity, mode), time_array

    def get_one_time_corr_function(self, delta_time: int = 1) -> Tuple[np.ndarray, np.ndarray]:
        if not self.status:
            raise ValueError('Cut region should be provided.')
        intensity, time_array = self.get_intensities(delta_time=delta_time)
        return calculate_1d_corr(intensity), time_array

    def get_intensities(self, **kwargs) -> Tuple[np.ndarray, np.ndarray]:
        return get_intensities(self.h5_filepath, mask=self.mask, group_key=self.h5_path, **kwargs)

    def get_time_axis(self, **kwargs) -> np.ndarray:
        return get_time_axis(self.h5_filepath, group_key=self.h5_path, **kwargs)
