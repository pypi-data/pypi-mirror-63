...
class XPCSMap(object):
    ...
    def get_time_axis(self, delta_time: int = 1):
        return self.xpcs.get_time_axis(delta_time=delta_time)

    def set_name(self, name: str) -> None:
        self.name = name

    def generate_2d_corr(self, mode: str = 'std', delta_time: int = 1) -> GeneratorType:
        for region in self.region_grid:
            self.xpcs.set_region(region)
            corr, time_array = self.xpcs.get_two_time_corr_function(mode, delta_time=delta_time)
            yield corr, region

    def generate_1d_corr(self) -> GeneratorType:
        for region in self.region_grid:
            self.xpcs.set_region(region)
            corr, time_array = self.xpcs.get_one_time_corr_function()
            yield corr, region

    def get_intensity_cuts(self, sum_along_axis: int = 1, delta_time: int = 1) -> Tuple[np.ndarray, np.ndarray]:
        common_region = self.region_grid.get_common_area()
        self.xpcs.set_region(common_region)
        return self.xpcs.get_intensities(preserve_shape=True,
    ...
    def save_2d_corr(self, mode: str = 'std', delta_time: int = 1) -> None:
        _save_to_h5(self.name, self.folder_num,
                    self.generate_2d_corr(mode, delta_time=delta_time), self.parameters)
