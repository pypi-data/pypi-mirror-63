...
class BaseSearcher(object):
    ...
    def search_by_multi_bool(self, must=[], must_not=[], should=[], sort_field='', offset=0, limit=1000):
        search = self.doc_model.search()
        should_match = 1 if should else 0
        search = search.query(Q('bool', must=must, must_not=must_not, should=should, minimum_should_match=should_match))
        if sort_field:
