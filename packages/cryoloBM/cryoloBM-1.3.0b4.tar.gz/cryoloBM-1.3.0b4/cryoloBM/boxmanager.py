...
class MainWindow(QtG.QMainWindow):
    ...
    def write_box_files(self):
        ...
        for filename, rectangles in self.box_dictionary.items():
            ...
            if pd.wasCanceled():
                ...
                pd.show()
                pd.setValue(int((counter + 1) * 100 / len(self.box_dictionary.items())))
    ...
    def _import_boxes(self, box_dir, keep=False):
        ...
        #all_file_paths = [os.path.join(box_dir, file) for file in onlyfiles]

        '''
        all_helicon_files = [CoordsIO.is_eman1_helicon(path) for path in all_file_paths]

        all_eman1_start_end_files = [
            True
            if not all_helicon_files[path_index]
            and CoordsIO.is_eman1_filament_start_end(path)
            else False
            for path_index, path in enumerate(all_file_paths)
        ]

        all_star_files = [
            True
            if not all_helicon_files[path_index]
            and not all_eman1_start_end_files[path_index]
            and path.endswith(".star")
            else False
            for path_index, path in enumerate(all_file_paths)
        ]
        
        all_cbox_files = [
            (os.path.splitext(os.path.basename(path))[0], CoordsIO.read_cbox_boxfile(path),rand_color)
            for path in all_file_paths
            if path.endswith(".cbox")
        ]
        print("Total time B", t.time() - t_start)
        for ident_rect_tuble in all_cbox_files:
            self.is_cbox = True
            if ident_rect_tuble[0] in self.box_dictionary:
                self.box_dictionary[ident_rect_tuble[0]].extend(ident_rect_tuble[1])
            else:
                self.box_dictionary[ident_rect_tuble[0]] = ident_rect_tuble[1]
            box_imported = box_imported + len(ident_rect_tuble[1])
