...
class Client:
    ...
    def __init__(self, url="http://0.0.0.0:5000", username=None, password=None):
    ...
    def available_datasets(self):
        request = requests.get(f'{self.base_url}/datasets', cookies=self.cookies)
    ...
    def fetch_dataset(self,id):
        ...
        """
        try:
            request = requests.get(f'{self.base_url}/datasets/{id}', cookies=self.cookies)
    ...
    def fit_v1(self,keras_model_def, dataset, non_DP_training=False):
        ...
        request = requests.post(f'{self.base_url}/fit_v1',
            ...
            headers={'Content-Type': 'application/octet-stream'}, cookies=self.cookies)
    ...
    def training_status(self, id):
        ...
        request = requests.get(f'{self.base_url}/tasks/{id}', cookies=self.cookies)
        return request.json()
    ...
    def fetch_model(self, id):
        ...
        """
        response = requests.get(f'{self.base_url}/models/{id}', cookies=self.cookies)
