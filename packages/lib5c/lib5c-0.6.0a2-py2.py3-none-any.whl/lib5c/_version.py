...
The version naming scheme is specified by defining functions
``version_scheme()`` and ``local_scheme()`` in this module. We have written
these so that they mimic versioneer's "pep440" style. These functions are used
in this module, and they are also imported in ``setup.py`` at install time.
...
otherwise. This is primarily useful for Docker image building (where it doesn't
make sense to make the host install our package just to obtain the version
information to pass to the image build process) and for editable installs (where
having setuptools_scm installed is the only way to get accurate version
information).
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                version = importlib_version(__name__)
