...
class PretrainedModelMixin(ABC):
	...
	def load_for_finetune(self, weights, n_classes, headless=False, **kwargs):
		"""
		...
		"""
		self.load(weights, headless=headless)
		self.reinitialize_clf(n_classes, **kwargs)

	def load_for_inference(self, weights, n_classes, headless=False, **kwargs):
		"""
	...
	def load(self, weights, strict=False, headless=False):
		if weights not in [None, "auto"]:
			...
			npz.load_npz(weights, self, strict=strict, ignore_names=ignore_names)
