...
""" TagDetailView powered by taggit. """
class TagDetailView(DetailView):
    template_name = "djangoadmin/djangoarticle/tag_detail_view.html"
    model = Tag
    ...
    def get_context_data(self, **kwargs):
        ...
        instance = self.kwargs['tag_slug']
        context['article_filter'] = ArticleModelScheme.objects.filter_publish().filter(tags__slug=instance).filter(is_promote=False)
        context['is_promoted'] = ArticleModelScheme.objects.filter_promoted().filter(tags__slug=instance)
        context['is_trending'] = ArticleModelScheme.objects.filter_trending()
        context['promo'] = ArticleModelScheme.objects.by_promo().filter(tags__slug=instance)
        return context
