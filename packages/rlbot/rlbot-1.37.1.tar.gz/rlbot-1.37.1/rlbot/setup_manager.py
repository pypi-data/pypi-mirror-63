...
class ROCKET_LEAGUE_PROCESS_INFO:
    ...
    # We were warned that the new 64 bit Rocket League exe might not work well with RLBot.
    # Per a recommendation from Psyonix, we are passing the -dx9 flag for now to avoid using that new exe.
    # TODO: stop passing -dx9 if/when we confirm it's safe to use the new 64 bit exe.
    IDEAL_ARGS = ['-rlbot', f'RLBot_ControllerURL=127.0.0.1:{PORT_PLACEHOLDER}', '-nomovie', '-dx9']
