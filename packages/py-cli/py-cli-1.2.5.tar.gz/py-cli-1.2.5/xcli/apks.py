...
def gradle_release(project_path, app_module):
    """
    执行gradle脚本
    ...
    # 判断是否存在'resguardRelease'
    gradle_command = "./gradlew tasks | grep -q \'resguardRelease\'"
    result = subprocess.call(gradle_command, shell=True, cwd=project_path)
    gradle_task = 'assembleRelease' if result else 'resguardRelease'

    gradle_command = "./gradlew -q -p %s clean %s" % (app_module, gradle_task)
    print('[INFO]: %s: %s' % (gradle_command, project_path))
    subprocess.run(gradle_command, shell=True, cwd=project_path)
