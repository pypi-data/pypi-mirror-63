...
class BlueprintDoesNotExistVisitor(BaseBlueprintVisitor):
    ...
    def _is_register_blueprint(self, call_node: ast.Call) -> bool:
        ...
        is_register_blueprint = call_node.func.attr == "register_blueprint"

        if not is_register_blueprint:
            return False

        # Else, infer Flask app name
        if not self._is_caller_flask_app(call_node.func):
            logger.debug("Flask app not identified. Determining if this is Flask...")
            if not MODULE_NAME in self.modules:
                logger.debug("Flask not imported, so this probably isn't Flask.")
                return False
            try:
                self.flask_app_name = call_node.func.value.id
                logger.debug(f"Inferring Flask app name: {self.flask_app_name}")
            except Exception:
                logger.debug("Could not infer Flask app name.")
                return False
    ...
    def _resolve_blueprint_file(self, blueprint_variable_name: str) -> List[str]:
        ...
        for module in self.modules:
            if blueprint_variable_name in self.methods[module]:
                ...
                if os.path.exists(guessed_file):
                    guessed_files.append(guessed_file)

                # If not, try again with full path 
                guessed_file = os.path.join(
                    self.base_path,
                    *module.split(".")[:-1],
                    module.split(".")[-1] + ".py",
                ...
                if os.path.exists(guessed_file):
                    ...
                    guessed_files.extend([os.path.join(module_dir, f) for f in module_files])

                # Aaaand finally, try with full path + __init__
                guessed_file = os.path.join(
                    self.base_path, *module.split("."), "__init__.py"
                )
                if os.path.exists(guessed_file):
                    # Assume that the blueprint definition is spread out in the module.
                    module_dir = os.path.dirname(guessed_file)
        ...
        logger.debug(f"Looking for blueprint '{blueprint_variable_name}' in {guessed_files}")
        if not guessed_files:
            logger.info(
                f"Could not find the file where the blueprint '{blueprint_variable_name}' is defined."
            )
        return guessed_files
    ...
    def _run_blueprint_visitor(self, blueprint_file_paths: List[str]):
        ...
        for blueprint_file_name in blueprint_file_paths:
            blueprint_visitor = BlueprintVisitor(blueprint_file_name)
