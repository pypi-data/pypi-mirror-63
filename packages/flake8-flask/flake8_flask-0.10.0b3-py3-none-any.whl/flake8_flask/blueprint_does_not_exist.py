...
class BlueprintDoesNotExistVisitor(BaseBlueprintVisitor):
    ...
    def _generate_message(self, url_for_lookup: UrlForLookup, extra=False) -> str:
        ...
        """
        message = f"{self.name} Invalid url_for blueprint: '{url_for_lookup.node.s}'. This will cause a runtime error upon lookup. Check to make sure everything is spelled correctly."
        if extra:
