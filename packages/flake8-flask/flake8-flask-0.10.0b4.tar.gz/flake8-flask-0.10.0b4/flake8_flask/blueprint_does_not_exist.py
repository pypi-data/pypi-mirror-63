...
class BlueprintDoesNotExistVisitor(BaseBlueprintVisitor):
    ...
    def _run_blueprint_visitor(self, blueprint_file_paths: List[str]):
        ...
        for blueprint_file_name in blueprint_file_paths:
            ...
            logger.debug(
                f"After running BlueprintVisitor: self.url_for_lookups: {[n.node.s for n in self.url_for_lookups]}; self.blueprints: {self.blueprints}"
            )
    ...
    def _generate_message(self, url_for_lookup: UrlForLookup, extra=False) -> str:
        ...
        """
        message = f"{self.name} Invalid url_for blueprint: '{url_for_lookup.node.s}' found in file '{self.filename}'. This will cause a runtime error upon lookup. Check to make sure everything is spelled correctly."
        if extra:
