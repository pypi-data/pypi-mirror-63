...
# logging
log = logging.getLogger('kitir.utils.mail')
...
def send_email(recipients, subject, text, attachments=None, message_callback=None, **kwargs):
    ...
    if attachments:
        ...
        for fname in attachments:
            ...
            try:
                ...
                log.warning('Exception preparing attachment: file={} exc={}'.format(fname, exc.message), exc_info=True)
            else:
    ...
    while retries > 0:
        ...
        try:
            ...
            log.error('Exception handshaking to SMTP server: exc={} trace...'.format(exc), exc_info=True)
            continue
        else:
            ...
            try:
                try:
                    ...
                    if 'size limits' in exc.smtp_error:
                        log.warning(
                            'Exception sending mail due to size limits - sending without attachments: {}'.format(
