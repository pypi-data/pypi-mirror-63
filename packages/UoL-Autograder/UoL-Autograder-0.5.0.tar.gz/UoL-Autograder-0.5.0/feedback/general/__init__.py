    ...
    def replace_files(self, config):
        ...
        for path in file_paths:
            ...
            if target_path.is_file():
                if self.verbose: print(f"Found file {path.name} in tmp directory, deleting")
                target_path.unlink()
            shutil.copyfile(path, target_path)
            if self.verbose: print(f"Copied {path.name} to {target_path}")
