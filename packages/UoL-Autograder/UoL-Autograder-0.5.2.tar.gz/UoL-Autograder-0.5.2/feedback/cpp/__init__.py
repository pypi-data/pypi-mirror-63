...
class CppRunner(Runner):
    ...
    def test_functionality(self, config):
        ...
        print([executable, output_file.absolute().as_posix()])

        _, stdout, stderr = util.execute([executable, output_file.absolute().as_posix()], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT)
    ...
    def test_executable_functionality(self, config):
        ...
        _, stdout, stderr = util.execute([PY_RUNNER, config.tester_file.absolute().as_posix(), executable, output_file.absolute().as_posix(), get_eval_util_path()], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT)
        self.print_verbose(f"stdout:\n{stdout}\nstderr:\n{stderr}")
    ...
    def _get_executable_standalone(self, tested_path):
        ...
        if not compiled:
            self.print_verbose(f"Failed to link {tested_o}\n{output}")
            return None
