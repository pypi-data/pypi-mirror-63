...
import hashlib
import requests
def down_content(url,outfile,downloader):
    if outfile.exists():
        print(outfile,'Exists')
        return
    else:
        print('downloading',url,outfile.name)
...
def download_m3u8_requeset(url,out_name,delete_m3u8,suffix,multi_requests):
    ...
    pwd = os.getcwd()
    os.chdir(m3u8_dir)
    all_tasks = [pool.submit(down_content,url_dir + i,Path(i),session) for i in files]
...
@click.option('--delete','-D',default=False,type=click.BOOL,help='是否删除源文件,针对url下载项使用')
@click.option('--downloader',default='requests',type=click.Choice(['requests','ffmpeg']),help='m3u8 下载器选择(默认requests)')
@click.option('--multi_requests',default=4,help='requests 下载单个url的线程数')
def main(dirname=None,url=None,suffix='.ts',output=None,multi=1,delete=False,downloader='requests',multi_requests=4):
    suffix = '*'+suffix
    if url:
        if Path(url).is_file():
            url = open(url).read()
