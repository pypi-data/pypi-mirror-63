...
import click
import hashlib
def download_m3u8(url,out_name,delete_m3u8,suffix):
...
@click.option('--multi','-m',default=1,help='下载m3u8时是否采用多线程')
@click.option('--delete','-D',default=False,help='是否删除源文件,针对url下载项使用')
def main(dirname=None,url=None,suffix='.ts',output=None,multi=1,delete=False):
    ...
    if url:
        ...
        for i,s in enumerate(url.strip().split('\n')):
            ...
            if po is not None:
                po.submit(download_m3u8,url,out_name,delete,suffix)
            else:
                download_m3u8(url,out_name,delete,suffix)
