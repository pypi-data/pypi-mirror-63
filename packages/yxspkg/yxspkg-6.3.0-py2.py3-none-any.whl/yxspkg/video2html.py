...
def generate_poster(dirname,ts=2):
        ...
        if suffix in video_set:
            ...
            if not is_first:
                ...
                if not post_dir.exists():
                    os.mkdir(post_dir)
            try:
                # if vfile.name.find(' ') != -1:
                #     tvfile = vfile.with_name(vfile.name.replace(' ',''))
                #     os.rename(vfile,tvfile)
                #     vfile = tvfile
                print(vfile,ts)
                t = imageio.read(vfile)
                nn = t.count_frames()
                frame = t.get_data(min(int(ts * 1800),int(nn/5)))
...
def generate_poster_all(dirname):
    for i,v in enumerate(sys.argv):
        if v=='-t':
            ts = float(sys.argv[i+1])
            break 
    else:
        ts = 2
    for root,_,_ in os.walk(dirname):
        generate_poster(root,ts)
    generate_poster(dirname,ts)
