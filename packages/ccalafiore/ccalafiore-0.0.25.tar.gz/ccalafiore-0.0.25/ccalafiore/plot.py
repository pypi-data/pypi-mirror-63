...
def heat_maps(values, id_figures=None, cmap=None, shape_per_figure=None,
    ...
    if shape_per_figure is None:
        rows = values.shape[1]
        colomns = 1

    else:
        rows = shape_per_figure.shape[0]
        colomns = shape_per_figure.shape[1]
...
def save(id_figures=None, directories=None, frm=['svg', 'png']):
    ...
    for i in range(n_figures):
        ...
        for j in range(n_formats):
            ...
            directory_i_j = '.'.join([directories[i], frm[j]])
