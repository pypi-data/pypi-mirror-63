...
class RequestFull():
    ...
    def validate(self, url=None, paginate_type=None, auth=None, object_name=None, params=None):
        ...
        if  ( paginate_type == 3
        and ( "pageField" not in params or "pageLenField" not in params or "total" not in params)):
            return { "error": "params inválido. Informar um objeto params com as variáveis de controle no retorno da API" }

        if  ( paginate_type == 3
        and ( params["pageField"] == None or params["pageLenField"] == None or params["total"] == None )):
            return { "error": "params inválido. Informar um objeto params com as variáveis de controle no retorno da API" }
