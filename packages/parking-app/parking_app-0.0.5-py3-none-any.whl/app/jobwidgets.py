...
class JobAttributesWidget(QWidget):
    ...
    def saveJob(self):
        ...
        asnc = getAsync()
        if  'created' in self.job:
            asnc.jobEdit(self.jobOperationDone, self.job['id'], json.dumps(self.job))
        else:
            asnc.jobAdd(self.jobOperationDone, json.dumps(self.job))
    ...
    def deleteJob(self):
        ...
        if ret == QMessageBox.No:
            return
        asnc = getAsync()
        asnc.jobDelete(self.jobDeleteDone, self.job['id'])
    ...
    def startStopJob(self):
        if self.job and 'status' in self.job and 'created' in self.job:
            ...
            asnc = getAsync()
            asnc.jobStartStop(self.jobOperationDone, self.job['id'], operation)
...
class JobWidget(QWidget):
    ...
    def loadLastJobImage(self):
        ...
        asnc = getAsync()
        asnc.loadLastJobImage( self.lastJobImageLoaded, job['id'])
...
class JobLogsWidget(QWidget):
    ...
    def loadLogs(self, vals):
        if not self.job or not 'created' in self.job:
            return
        asnc = getAsync()
        asnc.loadJobLogs(self.logsInfo, self.job['id'], 'INFO', vals)
        asnc.loadJobLogs(self.logsError, self.job['id'], 'ERROR', vals)
        asnc.loadJobLogs(self.logsDebug, self.job['id'], 'DEBUG', vals)
...
class JobOverviewWidget(QWidget):
    ...
    def loadStats(self, prec, vals):
        ...
        if not job or not 'created' in job:
            return
        asnc = getAsync()
        asnc.loadJobStats(self.statsLoaded, job['id'], prec, vals)
