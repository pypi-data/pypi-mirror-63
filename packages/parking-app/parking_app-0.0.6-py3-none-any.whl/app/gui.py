...
from app.asnc import initAsync, getAsync
...
class TopWidget(QWidget):
    def __init__(self, parent):
        ...
        title = QLabel('Cloud storage keyfile')
        self.layout.addWidget(title)
        self.keyfile =QLabel(get_key_file())
        self.layout.addWidget(self.keyfile)
        keyfile_button = QPushButton('Select')
        keyfile_button.clicked.connect(self.keyfile_select)
        self.layout.addWidget(keyfile_button)
    ...
    def keyfile_select(self):
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getOpenFileName(self, "Select account keyfile", "",
                                                  "All Files (*);;Json Files (*.json)", options=options)
        if fileName:
            self.keyfile.setText(fileName)
            sets = get_settings()
            sets[GCLOUD_KEYFILE] = fileName
            save_settings(sets)
            self.parentWidget().reloadUI()
