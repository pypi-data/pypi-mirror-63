...
def only(type_, lenth = 800, name = '小明'):
    ...
    if type_ == 'famous_quotes':
        ...
        achievement = json.load(open(dirs + '//wc_data//bullshit_achievement.json', 'r', encoding = 'gb2312'))
        school = json.load(open(dirs + '//wc_data//bullshit_school.json', 'r', encoding = 'gb2312'))
        job = json.load(open(dirs + '//wc_data//bullshit_job.json', 'r', encoding = 'gb2312'))
        body += name + ',' + random.choice(place) + \
                '省' + random.choice(place) + '市人。\n' \
                + str(random.randint(9, 12)) + '岁以' + \
                random.choice(achievement) + '的好成绩' + \
                '顺利考入' + random.choice(school) + \
                random.choice(job) + '学院。\n' + \
