...
class CustomUserAdmin(ImageCroppingMixin, BaseUserAdmin):
    ...
    readonly_fields = ['email', 'last_login', 'date_joined',
                       'editByGoogle', 'google_isPlusUser', 'google_plusUrl', 'google_circledByCount',
                       'google_language', 'google_kind', 'google_verified']
    # form = UserChangeForm
    # add_form = UserCreationForm
    list_display = ('email', 'username', 'gender', 'first_name', 'last_name', 'is_active', 'manually_edited',
                    'visible', 'date_joined', 'last_login')
...
class ProxyRefreshToken(RefreshToken):
    ...
    class Meta:
        proxy = True
        verbose_name = "Token: " + str(RefreshToken._meta.verbose_name)
        verbose_name_plural = "Token: " + str(RefreshToken._meta.verbose_name_plural)
