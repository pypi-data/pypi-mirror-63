...
def import_auth():
 try:
  ...
  raise mqKDY('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=mqKDp)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=mqKDj)
def get_params(args,names=[],mandatory=mqKDp):
 result=()
 for name in names:
  value=mqKDx
  if name in args:
  ...
  if not value and mandatory:
   raise mqKDY('Please provide %s=... parameter'%name)
  result+=(value)
 if mqKDC(result)==1:
  return result[0]
 return result
