...
class AttrDict(psoab):
 def __init__(self,*args,**kwargs):
  psoaB(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and psoae(socket,'IPV6_V6ONLY'):
...
class Half(psoaQ):
 def __init__(self,opt,sock,addr,psoaE):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.psoaE=psoaE
  self.name='peer' if self.psoaE else 'client'
  self.queue=[]
  self.dest=psoaV
  self.err=psoaV
  self.ready=psoaW
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':psoaW,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
