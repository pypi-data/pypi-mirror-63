...
class AttrDict(eQbLY):
 def __init__(self,*args,**kwargs):
  eQbLW(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and eQbLD(socket,'IPV6_V6ONLY'):
...
class Half(eQbLo):
 def __init__(self,opt,sock,addr,eQbLp):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.eQbLp=eQbLp
  self.name='peer' if self.eQbLp else 'client'
  self.queue=[]
  self.dest=eQbLI
  self.err=eQbLI
  self.ready=eQbLB
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':eQbLB,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
