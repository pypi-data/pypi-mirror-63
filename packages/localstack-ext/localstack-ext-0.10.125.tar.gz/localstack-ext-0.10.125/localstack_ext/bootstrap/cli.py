...
def import_auth():
 try:
  ...
  raise TIKsr('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=TIKsg)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=TIKsW)
def get_params(args,names=[],mandatory=TIKsg):
 result=()
 for name in names:
  value=TIKsM
  if name in args:
  ...
  if not value and mandatory:
   raise TIKsr('Please provide %s=... parameter'%name)
  result+=(value)
 if TIKsA(result)==1:
  return result[0]
 return result
