...
def import_auth():
 try:
  ...
  raise EYCXB('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=EYCXU)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=EYCXn)
def get_params(args,names=[],mandatory=EYCXU):
 result=()
 for name in names:
  value=EYCXi
  if name in args:
  ...
  if not value and mandatory:
   raise EYCXB('Please provide %s=... parameter'%name)
  result+=(value)
 if EYCXz(result)==1:
  return result[0]
 return result
