...
class AttrDict(lHiOX):
 def __init__(self,*args,**kwargs):
  lHiOw(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and lHiOr(socket,'IPV6_V6ONLY'):
...
class Half(lHiOh):
 def __init__(self,opt,sock,addr,lHiOp):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.lHiOp=lHiOp
  self.name='peer' if self.lHiOp else 'client'
  self.queue=[]
  self.dest=lHiOF
  self.err=lHiOF
  self.ready=lHiOQ
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':lHiOQ,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
