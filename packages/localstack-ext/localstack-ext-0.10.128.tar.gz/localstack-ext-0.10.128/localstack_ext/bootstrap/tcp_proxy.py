...
class AttrDict(fNslP):
 def __init__(self,*args,**kwargs):
  fNslS(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and fNslj(socket,'IPV6_V6ONLY'):
...
class Half(fNslq):
 def __init__(self,opt,sock,addr,fNslT):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.fNslT=fNslT
  self.name='peer' if self.fNslT else 'client'
  self.queue=[]
  self.dest=fNslg
  self.err=fNslg
  self.ready=fNslc
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':fNslc,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
