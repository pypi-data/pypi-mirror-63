...
def import_auth():
 try:
  ...
  raise MToaL('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=MToal)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=MToaX)
def get_params(args,names=[],mandatory=MToal):
 result=()
 for name in names:
  value=MToaV
  if name in args:
  ...
  if not value and mandatory:
   raise MToaL('Please provide %s=... parameter'%name)
  result+=(value)
 if MToar(result)==1:
  return result[0]
 return result
