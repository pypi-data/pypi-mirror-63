...
def import_auth():
 try:
  ...
  raise uXhoY('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=uXhoL)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=uXhok)
def get_params(args,names=[],mandatory=uXhoL):
 result=()
 for name in names:
  value=uXhoD
  if name in args:
  ...
  if not value and mandatory:
   raise uXhoY('Please provide %s=... parameter'%name)
  result+=(value)
 if uXhoe(result)==1:
  return result[0]
 return result
