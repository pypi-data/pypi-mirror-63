...
class AttrDict(evNms):
 def __init__(self,*args,**kwargs):
  evNma(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and evNmX(socket,'IPV6_V6ONLY'):
...
class Half(evNmz):
 def __init__(self,opt,sock,addr,evNmn):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.evNmn=evNmn
  self.name='peer' if self.evNmn else 'client'
  self.queue=[]
  self.dest=evNml
  self.err=evNml
  self.ready=evNmo
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':evNmo,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
