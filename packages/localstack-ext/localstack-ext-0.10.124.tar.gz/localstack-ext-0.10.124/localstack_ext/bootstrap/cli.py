...
def import_auth():
 try:
  ...
  raise jNxug('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=jNxuG)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=jNxuU)
def get_params(args,names=[],mandatory=jNxuG):
 result=()
 for name in names:
  value=jNxuH
  if name in args:
  ...
  if not value and mandatory:
   raise jNxug('Please provide %s=... parameter'%name)
  result+=(value)
 if jNxuP(result)==1:
  return result[0]
 return result
