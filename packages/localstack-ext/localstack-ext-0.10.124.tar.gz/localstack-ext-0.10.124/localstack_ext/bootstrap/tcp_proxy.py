...
class AttrDict(gYzmy):
 def __init__(self,*args,**kwargs):
  gYzmn(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and gYzmG(socket,'IPV6_V6ONLY'):
...
class Half(gYzmL):
 def __init__(self,opt,sock,addr,gYzmj):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.gYzmj=gYzmj
  self.name='peer' if self.gYzmj else 'client'
  self.queue=[]
  self.dest=gYzmU
  self.err=gYzmU
  self.ready=gYzmT
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':gYzmT,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
