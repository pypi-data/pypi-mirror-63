...
class AtomicUserStory(BaseEntity):
    ...
    def do(self,data):
        ...
        try:
            ...
            type = content['resource']['fields']['System.WorkItemType']
            if type != 'User Story' and  type != 'Product Backlog Item':
                return None
    ...
    def __create(self, work_item):
        ...
        atomic_user_story_mapping = factories.AtomicUserStoryFactory()
        atomic_user_story = atomic_user_story_mapping.create(work_item)
        
        self.create_application_reference(
            work_item.id, 
            work_item.url, 
            "WorkItem",
            atomic_user_story.uuid, 
            atomic_user_story.__tablename__)
