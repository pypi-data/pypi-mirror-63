...
sys.path.append("/".join(sys.path[-1].split("/")[:-1]))
...
class Config:
    ...
    def logging_path(self) -> str:
        if self._path_suffix is None:
            self._path_suffix = ""
        return os.path.join(self._logs_dir, self._path_suffix)
    ...
    def set_attributes_from_path(self, config_path: str) -> None:
        ...
        assert "version" in version and version.split("_")[-1].isdigit(), (
        ...
        self._path_suffix = os.path.join(
            f"version_{self._version}", self._language, self._task_name, self._experiment_name
        )
