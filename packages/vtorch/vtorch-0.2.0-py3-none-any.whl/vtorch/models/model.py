...
class Model(torch.nn.Module):  # type: ignore
    ...
    def load(
        ...
        **kwargs: Any,
        ...
        transformer_config_path = os.path.join(serialization_dir, "config.json")
        transformer_config = getattr(transformers, model_params.transformer_config_type).from_json_file(
            transformer_config_path
        )

        model = cls(
            getattr(transformers, model_params.transformer_model_type)(transformer_config),
            vocab=vocab,
    ...
    def from_config(cls: Type[T], config: ModelParams, vocab: Vocabulary, cuda_device: int = -1) -> T:
        model = cls(
            getattr(transformers, config.transformer_model_type).from_pretrained(config.transformer_model_name),
            vocab=vocab,
            model_params=config,
        )
        cls._model_to_device(model, cuda_device)
