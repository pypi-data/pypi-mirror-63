...
class SubrequestClient(RequestLoggerClient, ErrorMatcher):
    ...
    async def _fetch__retries_logic(self, request: HttpSubrequest, *, attempts, exponential_retries, intercept_connection_errors, retry_timeout):
        ...
        for i in revrange(attempts):
            ...
            try:
                ...
                request.logger.develop(f"Request finished with status code {response.code} (is last attempt: {_last_attempt}; intercept connection errors: {_intercept_connection_errors}, attempt: {attempts - i} of {attempts})")
                break
        return response
    ...
    async def _fetch_json__parse_json(self, response: HttpSubrequestResponse, json_load_options: Dict[str, Any]) -> JsonSerializable:
        ...
        try:
            ...
            raise SubrequestFailedError(message=f"Cannot decode JSON from the response: '{response.body}'", server_response=str(response.body), error_code=SubrequestFailedErrorCodes.InvalidResponseBody, response=response)
        except Exception as e:
            raise SubrequestFailedError(code=500, error="Unknown Error", message=f"Unknown error: {e}", response=response)
        else:
            return resp_json
    async def _fetch_json__extract_error(self, extracted_json: JsonSerializable) -> Optional[str]:
        if (isinstance(extracted_json, dict)):
            return extracted_json.get('reason') or extracted_json.get('error') or extracted_json.get('message')
    ...
    async def fetch_json(self, request: Union[str, HTTPRequest], *, check_content_type_header: bool = True, json_load_options: Dict[str, Any] = None, **kwargs) -> Tuple[HttpSubrequestResponse, JsonSerializable]:
        ...
        try:
            ...
            try:
                ...
                error_message = await self._fetch_json__extract_error(resp_json)
            
            raise SubrequestFailedError(response, base_error=e, message=error_message, error_code=SubrequestFailedErrorCodes.InvalidResponseBody)
