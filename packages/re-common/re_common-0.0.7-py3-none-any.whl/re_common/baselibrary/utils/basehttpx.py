...
class BaseHttpx(object):
    ...
    def base_sn_httpx(self, url, proxies, headers, endstring="", marks=[], **kwargs):
        if headers == "":
            self.headers['User-Agent'] = USER_AGENT
        else:
            self.headers = headers
        if proxies != "":
            self.proxies = set_proxy_httpx(proxies)
            sn = httpx.Client(proxies=self.proxies, headers=self.headers)
        else:
            sn = httpx.Client(headers=headers)
        r = None
        exMsg = None
        try:
    ...
    def base_sn_post_httpx(self, url, proxies, headers, data=None, endstring="", marks=[], **kwargs):
        if headers == "":
            self.headers['User-Agent'] = USER_AGENT
        else:
            self.headers = headers
