...
class BaseDir(object):
    ...
    def get_dir_size(cls, dirpath):
        ...
        for filepath in cls.get_dir_all_files(dirpath):
            size += sum(cls.get_file_size(filepath))
        return size
    ...
    def copy_file_to_dir(cls, filePath, tarDirPath):
        ...
        :return:
        """
        assert cls.is_file_exists(filePath), FileNotFoundError("需要copy的文件不存在")
        assert cls.is_file(filePath), FileNotFoundError("需要copy的不是一个文件")
