...
def create_device_apigw_jwt(sender, instance, **kwargs):
    ...
    if instance._state.adding:
        logger.info('Creating apigw JWT for Device "{}"'.format(instance))

        status_code, r1 = create_apigw_consumer_jwt(instance.profile)
        if status_code != 201:
        ...
        instance.apigw_jwt_id = r1.get('id')
        instance.apigw_jwt_key = r1.get('key')
        instance.apigw_jwt_secret = r1.get('secret')


@receiver(pre_delete, sender=Device)
def delete_device_api_jwt(sender, instance, **kwargs):
    logger.info('Deleting apigw JWT for Device "{}", jwt_id: "{}"'.format(instance, instance.apigw_jwt_id))

    status_code, r1 = delete_apigw_consumer_jwt(instance.profile, instance.apigw_jwt_id)
    logger.info('Kong JWT deleted ({0}), {1}'.format(status_code, r1))
    if status_code >= 300:
        if status_code != 404:
            # delete_apigw_consumer(username)
            logger.error('Error ({}) Deleting apigw JWT for Device "{}", "{}"'.format(status_code, instance, r1))
...
def delete_apigw_user_consumer(sender, instance, **kwargs):
    ...
    if not is_staff(user):
        logger.info('Profile "{}" will be deleted, removing api gateway consumer'.format(instance))

        status_code, r1 = delete_apigw_consumer(instance)
