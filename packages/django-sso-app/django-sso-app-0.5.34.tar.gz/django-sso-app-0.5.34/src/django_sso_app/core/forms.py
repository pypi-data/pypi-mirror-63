...
class LoginForm(allauth_LoginForm):
    fingerprint = CharField(label='fingerprint', max_length=255, required=False, widget=HiddenInput(), initial='undefined')

    def clean(self):
        try:
            super(LoginForm, self).clean()
        except Exception as e:
            setattr(self, '__django_sso_app__login_errors', e)
            raise e
...
class BaseSignupForm(allauth_SignupForm):
    ...
    def clean(self):
        try:
            ...
            logger.info('Signup error {}'.format(e))
            setattr(self, '__django_sso_app__signup_errors', e)
            raise e
