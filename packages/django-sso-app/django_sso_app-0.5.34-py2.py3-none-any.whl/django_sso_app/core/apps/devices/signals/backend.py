...
def post_profile_saved(sender, instance, created, **kwargs):
    ...
    if not created:  # if instance.pk:
        logger.debug('Profile "{}" post_save signal'.format(instance))

        rev_updated = getattr(instance, '__rev_updated', False)

        if rev_updated:
            logger.info('Rev updated, removing all user devices for Profile "{}"'.format(instance))

            adapter.remove_all_user_profile_devices(instance.user)
...
def pre_device_save(sender, instance, **kwargs):
    if not app_settings.APIGATEWAY_ENABLED:
        ...
        if not is_staff(user):
            logger.debug('Skip creating api gateway JWT for Device "{}"'.format(instance))

            instance.apigw_jwt_id = None
            instance.apigw_jwt_key = None
            instance.apigw_jwt_secret = get_random_string(32)
