...
class AbstractProfileModel(CreatedAtModel, UpdatableModel, DeactivableModel, PublicableModel):
    ...
    sso_id = models.CharField(max_length=36, unique=True, null=True, blank=True, db_index=True)  # unique on not null , editable=False
    sso_rev = models.PositiveIntegerField(default=0)

    username = models.CharField(_('username'), max_length=150, null=True, blank=True)  # django
    email = models.EmailField(_('email address'), null=True, blank=True)  # django
    ...
    def get_absolute_url(self):
        return reverse("django_sso_app_profile:detail", args=[self.sso_id])
    ...
    def is_incomplete(self):
        if app_settings.LOGIN_MUST_FILL_PROFILE:
...
class Profile(AbstractProfileModel):
    ...
    sso_id = models.CharField(_('sso id'), max_length=36, unique=True, db_index=True, default=uuid.uuid4)
    ...
    groups = models.ManyToManyField(Group, blank=True, related_name='sso_app_profiles', verbose_name=_('groups'), related_query_name='sso_app_profile')
