...
from ..core.apps.profiles.views import ProfileView, ProfileUpdateView
...
django_sso_app_i18n_urlpatterns = allauth_i18n_urlpatterns + django_sso_app_profile_urlpatterns
...
if settings.I18N_PATH_ENABLED:
    for lang, _name in settings.LANGUAGES:
        django_sso_app_urlpatterns.append(url(r'^login/$',
                                              RedirectView.as_view(url='/{}/login/'.format(lang), permanent=False)))
        django_sso_app_urlpatterns.append(url(r'^signup/$'.format(lang),
                                              RedirectView.as_view(url='/{}/signup/'.format(lang), permanent=False)))
        django_sso_app_urlpatterns.append(url(r'^email/$'.format(lang),
                                              RedirectView.as_view(url='/{}/email/'.format(lang), permanent=False)))
