...
df=spark.read.csv('Linear_regression_dataset_1.csv',inferSchema=True,header=True)
...
df.select(corr('x1','y')).show()             # 计算数据方差
...
print('-------------- 数据转换 ------------------')
vec_assmebler=VectorAssembler(inputCols=['x1', 'x2', 'x3', 'x4', 'x5'],outputCol='features') # 转换，这里相对将多元一次方程中的各变量存放到一个向量中
features_df=vec_assmebler.transform(df)

features_df.printSchema() # 查看变换后的结构。

model_df=features_df.select('features','y')     # 构建用于线性回归的数据模型
...
lin_Reg=LinearRegression(labelCol='y')                 # labelCol,相对于featrues列，表示要进行预测的列
