...
class LSHAttention(nn.Module):
    ...
    def forward(self, qk, v, query_len = None, input_mask = None, input_attn_mask = None):
        ...
        o, logits = UnsortLogits.apply(so, slogits, undo_sort, sticker, buckets_and_t)
...
class UnsortLogits(Function):
    @staticmethod
    def forward(ctx, so, slogits, undo_sort, sticker, buckets_and_t):
        ctx.save_for_backward(sticker, buckets_and_t)
        so = so.detach()
