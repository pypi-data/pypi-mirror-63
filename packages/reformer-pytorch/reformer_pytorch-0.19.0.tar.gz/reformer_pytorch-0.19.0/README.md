...
loss = enc_dec(x, yi, return_loss = True, enc_input_mask = input_mask) # will automatically forward encoder input mask to decoder context mask
loss.backward() # do your training

evaluate with the following
start_tokens = torch.tensor([[0.]]).long() # assume 0 is id of start token
samples = enc_dec.generate(x, start_tokens, seq_len = 1024)
