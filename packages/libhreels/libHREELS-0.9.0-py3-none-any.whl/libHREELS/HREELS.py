...
class HREELS:
    ...
    path = "\\\\backup-ep3\\BackUp02\\0_experiments\\EELS-PC4\HREELS\\gph_dat\\2019\\"
    # path = "./"
    def __init__(self, _filename, datapath=path):
        global eV2cm
        eV2cm = 8065.6
        self.maxIntensity = 1.
        if (datapath[-2:] != '\\' or datapath[-1] != '/'):
            datapath += '\\'
...
if __name__ == '__main__':
    ...
    # data1 = HREELS("GAB02",datapath)
    datapath = "D:\\Data\\Python\\HREELS\\data"
    data1 = HREELS("I2L02",datapath)
    if data1.valid:
        data1.info()
        data1.figure()
        data1.plot(label=data1.fname+"\n"+str(data1.startTime))
        data1.plot(xmin=300, factor= 400, color='r-', label='')
        # print("Selected data: ",data2.selectData(30,40))
        if data1.drawAndWait() == 'm':
            data1.plot(xmin=1400, factor= 600, color='g.', label='')

    a = data1.infoText()
    print('InfoText:\n',a)
    print('Filament:', data1.filament)
    print('Energy:', data1.energy)
    # plt.plotInfoAmp()
