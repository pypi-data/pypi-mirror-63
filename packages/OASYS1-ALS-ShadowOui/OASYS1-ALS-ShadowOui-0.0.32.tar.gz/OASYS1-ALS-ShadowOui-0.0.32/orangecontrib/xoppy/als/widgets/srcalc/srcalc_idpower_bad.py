...
class OWsrcalc_idpower(XoppyWidget, WidgetDecorator):
    ...
    description = "ID Power Absorbed and Transmitted by Optical Elements"
        ...
        for i in range(number_of_runs):
            ...
            if self.RAY_TRACING_IMAGE == 0:
                ...
                real_space_shuffle = [
                    ...
                    (numpy.random.random() - 0.5) * self.NUMBER_OF_PERIODS * self.PERIOD_LENGTH,
...
def ray_tracing(
    ...
    # define ray tracing results to output
...
def compute_power_density_on_optical_elements(dict1,do_interpolation=True):
    ...
    if "POWER_DENSITY_FOOTPRINT" in dict1.keys():
        for element_index in range(number_of_elements_traced):
            ...
            print("\n\n\n\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>> adding power from  element %d "%(element_index+1))

    else:
        print("\n\n\n\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>> creating power from  element %d " % (element_index + 1))
