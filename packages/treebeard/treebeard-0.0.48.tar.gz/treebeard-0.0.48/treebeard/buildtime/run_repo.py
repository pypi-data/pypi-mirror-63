...
def run_repo(
    ...
    try:
        ...
        subprocess.check_output(["nbstripout", " ".join(glob("./*.ipynb"))])
    finally:
    ...
    project_base_image = "gcr.io/treebeard-259315/project-base-image"
...
if __name__ == "__main__":
    ...
    if not build_tag:
        click.echo(f"Fatal: No build_tag provided inside {build_tag_key}")
        sys.exit(1)

    repo_url = os.getenv(repo_url_key)
    if not repo_url:
        click.echo(f"Fatal: No repo_url provided inside {repo_url_key}")
        sys.exit(1)
