...
$                                    #end of line""", re.VERBOSE)
    ...
    def __init__(self):
        ...
        self.ioc_patterns.update({
            ...
            'domain': DomainIOC(self.is_domain),
            'unknown': AnyIOC()
        })
    ...
    def is_domain(self, dom):
        split_domain = self.split_domain(dom)
        if split_domain['ip']:
            return True
        elif split_domain['sld']:
            d_tld = split_domain['tld']
            d_sld = split_domain['sld']
            if "." in d_tld or d_tld in IOCTyper.COMMON_TLDS:
                if self.tld_patterns['validSLD'].match(d_sld):
                    return True
                else:
                    return False
        else:
...
class URLIOC(IOCObj):
    ...
    def run(self, value):
        ...
        if match:
            if "/" in match.group():
                match_domain = match.group("domain")
                if match_domain and self.typer.is_domain(match_domain):
                    return True
                match_ipv4 = match.group("ipv4")
                if match_ipv4 and self.typer.is_ip(match_ipv4):
                    return True

                match_ipv6 = match.group("ipv6")
                if match_ipv6 and self.typer.is_ip(match_ipv6):
                    return True

        match = re.compile(IOCTyper.FILE_REGEX).search(value)
        if match and self.typer.is_domain(match.group()):
            return True
...
class DomainIOC(IOCObj):
    def __init__(self, is_domain):
        self.is_domain = is_domain

    def run(self, value):
        match = re.compile(r'(([A-Za-z0-9-]+[.])+[a-z]{2,})').search(value)

        if match and len(match.group()) != len(value):
            return False

        if match and self.is_domain(value):
            return True
