...
from .exceptions import AgentDVRConnectionError, AgentDVRError
	...
	async def _agent_request(self, method, api_url,timeout=DEFAULT_TIMEOUT) -> dict:
		...
		try:
			...
			raise AgentDVRConnectionError(
				"Timeout occurred while connecting to AgentDVR server"
			) from exception
		except (
			aiohttp.ClientError,
			...
			raise AgentDVRConnectionError(
				"Error occurred while communicating with AgentDVR server"
			) from exception
		...
		if "application/json" not in content_type:
			...
			raise AgentDVRError(
				"Unexpected response from the AgentDVR server",
