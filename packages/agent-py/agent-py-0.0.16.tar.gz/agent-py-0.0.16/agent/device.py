...
class Device:
	...
	async def motion_on(self):
		state = await self._client.get_state('command.cgi?cmd=motionOn&oid={0}&ot={1}'.format(self._oid, self._ot))
		if state is not None:
			self._raw_result['data']['detectorActive'] = True


	async def motion_off(self):
		state = await self._client.get_state('command.cgi?cmd=motionOff&oid={0}&ot={1}'.format(self._oid, self._ot))
		if state is not None:
