...
def download_file(url,file_name,retries=10):
    ...
    from limic.util import status, md5file, md5sum
...
def common(countries,url=None,show=False,osm=False):
    if countries:
        ...
        for country in countries:
            if country == 'w/o':
                anti = True
            country = compile(translate(country).replace(".*","[^_]*").replace("[^_]*[^_]*",".*"))
            for available in OSM_COUNTRIES if osm else COUNTRIES:
        ...
        countries = OSM_COUNTRIES if osm else COUNTRIES
    if show:
