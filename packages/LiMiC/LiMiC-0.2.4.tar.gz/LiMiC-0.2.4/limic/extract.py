...
CONFIG = [
    ...
    ("osm",{'args':[BLACK,WHITE,AROUND,EPS,SAFEDIST,PENALIZE,OSM,OUT]}),
    ("osm_pre",{'args':[BLACK,WHITE,OSM,OUT]}),
...
def extract_cache(file_name_in,file_name_out,overpass_url,area=None,around=1000,eps=0.01,safe_dist=100,penalize=20):
    ...
    from limic.util import start, end, file_size, status, load_pickled, save_pickled, verbosity, replace, check_overwrite
...
def extract_osm(file_name_in,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20,white="{'power':'line'}",black="{'power':'substation'}"):
    from limic.util import check_overwrite
    if not check_overwrite(file_name_in,file_name_out):
        return
    lim = osm_pre(file_name_in,white,black)
    osm_post(lim,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20)

def extract_osm_pre(file_name_in,file_name_out,white="{'power':'line'}",black="{'power':'substation'}"): #,conserve_mem):
    from limic.util import start, end, save_pickled, file_size, check_overwrite
    if not check_overwrite(file_name_in,file_name_out):
        return
    white,black = eval(white).items(), eval(black).items()
    lim = osm_pre(file_name_in,white,black)
...
def osm_pre(file_name_in,white=[("power","line")],black=[("power","substation")]):
    from limic.util import start, end, status, file_size, save_pickled
