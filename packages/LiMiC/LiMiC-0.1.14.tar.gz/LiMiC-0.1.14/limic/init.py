...
CONFIG = [
    ...
    ("stage1",{'help':"download map data and generate all graph files (SLOW)",'args':[
        ...
        ("osm",{'help':"compressed XML from GeoFabrik",'args':[SHOW,NOGT,URL,COUNTRIES]})
    ]}),
...
def extract_nx_all(overpass_url,countries):
    from limic.extract import extract_nx
    for country in countries:
        file_name_in = "cache."+country
        file_name_out = "graph."+country+".nx"
        extract_nx(file_name_in,file_name_out,overpass_url,area=None,around=1000,eps=0.01,safe_dist=100,penalize=20)

def extract_osm_all(countries):
    from limic.extract import extract_osm
    for country in countries:
        file_name_in = country+"-latest.osm.bz2"
        file_name_out = "graph."+country+".nx"
        extract_osm(file_name_in,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20)
...
def init_stage0(overpass_url,countries,no_gt,url=None,show=False):
    ...
    extract_nx_all(overpass_url,countries)
...
def init_stage1_osm(countries,no_gt,url=None,show=False):
    from limic.download import download_osm, common
    countries, url = common(countries,url,show,osm=True)
    download_osm(countries,url=url)
    extract_osm_all(countries)
    convert_merge_all(countries,no_gt)
