...
def transform_nx_npz(g,rescale=False):
    ...
    h['edges_air'] = array(edges_air,dtype=bool)
...
def transform_npz_nx(g,penalize=20):
    ...
    for index in range(len(ids)):
        ...
        for i in range(left,right):
            ...
            air = bool(edges_air[i])
            w = edges_weight[i]/((1000.*(penalize if air else 1)) if rescale else 1)
            h.add_edge(me,index2node[neighbor],weight=w,air=air)
    return h
