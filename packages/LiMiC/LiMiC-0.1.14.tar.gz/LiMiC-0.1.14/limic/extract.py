...
EPS = (("-e","--epsilon"),{'type':float,'dest':'eps','default':0.01,'help':"set minimum percentage of power line segment to consider non-logical intersections to EPS",'metavar':'EPS'})
AROUND = (("-r","--around"),{'type':int,'dest':'around','default':1000,'help':"set maximum free flying distance in m around power towers to AROUND",'metavar':'AROUND'})
SAFEDIST = (("-s","--safe-dist"),{'type':int,'dest':'safe_dist','default':100,'help':"set minimum safe distance from transformer stations and other perilous structures to SAFE",'metavar':'SAFE'})
...
CONFIG = [
    ...
    ("osm",{'args':[AROUND,EPS,SAFEDIST,PENALIZE,OSM,OUT]}),
    ("osm_pre",{'args':[OSM,OUT]}),
    ...
    for tower in list(g.nodes()):
        ...
        for neighbour in find_all_neighbours(tower,around,eps,safe_dist,minusid,latlon2id,penalize):
            if neighbour.tower in g or neighbour.tower.id < 0:
                ...
                g.add_edge(tower,pylon(neighbour.tower.id,neighbour.tower.latlon),weight=neighbour.dist,air=neighbour.air)
                if neighbour.tower.id < 0:
                    for intersection in neighbours2intersection.setdefault(neighbour.tower.neighbours,[]):
                        if intersection.latlon != neighbour.tower.latlon:
                            ...
                            g.add_edge(intersection,neighbour.tower,weight=distance(intersection.latlon,neighbour.tower.latlon),air=False)
                    neighbours2intersection[neighbour.tower.neighbours].append(neighbour.tower)


def extract_nx(file_name_in,file_name_out,overpass_url,area=None,around=1000,eps=0.01,safe_dist=100,penalize=20):
    from limic.overpass import distance, find_all_neighbours, is_safe, set_server, pylon, region, get_towers_by_area
    from limic.util import start, end, file_size, status, load_pickled, save_pickled, verbosity
...
def common_end(g,file_name_out):
...
def extract_osm(file_name_in,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20):
    lim = osm_pre(file_name_in)
    osm_post(lim,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20)

def extract_osm_pre(file_name_in,file_name_out): #,conserve_mem):
    from limic.util import start, end, save_pickled, file_size
    lim = osm_pre(file_name_in)
...
def extract_osm_post(file_name_in,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20):
    from limic.util import start, end, file_size, load_pickled
...
def osm_post(lim,file_name_out,around=1000,eps=0.01,safe_dist=100,penalize=20):
    from limic.util import start, end, status, file_size, load_pickled, haversine_distance, verbosity
    #from limic.mtree import MTree
    from scipy.spatial import cKDTree as KDTree
    from networkx import Graph
    from pyproj import CRS, Transformer
    lines, substations, towers, nodes, id2elem, id2lines, id2substations = lim
    #start("Loading filtered OSM data from",file_name_in)
...
def osm_pre(file_name_in):
        ...
        def startElement(self, name, attrs):
            if name == 'way':
                ...
                if attrs['k'] == 'power':
                    if attrs['v'] == 'line':
                        self.power = "line"
                    elif attrs['v'] == 'substation':
                        self.power = "substation"
            elif name == 'nd':
    ...
    status(len(ways.lines),end='   ')
    status(len(ways.substations),end='   ')
    id2lines = {}
    id2substations = {}
    for line in ways.lines:
        for node in line:
            temp = id2lines.setdefault(node,[])
            temp.append(line)
    for substation in ways.substations:
        for node in substation:
            temp = id2substations.setdefault(node,[])
            temp.append(substation)
    towers = []
    nodes = set()
    id2elem = {}
    ...
    #status(len(ids))
    #if conserve_mem:
    #start("Extracting nodes for power lines")
    #class nodes_handler(ContentHandler):
    #    def __init__(self,ids):
    #        self.towers = []
    #        self.ids = ids
    #    def startElement(self, name, attrs):
    #        if name == 'node' and int(attrs['id']) in self.ids:
    #            self.towers.append((int(attrs['id']),float(attrs['lat']),float(attrs['lon'])))
    #f = bopen(file_name_in,"rb") if file_name_in[-4:] == ".bz2" else open(file_name_in,"rb")
    #nodes = nodes_handler(ids)
    #parse(f, nodes)
    #f.close()
    #end('')
    #status(len(nodes.towers))
    #start("Checking that all nodes were extracted")
    #temp = set(map(lambda x:x.id,ways.towers))
    #temp.update(map(lambda x:x.id,ways.nodes))
    #assert(temp==ids)
    #del temp
    #end()
    #start("Saving data to",file_name_out)
    #save_pickled(file_name_out,(ways.lines,ways.substations,ways.towers,ways.nodes,id2elem,id2lines,id2substations))
    #end()
    return ways.lines,ways.substations,ways.towers,ways.nodes,id2elem,id2lines,id2substations
