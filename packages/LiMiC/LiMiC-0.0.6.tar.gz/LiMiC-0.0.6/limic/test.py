...
PACKAGES="requests,numpy,dogpile.cache,networkx,graph_tool,dbm.gnu"
...
def test(overpass_url,no_gt):
    ...
    init.init_stage1(overpass_url,countries,no_gt)
    ...
    for country, ids in zip(countries,idss):
        for suffix in ("nx","gt","npz"):
            if no_gt and suffix == "gt" and no_gt:
                continue
            ...
            vars(select)['select_'+suffix](file_name_in=graph_name,file_name_out="pruned."+country+"."+suffix,polygon=polygon,overpass_url=overpass_url)
