...
from limic.util import distance, get_disk_cache
...
if disk_cache:
    region = make_region(function_key_generator=key_generator).configure("dogpile.cache.dbm",arguments={"filename":disk_cache})
else:
...
class dist:
    def __init__(self,dist,tower):
        self.dist = dist
        self.tower = tower
    def __lt__(self,other):
        return self.dist < other.dist
    def __repr__(self):
        return "dist(dist="+repr(self.dist)+",tower="+repr(self.tower)+")"

def overpass(query,timeout=3600,retries=10):
    from limic.util import verbosity, set_failed
    ...
    while retries:
        try:
            ...
            if verbosity >= 1 and retries: print("RETRYING",url)
    set_failed(True)
...
def dist_towers(towers, location):
    ...
    for tower in towers:
        ...
        d_towers.append(dist(d,tower))
    return d_towers

def intersect(p1,p2,p3,p4,eps):
...
def find_all_neighbours(tower, around, eps, safe_dist, minusid, latlon2id, penalty):
    ...
    for neighbour in find_towers(tower.latlon, around):
        if not neighbour in direct_neighbours and neighbour.id != tower.id:
             neighbours.append(dist(penalty*distance(neighbour.latlon,tower.latlon),neighbour))
        #else:
    ...
    for indirect_segment in indirect_segments:
        for direct_segment in direct_segments:
            ...
            if intersection:
                ...
                neighbours.append(dist(distance(intersection,tower.latlon),pylon(latlon2id[intersection], intersection,neighbours=indirect_segment)))
