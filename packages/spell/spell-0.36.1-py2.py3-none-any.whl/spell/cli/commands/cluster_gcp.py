...
def get_container_registry_bucket_name(ctx, storage_service):
    ...
    if len(buckets) == 0:
        ...
        click.echo("Spell found multiple container registries associated with your Google project:\n" +
                   "\n".join(["{} : {}".format(i, b.location) for i, b in enumerate(buckets)]))
        bucket_number = click.prompt("Enter the number for the registry which you would like to select",
                                     type=click.IntRange(0, len(buckets) - 1), default=0)
