...
class Template():
    ...
    def __init__(self):
        if is_installed_package(self.dir_path):
            self.root_path = os.path.join(tempfile.gettempdir(),
                                          "salesforcexytools", "templates")
    ...
    def load_config(self, type):
        config_path = os.path.join(self.root_path, self.template_folder,
                                   self.config_file)
    ...
    def get(self, type, name):
        template_path = os.path.join(self.root_path, self.template_folder, type,
                                     name)
...
class AntConfig():
    ...
    def __init__(self):
        ...
        self.template_ant_dataloader_path = os.path.join(
            self.root_path, self.template_folder, self.AntDataloader_folder)
    ...
    def build_migration_tools(self,
                              save_path,
                              config_data,
                              template_name="MigrationTools"):
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        tmp_migration_tools_path = os.path.join(self.root_path,
        ...
        self._save_file(os.path.join(save_path, "build.properties"),
    ...
    def build_ant_dataloader(self, save_path, config_data):
        ...
        build_properties_src = self.get_file(self.template_ant_dataloader_path,
                                             "config.properties")
        build_properties_src = build_properties_src.format(**config_data)
        self._save_file(os.path.join(save_path, "config.properties"),
                        build_properties_src)

        build_xml_src = self.get_file(self.template_ant_dataloader_path,
                                      "build.xml")
    ...
    def _get_ant_proxy_body(self, config_data):
        ...
        if "use_proxy" in proxy_config and proxy_config["use_proxy"]:
            if "nonproxyhosts" in proxy_config and proxy_config["nonproxyhosts"]:
                xml_str_list.append('nonproxyhosts="' +
            ...
            if "proxypassword" in proxy_config and proxy_config["proxypassword"]:
                xml_str_list.append('proxypassword="' +
            ...
            if "proxyuser" in proxy_config and proxy_config["proxyuser"]:
                xml_str_list.append('proxyuser="' + proxy_config["proxyuser"] +
                                    '"')
            if "socksproxyhost" in proxy_config and proxy_config[
                    "socksproxyhost"]:
                xml_str_list.append('socksproxyhost="' +
                                    proxy_config["socksproxyhost"] + '"')
            if "socksproxyport" in proxy_config and proxy_config[
                    "socksproxyport"]:
                xml_str_list.append('socksproxyport="' +
                                    proxy_config["socksproxyport"] + '"')
