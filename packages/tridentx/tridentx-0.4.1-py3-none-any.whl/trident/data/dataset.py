...
class MaskDataset(Dataset):
    ...
    def image_transform(self, img_data):
        if len(self.image_transform_funcs) == 0:
            return label_backend_adaptive(img_data, expect_data_type=self.expect_data_type)
        if isinstance(img_data, np.ndarray):
            # if img_data.ndim>=2:
            for fc in self.image_transform_funcs:
                ...
                img_data = fc(img_data)
            img_data = label_backend_adaptive(img_data, expect_data_type=self.expect_data_type)
...
class SsdBboxDataset(BboxDataset):
    def __init__(self, boxes=None, image_size=None, priors=None, center_variance=0.1, size_variance=0.2,
                 expect_data_type=ExpectDataType.absolute_bbox, class_names=None, symbol=None, name=''):
    ...
    def assign_priors(self, gt_boxes, gt_labels, center_form_priors, iou_threshold):
        ...
        for target_index, prior_index in enumerate(best_prior_to_target_index):
            ...
            overlap_match=np.where(gt_overlap>(best_prior_to_target[target_index]*0.75))[0]
        ...
        try:
            ...
            if isinstance(self.label, BboxDataset):
                ...
                if isinstance(label, tuple):
                    ...
                    mask = label
                    label = None
                data, mask = self.paired_transform(data, mask)
            ...
            if self.signature is None:
                names = []
                names.append('data' if data is not None else '')
                names.append('bbox' if bbox is not None else '')
                names.append('mask' if mask is not None else '')
                names.append('label' if label is not None else '')
                names.append('unpair' if unpair is not None else '')
                self.signature = namedtuple('signature', [item for item in names if item != ''])
