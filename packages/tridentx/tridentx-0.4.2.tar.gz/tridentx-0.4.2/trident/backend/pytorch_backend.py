...
__all__ = ['to_numpy','to_tensor','print_network','plot_tensor_grid','summary','calculate_flops', 'Layer', 'Sequential', 'Input', 'get_device', 'load','Combine','ReplayBuffer','check_keys','try_map_args_and_call','get_signature']
...
# class InputSpec(torch.IntTensor):
    ...
    def __call__(self, *input, **kwargs):
        ...
        if torch._C._get_tracing_state():
            ...
            if isinstance(result,torch.Tensor) :
                ...
                #self._output_tensor = result.clone().cpu().detach_()
...
class Sequential(Layer):
    ...
    def __init__(self, *args,name=''):
        ...
        if len(args) == 1 and isinstance(args[0], OrderedDict):
            ...
            for  idx, module in enumerate(args[0]):
                self.add_module(str(idx), module[1])
        else:
    ...
    def __getitem__(self, idx):
        if isinstance(idx, slice):
            return self.__class__(list(self._modules.items())[idx])
        else:
