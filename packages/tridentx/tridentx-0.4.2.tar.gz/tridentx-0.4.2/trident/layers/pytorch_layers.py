...
__all__ = ['Dense', 'Flatten', 'Concatenate', 'Concate', 'Add', 'Subtract', 'Dot', 'Conv1d', 'Conv2d', 'Conv3d',
           ...
           'CoordConv2d', 'Upsampling2d', 'Dropout', 'AlphaDropout', 'SelfAttention']
...
class _ConvNd(Layer):
    ...
    def __init__(self, kernel_size, num_filters, strides, auto_pad, padding_mode, use_bias, dilation, groups,
                 transposed, name, filter_rate, **kwargs):
    ...
    def build(self, input_shape):
        if self._built == False:
            if self.num_filters is None and self.filter_rate is not None:
                self.num_filters = int(round(self.input_filters * self.filter_rate,0))
...
class Conv1d(_ConvNd):
    def __init__(self, kernel_size, num_filters=None, strides=1, auto_pad=True, padding_mode='zero', activation=None,
                 use_bias=False, dilation=1, groups=1, name='', filter_rate=None, **kwargs):
        ...
        super(Conv1d, self).__init__(kernel_size, num_filters, strides, auto_pad, padding_mode, use_bias, dilation,
                                     groups, False, name, filter_rate, **kwargs)
        self.activation = get_activation(activation)
        self.auto_pad = auto_pad
...
class Conv2d(_ConvNd):
    ...
    def get_padding(self, input_shape):
        ...
        if self.padding_mode == 'circular':
            self.padding = ((pad_h + 1) // 2, pad_h // 2, (pad_w + 1) // 2, pad_w // 2)
        else:
...
class Conv3d(_ConvNd):
    ...
    def get_padding(self, input_shape):
        ...
        if self.padding_mode == 'circular':
            self.padding = ((pad_z + 1) // 2, pad_z // 2, (pad_h + 1) // 2, pad_h // 2, (pad_w + 1) // 2, pad_w // 2)
        else:
            self.padding = (
...
class TransConv1d(_ConvNd):
    def __init__(self, kernel_size, num_filters=None, strides=1, auto_pad=True, padding_mode='zero', activation=None,
        ...
        super(TransConv1d, self).__init__(kernel_size, num_filters, strides, auto_pad, padding_mode, use_bias, dilation,
                                          groups, True, name, filter_rate, **kwargs)
        self.activation = get_activation(activation)
    ...
    def build(self, input_shape):
        if self._built == False or self.conv1 is None:
            ...
            self.conv1 = DepthwiseConv3d(kernel_size=self.kernel_size, depth_multiplier=self.depth_multiplier,
                                         strides=self.strides, auto_pad=self.auto_pad, padding_mode=self.padding_mode,
                                         dilation=self.dilation, groups=self.input_filters, bias=self.use_bias)
            self.pointwise = Conv3d(kernel_size=(1, 1, 1), filter_rate=1, strides=1, use_bias=self.use_bias, dilation=1,
                                    groups=1)
...
class DepthwiseConv3d(Layer):
    ...
    def forward(self, *x):
        ...
        if self.auto_pad == True:
            ...
            if pad_z > 0 or pad_h > 0 or pad_w > 0:
                x = F.pad(x, [pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2, pad_z // 2,
                              pad_z - pad_z // 2], mode='constant' if self.padding_mode=='zero' else self.padding_mode)
        x = self.conv1(x)
        if self.activation is not None:
            x = self.activation(x)
        return x
...
class DeformConv2d(Layer):
    def __init__(self, kernel_size, num_filters=None, strides=1, offset_group=2, auto_pad=True, padding_mode='zero',
                 activation=None, use_bias=False, dilation=1, groups=1, name='', filter_rate=None, **kwargs):
        ...
        if self._built == False:
            ...
            print('{0} input:{1} -> output:{2}   {3}  {4}  gcd:{5} group:{6}   通道縮放倍數:{7} '.format(self.name,
                                                                                                   ...
                                                                                                   self.num_filters / float(
                                                                                                       self.input_filters)))
            self.get_padding([self.input_filters // self.groups, self.groups, self.input_shape[1], self.input_shape[2]])

            self.register_buffer('channel_kernal', torch.zeros((1)))
            self.register_buffer('channel_dilation', torch.zeros((1)))
            self.channel_kernal = torch.tensor(2 if self.crossgroup_fusion == True and self.groups > 3 else 1)
            self.channel_dilation = torch.tensor(1)
            if self.crossgroup_fusion == True and self.groups > 4:
                self.channel_dilation = torch.tensor(2)

            reshape_input_shape = [-1, self.input_filters // self.groups, self.groups, self.input_shape[1],
                                   self.input_shape[2]]
...
class GcdConv2d_1(Layer):
    def __init__(self, kernel_size, num_filters=None, strides=1, auto_pad=True, padding_mode='zero', activation=None,
                 use_bias=False, divisor_rank=0, dilation=1, self_norm=True, name='', filter_rate=None, **kwargs):
