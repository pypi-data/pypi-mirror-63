...
class MaskDataset(Dataset):
    def __init__(self, images=None, expect_data_type: ExpectDataType = ExpectDataType.label_mask,
                 get_image_mode: GetImageMode = GetImageMode.processed, symbol=None, name=''):
        super().__init__(symbol=symbol, expect_data_type=expect_data_type, name=name)
        if expect_data_type not in [ExpectDataType.label_mask, ExpectDataType.binary_mask]:
    ...
    def mask_transform(self, img_data):
        if self.expect_data_type==ExpectDataType.binary_mask:
            img_data[img_data>0]=1
            return img_data.astype(np.int64)
        else:
            return img_data
...
class SsdBboxDataset(BboxDataset):
    def __init__(self, boxes=None, image_size=None, priors=None, center_variance=0.1, size_variance=0.2,gt_overlap_tolerance=0.5,
                 expect_data_type=ExpectDataType.absolute_bbox, class_names=None, symbol='bbox', name=''):
    ...
    def convert_boxes_to_locations(self, center_form_boxes, center_form_priors):
        if center_form_boxes.ndim==1:
            center_form_boxes=np.expand_dims(center_form_boxes, 0)
        if center_form_priors.ndim==1:
    ...
    def corner_form_to_center_form(self, boxes):
        return np.concatenate([(boxes[..., :2] + boxes[..., 2:]) / 2, boxes[..., 2:] - boxes[..., :2]],
                              len(boxes.shape) - 1)
    ...
    def assign_priors(self, gt_boxes, gt_labels, center_form_priors, iou_threshold):
        ...
        # return boxes, labels
        corner_form_priors = self.center_form_to_corner_form(center_form_priors)
        center_form_boxs = self.corner_form_to_center_form(gt_boxes)

        boxes = np.zeros((corner_form_priors.shape[0], 4)).astype(np.float32)
        labels = np.zeros((corner_form_priors.shape[0])).astype(np.int64)
        ...
        # _gt[:, 2] = _gt[:, 2] + _gt[:, 0]
        # _gt[:, 3] = _gt[:, 3] + _gt[:, 1]
        overlaps = self.iou_of(np.expand_dims(corner_form_priors, 0), np.expand_dims(gt_boxes, 1))
        # size: num_priors
        best_target_to_prior, best_target_to_prior_index =np.max(overlaps,axis=0), np.argmax(overlaps,axis=0)
        # size: num_targets
        best_prior_to_target, best_prior_to_target_index =np.max(overlaps,axis=1),np.argmax(overlaps,axis=1)
        center_priors=center_form_priors[best_prior_to_target_index]

        for target_index, prior_index in enumerate(best_prior_to_target_index):
            gt_overlap=overlaps[target_index]
            overlap_match=np.where(gt_overlap>(best_prior_to_target[target_index]*self.gt_overlap_tolerance))[0]
            labels[overlap_match] = gt_labels[target_index]
            loc= self.convert_boxes_to_locations(self.corner_form_to_center_form(gt_boxes[target_index]), center_form_priors[overlap_match])
            boxes[overlap_match] =loc

        for target_index, prior_index in enumerate(best_prior_to_target_index):
            best_target_to_prior_index[prior_index] = target_index
            labels[prior_index]=gt_labels[target_index]
            loc=self.convert_boxes_to_locations(self.corner_form_to_center_form(gt_boxes[target_index]), center_form_priors[prior_index])
            boxes[prior_index] =loc
        return boxes, labels
    ...
    def bbox_transform(self, bbox):
        if bbox is None or len(bbox) == 0:
            ...
            if bbox is None or len(bbox) == 0:
                boxes, labels = self.assign_priors(None, None, center_form_to_corner_form(to_numpy(self.priors)), 0.3)
                return to_numpy(boxes), to_numpy(labels)
            elif bbox.shape[-1] == 5:
                gt_box = bbox[:, :4]
                gt_label = bbox[:, 4]
...
class Iterator(object):
    ...
    def data(self, value):
        self._data = value
        if self._label is not None and isinstance(self._label, (MaskDataset, ImageDataset)) and isinstance(self._data,
    ...
    def __getitem__(self, index: int):
        try:
            ...
            if hasattr(self.label, 'label_transform') and not isinstance(self.label, BboxDataset):
            ...
            if self.signature is None or len(self.signature)==0:
                self.signature=OrderedDict()
                if data is not None:
                    self.signature['data' if self.data.symbol is None or len(self.data.symbol)==0 else self.data.symbol]=data.shape
                if bbox is not None:
                    self.signature['bbox' if self.label.symbol is None or len(self.label.symbol) ==0 else self.label.symbol] =  bbox.shape
                if mask is not None:
                    self.signature['mask' if self.label.symbol is None or len(self.label.symbol) ==0 else self.label.symbol] = mask.shape
                if label is not None:
                    self.signature['label'] = label.shape if  isinstance(label,np.ndarray) else type( label)
                if label is not None:
                    self.signature[ 'unpair' if self.unpair.symbol is  None or len(self.unpair.symbol)==0 else self.unpair.symbol] = unpair.shape
