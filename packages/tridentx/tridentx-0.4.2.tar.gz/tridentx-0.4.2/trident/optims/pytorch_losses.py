...
__all__ = ['MSELoss', 'CrossEntropyLoss', 'BCELoss', 'F1ScoreLoss', 'L1Loss', 'L2Loss', 'make_onehot', 'MS_SSIM',
           'CrossEntropyLabelSmooth', 'mixup_criterion', 'DiceLoss', 'focal_loss_with_logits', 'FocalLoss',
           'SoftIoULoss', 'CenterLoss', 'TripletLoss', 'LovaszSoftmax', 'PerceptionLoss', 'EdgeLoss',
           'TransformInvariantLoss', 'get_loss']
...
class BCELoss(_Loss):
    ...
    def forward(self, output, target):
        ...
        target1[target1 != 1] = 0
        target = target.float()
        if target1.astype(np.float32).mean() == 1:
...
class L2Loss(nn.MSELoss):
    def __init__(self, reduction='mean', name='MSELoss'):
        ...
        self.reduction = reduction
    def forward(self, output, target):
        return 0.5 * super(L2Loss, self)(output, target)
...
class MS_SSIM(_Loss):
    ...
    def ms_ssim(self, output, target, levels=5):
        ...
        for i in range(levels):
            ssim_map, mcs_map = self._ssim(output, target)
            msssim[i] = ssim_map
            mcs[i] = mcs_map
            filtered_im1 = F.avg_pool2d(output, kernel_size=2, stride=2)
            filtered_im2 = F.avg_pool2d(target, kernel_size=2, stride=2)
            output = filtered_im1
            target = filtered_im2
...
class FocalLoss(_Loss):
    ...
    def forward(self, output, target):
        ...
        for cls in range(num_classes):
            ...
            loss = loss + focal_loss_with_logits(cls_input, cls_target, gamma=self.gamma, alpha=self.alpha,
        ...
        if (target.ndim == 1 and (output.ndim == 1 or output.ndim == 2)):
            ...
            for k in range(num_class):
                if k != self.ignore_index:
                    ...
                    f1 = f1 + (1 - (1+self.beta ** 2) * (precision * recall) / (self.beta ** 2 * precision + recall + 1e-8))
                    n += 1
            if self.reduction == 'mean':
                f1 = f1 / n
            return f1
            ...
            raise ValueError(
                'target.ndim:{0}  output.ndim:{1} is not valid for F1 score calculation'.format(target.ndim,   output.ndim))
