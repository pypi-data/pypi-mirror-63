...
__all__ = ['read_image','read_mask','save_image','save_mask','image2array','array2image','mask2array','array2mask','list_pictures','normalize','unnormalize','channel_reverse','blur','random_blur','random_crop','resize','rescale','add_noise','gray_scale','to_rgb','auto_level','random_invert_color',
           'image_backend_adaptive','reverse_image_backend_adaptive','random_adjust_hue', 'random_channel_shift', 'random_cutout','random_rescale_crop','random_center_crop','adjust_gamma','random_adjust_gamma','adjust_contast','random_adjust_contast','clahe','erosion_then_dilation','image_erosion','image_dilation','adaptive_binarization','random_transform']
...
def random_adjust_hue(hue_range=(-20,20),saturation_range=(-50,50),lightness_range=(-30,30)):
    def img_op(image: np.ndarray):
...
def random_rescale_crop(h,w,order=1):
    def img_op(*image:np.ndarray):
        results = to_list(image)
        scale=np.random.choice(np.clip(np.random.normal(1,0.3,100),0.5,2))
        height, width = image[0].shape[:2]
