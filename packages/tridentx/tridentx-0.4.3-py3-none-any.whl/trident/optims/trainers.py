        ...
        try:
            ...
            for k, v in self._losses.items():
                ...
                try:
                    ...
                    this_loss = try_map_args_and_call(v, train_data, self.training_context[
                        'data_feed']) * loss_weight  # v.forward(output, target) if hasattr(v, 'forward') else v(
                    # output, target)
            ...
            if accumulate_grads == False:
                ...
                for k, v in self._metrics.items():
                    ...
                    this_metric = try_map_args_and_call(v, test_data if test_data is not None and len(
                        test_data) > 0 else train_data, self.training_context['data_feed'])
    ...
    def start_now(self, collect_data_inteval=1, is_resume=False):
        try:
            self.execution_id = str(uuid.uuid4())[:8].__str__().replace('-',
                                                                        '') if self.execution_id is None else self.execution_id
