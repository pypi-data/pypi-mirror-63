...
class OptimizerMixin(object):
    ...
    def adjust_learning_rate(self, new_lr, verbose=True):
        ...
        # elif _backend=='cntk':  #     old_lr = super(OptimizerMixin, self).learning_rate()  #     if old_lr!=
        # new_lr:  #         super(OptimizerMixin, self).reset_learning_rate(new_lr)  #         if verbose:  #
        # print('learning rate changed! ( form {0:.3e} to {1:.3e})'.format(old_lr,new_lr))
...
class ModelBase(object):
    ...
    def model(self, value):
        if isinstance(value, Layer):
            if value.built:
                ...
                args = [param.name for param in inspect.signature(self._model.forward).parameters.values()]
                self._signature = namedtuple('signature', args)
            else:
                self.inputs = OrderedDict()
                self.targets = OrderedDict()
    ...
    def update_signature(self,arg_names):
        ...
        elif len(self.signature.key_list)<len(arg_names) and len(self.signature.key_list)+len(self.outputs)==len(arg_names):
    ...
    def train_model(self, train_data, test_data, current_epoch, current_batch, total_epoch, total_batch,
        ...
        try:
            ...
            for k, v in self._losses.items():
                ...
                this_loss = try_map_args_and_call(v,train_data,self.training_context['data_feed']) * loss_weight#v.forward(output, target) if hasattr(v, 'forward') else v(output, target)
            ...
            if accumulate_grads == False:
                ...
                for k, v in self._metrics.items():
                    ...
                    this_metric= try_map_args_and_call(v, test_data if test_data is not None and len(test_data)>0 else train_data, self.training_context['data_feed'])
...
class TrainingPlan(object):
    ...
    def display_loss_metric_curve_scheduling(self, frequency: int, unit='batch', save_path: str = None,
                                      name_prefix: str ='loss_metric_curve_{0}.png', clean_ipython_output_frequency=5,imshow=False):
        ...
        try:
            ...
            if not is_resume:
                for item in self.training_items.values():
                    item.rebinding_input_output(item.inputs.value_list[0])
                    for callback in self.callbacks:
            ...
            for epoch in range(self.num_epochs):
                try:
                    ...
                    for mbs, return_data in enumerate(data_loader):
                        ...
                        for i in range(len(data_loader.signature._fields)):
                            name=data_loader.signature._fields[i]
