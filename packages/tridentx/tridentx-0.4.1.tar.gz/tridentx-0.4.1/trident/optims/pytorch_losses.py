...
__all__ = ['MSELoss','CrossEntropyLoss','BCELoss','F1ScoreLoss','L1Loss','L2Loss', 'make_onehot', 'MS_SSIM', 'CrossEntropyLabelSmooth', 'mixup_criterion', 'DiceLoss', 'focal_loss_with_logits','FocalLoss', 'SoftIoULoss', 'CenterLoss', 'TripletLoss', 'LovaszSoftmax','PerceptionLoss','EdgeLoss','TransformInvariantLoss','get_loss']
...
class FocalLoss(_Loss):
    ...
    def forward(self, input, target):
        ...
        for cls in range(num_classes):
            ...
            loss =loss+ focal_loss_with_logits(cls_input, cls_target, gamma=self.gamma, alpha=self.alpha,reduction=self.reduction
            )
        return loss.sum()
...
class F1ScoreLoss(_Loss):
    ...
    def forward(self, output, target):
        ...
        assert output.ndim == 1 or output.ndim == 2
