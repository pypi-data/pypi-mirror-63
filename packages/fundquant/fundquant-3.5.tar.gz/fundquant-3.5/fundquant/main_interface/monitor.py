...
class IndexMonitor:
    ...
    def _etf_statistic_to_df(self, statistics: list) -> pandas.DataFrame:
        ...
        for stat in statistics:
            ...
            pe_diff_2m_ago_signal.append(
                self.pe_cur_ago_signal(stat.pe_latest, stat.pe_2m_ago))
        ...
        df = pandas.DataFrame(
            data=np.array(
                [index_code, index_name, etf_code, etf_name, pe_less_prob_10y,
                 pe_diff_2m_ago_signal,
                 pe_less_prob_10y_2m_ago, pe_mean_div_rt, pe_max_rt, pe_latest, pe_mean, pe_max, pb_less_prob_10y,
                 ai_signal,
                 pe_volatility_3y, real_year, last_dt]).T,
            columns=['指数代码', '指数名称', 'ETF代码', 'ETF名称', 'PE分位', 'PE2m信号',
                     'PE分位2m', 'PE均值比', 'PE:max_rt', 'PE', 'PE均值', 'PE最大值', 'PB分位', 'AI', '3年波动', '跟踪年数',
                     '最后日期'])
        return df
