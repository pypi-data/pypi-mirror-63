...
def get_md5(string):
    ...
    #注意,python3中如果要得到与javascript/java相同的加密结果要用digest(),其他情况可用hexdigest()
    return md5(string.encode('utf8')).digest()

def get_sha1(string):
    from hashlib import sha1
    s1=sha1()
    s1.update(string.encode())
    #注意,python3中如果要得到与javascript/java相同的sha1加密结果要用s1.digest(),其他情况可用s1.hexdigest()
    return s1.digest()
