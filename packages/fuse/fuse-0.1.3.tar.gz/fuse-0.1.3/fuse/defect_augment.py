...
class DefectAugment(BatchFilter):
    ...
    def prepare(self, request):
        ...
        logger.debug('raw voxel size is %s', raw_voxel_size)
        assert raw_voxel_size is not None, 'raw voxel size undefined for key %s' % self.intensities
        ...
        if 'deformed_slice' in self.slice_to_augmentation.values():
            ...
            logger.debug("upstream request voxel size is %s" % spec.voxel_size)
