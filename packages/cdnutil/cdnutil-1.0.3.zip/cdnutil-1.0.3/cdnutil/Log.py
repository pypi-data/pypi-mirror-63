...
class Logger:
    def __init__(self, className="UNKNOWN"):
        flevel = None
        clevel = None
        logFileName = None
        from cdnutil import config
        if config.logType == config.LogType.INI:
            ...
            flevel = config.getPropertyValue("log", "flog_level")
            clevel = config.getPropertyValue("log", "slog_level")
            if logFileName is None:
                logFileName = config.getPropertyValue("log", "log_file_name")
