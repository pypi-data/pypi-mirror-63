...
class ImportTabularDataModule:
    ...
    def run(cls, input_dict: dict = None) -> DataFrameDirectory:
        ...
        try:
            ...
            if 'The SSL connection could not be established' in error_message:
                ErrorMapping.rethrow(import_exceptions, UserError(error_message))
            elif 'Failed to create or update the dataset definition. The dataset definition has been created or ' \
                 'update' in error_message:
                ErrorMapping.rethrow(import_exceptions, UserError(error_message))
            elif 'The provided path is not valid or the files could not be accessed' in error_message:
                ErrorMapping.rethrow(import_exceptions, UserError(error_message))
            elif 'Unable to open stream from Azure Blob' in error_message:
