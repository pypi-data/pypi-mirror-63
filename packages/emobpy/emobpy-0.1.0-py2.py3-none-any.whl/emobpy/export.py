...
class Export:
    ...
    def to_csv(self):
        ...
        for i, key in enumerate(self.listavailkeys):
                ...
                self.fleet.loc[i, 'ev_capacity'] = self.data.db[key]['battery_capacity'] / self.conversion
                self.fleet.loc[i, 'share_ev'] = 1/self.k
                self.fleet.loc[i, 'cons_rate'] = self.data.db[key]['energy_consumption'] / self.conversion
        ...
        self.consdf.columns = pd.MultiIndex.from_product([['Demand_'+ self.kwto +'h'], ['ev_ed'], self.consdf.columns])

        self.avaidf = pd.DataFrame(self.arr_avai.T, columns=self.name)
        self.avaidf.columns = pd.MultiIndex.from_product([['Grid_connect_'+ self.kwto], ['n_ev_p'], self.avaidf.columns])
        ...
        if self.subscen:
            ...
            for id1, cd in enumerate(self.code):
                if self.subscen[cd]:
                    for key, value in self.subscen[cd].items():
                        ...
                        df = df[0:self.rows].copy()
                        self.arr_options[id0, id1, :] = df['charge_grid'].values / self.conversion
            ...
            for opt, idopt in self.optdict.items():
                ...
                dfop.columns = pd.MultiIndex.from_product([[opt + '_' + self.kwto + 'h'], ['ev_ged_exog'], dfop.columns])
    ...
    def save_files(self, repository=""):
        ...
        self.final.to_csv(os.path.join(self.repository, 'time_series.csv'))
        self.fleet.to_csv(os.path.join(self.repository, 'data_input.csv'), index=False)
