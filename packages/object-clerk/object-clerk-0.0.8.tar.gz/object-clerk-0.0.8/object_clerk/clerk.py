...
class ObjectClerk:
    ...
    @mutate_client_exceptions
    def _upload_object(self, bytes_obj: bytes, bucket: str, object_key: str, content_type: str) -> None:
        """
        ...
        self.client.upload_fileobj(
            BytesIO(bytes_obj), Bucket=bucket, Key=object_key, Config=TRANSFER_CONFIG, ExtraArgs={"ContentType": content_type}
        )
    ...
    def upload_object(
        ...
        retry_call(
            self._upload_object,
            fkwargs={"bytes_obj": bytes_obj, "bucket": bucket, "object_key": object_key, "content_type": content_type},
            **self.retry_config,
        )
        if verify_checksum:
