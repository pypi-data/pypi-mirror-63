...
class Resource:
    ...
    def _process_res(self, r: Response):
        if not r.ok:
            log.debug(r)
            log.debug(r.text)
        r.raise_for_status()
...
class Script(BEObjectRef):
    ...
    @classmethod
    def upload_str(cls, code: str, make_public: bool = False, **kwargs) -> "Script":
        """Upload a function from the file (or current script?)"""
        with temp_fname(".py", keep=True) as fn:
            file = Path(fn)
            file.write_text(code)
        res = cls._create_from_file(file, **process_args(make_public, kwargs))
        return cls(dto=res)

    @classmethod
    def upload_file(cls, file: NPath, make_public: bool = False, **kwargs) -> "Script":
        res = cls._create_from_file(Path(file), **process_args(make_public, kwargs))
        return cls(dto=res)
    ...
    def run(self, disable_cache=False):
        res = Resource("/runs/").post(script=self.url, disable_cache=disable_cache)
        return res
...
class Report(BEObjectRef):
    ...
    def run(self, clone: bool = False) -> "Report":
        """
        run the given app, cloning if needed
        - TODO - add support for setting editable datasets here (forcing a clone)?
        """
