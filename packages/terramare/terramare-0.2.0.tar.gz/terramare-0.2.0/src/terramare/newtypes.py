    ...
    def __call__(self, value: Primitive) -> T:
        ...
        try:
            ...
            raise NewTypeDeserializationError(
                value, self._newtype_t, cause=e, always_display_msg=True
            ) from None
...
@attr.s(auto_attribs=True, frozen=True)
class NewTypeDeserializerFactory(factories.DeserializerFactory):
    ...
    def create_deserializer_recursive(
        ...
        if not getattr(type_, "__qualname__", None) == "NewType.<locals>.new_type":
            raise NewTypeDeserializerFactoryError(type_, "not a newtype")
        try:
