...
def accumulate_errors_l(
    exception_t: Type[E], fn_elts: Iterator[Tuple[Callable[[S], T], S]]
) -> Tuple[List[Tuple[int, T]], List[Tuple[int, E]]]:
...
def accumulate_errors_d(
    exception_t: Type[E], fn_elts: Mapping[K, Tuple[Callable[[S], T], S]]
) -> Tuple[Dict[K, T], Dict[K, E]]:
