...
def sync(
    ...
    *,
    ...
    tap_cmd = f"tap-{tap_name} --config {config_file} --catalog {catalog_file}"
    if io.file_exists(state_file):
        local_state_file = io.make_local(state_file)
        tap_cmd = f"{tap_cmd} --state {local_state_file}"
    sync_cmd = (
        f"{tap_cmd} | target-{target_name} --config {target_config_file} "
        f">> {state_file}"
    )
    jobs.run_command(sync_cmd)
    if local_state_file != state_file:
        io.upload_file(local_state_file, state_file)
