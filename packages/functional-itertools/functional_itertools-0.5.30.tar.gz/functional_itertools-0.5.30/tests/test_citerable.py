...
@mark.parametrize("cls", [CIterable, CList, CSet, CFrozenSet])
@given(data=data(), func=int_to_bool_funcs)
def test_filter(
    cls: Type, data: DataObject, func: Callable[[int], bool],
) -> None:
    x, cast = data.draw(lists_or_sets(cls, integers()))
    y = cls(x).filter(func)
    assert isinstance(y, cls)
    assert cast(y) == cast(filter(func, x))
...
@mark.parametrize("cls", [CIterable, CList, CSet, CFrozenSet])
@given(data=data(), func=int_to_bool_funcs)
def test_map(cls: Type, data: DataObject, func: Callable[[int], bool]) -> None:
    x, cast = data.draw(lists_or_sets(cls, integers()))
    y = cls(x).map(func)
    assert isinstance(y, cls)
    assert cast(y) == cast(map(func, x))
...
@mark.parametrize("cls", [CIterable, CList, CSet, CFrozenSet])
@given(
    data=data(), key=none() | int_to_int_funcs, reverse=booleans(),
)
def test_sorted(
    cls: Type,
    data: DataObject,
...
@mark.parametrize("cls", [CIterable, CList, CSet, CFrozenSet])
@given(
    data=data(), initial=integers() | just(sentinel),
)
def test_reduce(
    cls: Type, data: DataObject, initial: Union[int, Sentinel],
) -> None:
    x, cast = data.draw(lists_or_sets(cls, integers()))
...
@given(
    start=integers(), step=integers(), n=small_ints,
)
...
@given(x=lists(integers()), times=integers(), n=small_ints)
...
@given(x=lists(integers()), xs=lists(lists(integers())), n=small_ints)
...
def test_take(x: List[int], n: int) -> None:
    y = CIterable(x).take(n)
    assert isinstance(y, CIterable)
...
@given(x=small_lists(integers()))
@settings(deadline=None)
def test_pmap(x: List[int]) -> None:
    y = CIterable(x).pmap(neg, processes=1)
    assert isinstance(y, CIterable)
    assert list(y) == list(map(neg, x))
...
def test_iterdir(cls: Type, data: DataObject, use_path: bool) -> None:
    x, func = data.draw(
        lists_or_sets(
            cls, text(alphabet=ascii_lowercase, min_size=1), unique=True,
        ),
    )
