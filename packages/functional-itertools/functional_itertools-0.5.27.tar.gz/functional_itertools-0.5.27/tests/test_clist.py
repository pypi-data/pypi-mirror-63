...
def test_range(
    ...
    x = CList.range(start, *args)
    assert isinstance(x, CList)
    assert x == list(range(start, *args))
...
def test_pmap(x: List[str]) -> None:
    y = CList(x).pmap(neg, processes=1)
    assert isinstance(y, CList)
    assert y == list(map(neg, x))
