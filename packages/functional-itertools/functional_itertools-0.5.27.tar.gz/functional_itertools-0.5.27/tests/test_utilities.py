...
def lists_or_sets(
    ...
    *,
    ...
    unique: bool = False,
) -> SearchStrategy[Tuple[Type, Union[List[_T], Set[_T], FrozenSet[_T]]]]:
    if cls is CIterable:
        func, strategy = list, partial(lists, unique=unique)
    elif cls is CList:

        def identity(x: _T) -> _T:
            return x

        func, strategy = identity, partial(lists, unique=unique)
    elif cls is CSet:
        func, strategy = set, sets
    elif cls is CFrozenSet:
        func, strategy = frozenset, frozensets
    else:
        raise TypeError(cls)
    return tuples(just(func), strategy(elements, max_size=max_size))
