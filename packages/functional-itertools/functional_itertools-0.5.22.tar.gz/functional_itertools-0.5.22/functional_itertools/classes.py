...
class CIterable(Iterable[_T]):
    ...
    def tee(self: "CIterable[_T]", n: int = 2) -> "CIterable[Iterator[_T]]":
        return CIterable(tee(self._iterable, n))
    ...
    def pmap(
        self: "CIterable[_T]", func: Callable[[_T], _U],
    ) -> "CIterable[_U]":
        with Pool() as pool:
...
class CList(List[_T]):
    ...
    def reduce_as_iterable(
        ...
        return self.iter().reduce_as_iterable(func, initial=initial)

    # multiprocessing

    def pmap(self: "CList[_T]", func: Callable[[_T], _U]) -> "CList[_U]":
        return self.iter().pmap(func).list()
...
class CSet(Set[_T]):
    ...
    def pmap(self: "CSet[_T]", func: Callable[[_T], _U]) -> "CSet[_U]":
        return self.iter().pmap(func).set()
...
class CFrozenSet(FrozenSet[_T]):
    ...
    def pmap(
        self: "CFrozenSet[_T]", func: Callable[[_T], _U],
    ) -> "CFrozenSet[_U]":
        return self.iter().pmap(func).frozenset()
