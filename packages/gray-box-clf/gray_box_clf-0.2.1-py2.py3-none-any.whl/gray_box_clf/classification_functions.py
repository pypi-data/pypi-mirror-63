...
def confedince_threshold_auto(y_test, y_prob, min_thr=0.01, steps_thr=0.001, max_thr=1.0, def_thresh=0):
    ...
    for thr in thresholdes:
        ...
        y_hat[(thr < y_prob) & (y_prob < (thr + def_thresh))] = 888
    ...
    y_hat_best[(threshold[np.argmax(acc)] < y_prob) & (y_prob < (threshold[np.argmax(acc)] + def_thresh))] = 888
...
def best_result_with_confidence(y_test, y_prob, best_threshold, def_thresh):
    ...
    y_predcit_, y_test_, samples_sacrifice_rate, missed_classified_index = best_result_with_confidence(y_test, y_predcit, y_prob, best_threshold,
                                                                                                       def_thresh)
    """
    y_predcit = np.zeros(len(y_test))
    y_predcit[y_prob < best_threshold] = 0
    y_predcit[y_prob > best_threshold] = 1
    y_predcit[(best_threshold < y_prob) & (y_prob < (best_threshold + def_thresh))] = 888
    ...
    return y_predcit_, y_test_, samples_sacrifice_rate, missed_classified_index
...
class PraperClassified:
    ...
    def grid_search(self, cv=5, n_features_list=["all"], conf_interval_width=0, random_state=10, validation_size=0.33):
    ...
    def leave_one_out(self, sensitive_label=1, confidence_interval=False):
        ...
        if confidence_interval:
            self.y_predict_, self.y_test_, self.samples_sacrifice_rate, self.missed_classified_index = best_result_with_confidence(self.y, y_hat_prob,
                                                                                                                                   self.best_treshold_conf_,
                                                                                                                                   self.conf_interval_width)
            y_hat_prob = np.delete(y_hat_prob, self.missed_classified_index, 0)
            self.fpt_, self.tpr_, self.threshold_ = roc_curve(np.delete(self.y, self.missed_classified_index, 0), y_hat_prob)
            self.auc_ = auc(self.fpt_, self.tpr_)
    ...
    def train_test_split(self, test_size=0.3, sensitive_label=1, confidence_interval=False):
        ...
        if confidence_interval:
            self.y_predict_, self.y_test_, self.samples_sacrifice_rate, self.missed_classified_index = best_result_with_confidence(self.y_test_,
                                                                                                                                   y_hat_prob,
                                                                                                                                   self.best_treshold_conf_,
                                                                                                                                   self.conf_interval_width)
            y_hat_prob = np.delete(y_hat_prob, self.missed_classified_index, 0)
    ...
    def train_test_custom(self, train_indexs=[1, 2, 3, 4], sensitive_label=1, confidence_interval=False):
