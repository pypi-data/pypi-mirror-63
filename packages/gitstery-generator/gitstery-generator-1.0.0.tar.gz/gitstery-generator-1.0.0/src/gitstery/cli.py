...
from click import group, pass_context, argument, option, prompt, confirm, echo, secho, IntRange
...
@cli.command()
@argument('repo_dir', envvar='GITSTERY_TEMP_DIR', type=Path)
...
def generate(ctx, repo_dir, force, seed_value, chosen_phases, no_phases, no_solution, push_remote,
    ...
    repo_root.joinpath('instructions.txt').write_text(wrap_paragraphs(instructions) + '\n')
