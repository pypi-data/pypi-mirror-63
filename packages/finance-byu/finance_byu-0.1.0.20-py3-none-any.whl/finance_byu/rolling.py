...
def roll_sum(input,win,minp,errors='raise'):
    ...
    if eflg:
        r = pd.DataFrame(np.nan,columns=input.columns,index=input.index)
    else:
        r = pd.DataFrame(_roll_sum(input.values,win,minp),input.index)
    return r
...
def roll_mean(input,win,minp,errors='raise'):
    ...
    if eflg:
        ...
        r = pd.DataFrame(_roll_mean(input.values,win,minp),input.index)
    return r
...
def roll_var(input,win,minp,ddof=1,errors='raise'):
    ...
    if eflg:
        ...
        r = pd.DataFrame(_roll_var(input.values,win,minp,ddof=ddof),input.index)
    return r
...
def roll_std(input,win,minp,ddof=1,errors='raise'):
    ...
    if eflg:
        ...
        r = pd.DataFrame(_roll_std(input.values,win,minp,ddof=ddof),input.index)
    return r
...
def _assertions(data,yvar,xvar,intercept,backend,roll,residuals,ddof,predispatch,flagop):
...
def rolling_multiple(data,yvar,xvar,roll,intercept=True,residuals=False,backend='loky',ddof='default',predispatch='all',flagop='nan'):
    """
    ...
    _assertions(data,yvar,xvar,intercept,backend,roll,residuals,ddof,predispatch,flagop)
    ...
    if dataflag:
        ...
        if flagop=='nan':
            if residuals == False:
                ...
                d = pd.DataFrame(np.NaN,index=[i for i in range(count)],columns=xvar+['resid'])
            elif residuals == 'residvol':
                d = pd.DataFrame(np.NaN,index=[i for i in range(count)],columns=xvar+['resid','residvol'])
