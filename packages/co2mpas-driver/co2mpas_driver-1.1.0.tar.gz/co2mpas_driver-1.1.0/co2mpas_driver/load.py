...
def load_vehicle_db(db_path):
    ...
    d = df['gearbox_type'] == 'automatic'
    # d |= df['gearbox_type'] == 'single-speed fixed gear'
    df['transmission'] = np.where(d, 'automatic', 'manual')
    df.loc[b, ['transmission']] = np.nan
    df['driveline_efficiency'] = np.where(d, .9, .93)
...
def get_vehicle_inputs(vehicle_id, vehicle_db):
    ...
    :rtype: dict
    """
    return sh.combine_dicts(vehicle_db[vehicle_id], dict(vehicle_id=vehicle_id))
