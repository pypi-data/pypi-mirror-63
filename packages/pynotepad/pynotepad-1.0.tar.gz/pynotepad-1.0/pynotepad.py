...
def _is_pythonfile(filename):
    pythonext=".py",".pyc",".pyw" #ext:文件扩展名
...
class Editor(Tk):
    TITLE="PyNotepad"
    encodings="ansi","utf-8","utf-16","utf-32","gbk"
    ICON="notepad.ico"
    NORMAL_CODING="utf-8"
    ...
    def update_status(self,event=None):
        if self.isbinary:
            try:
                ...
                self.charmap.insert(INSERT,bytes_to_charmap(data))
...
是否使用二进制模式打开?"""%self.coding.get())
    ...
    def change_title(self,running=False):
        newtitle="简易文本编辑器 - "+os.path.split(self.filename)[1]+\
