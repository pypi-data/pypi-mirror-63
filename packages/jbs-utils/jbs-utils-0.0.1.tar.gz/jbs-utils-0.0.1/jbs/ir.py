...
def scope_clear():
    global _PROPERTIES_ALIASES, _PROPERTIES_ALIASES, _SCOPE_PROPERTIES, _SCOPE_BOOKS,\
        _SCOPE_AUTHORS, _SCOPE_SECTIONS, _SCOPE_TYPE

    _SCOPE_TYPE = 'jbo:Text'
    _SCOPE_SECTIONS.clear()
    _SCOPE_AUTHORS.clear()
    _SCOPE_BOOKS.clear()
    _SCOPE_PROPERTIES.clear()
    _PROPERTIES_ALIASES.clear()
    _PROPERTIES_FILTERS.clear()


def scope_add_property(prop: str):
    """
    ...
    a property name from the ontology! (e.g., 'book').
    ...
    :return:
    """
    global _SCOPE_PROPERTIES, _PROPERTIES_ALIASES, _PROPERTIES_FILTERS
    ...
    if has_alias:
        _PROPERTIES_ALIASES[prop] = alias
    if has_filter:
        _PROPERTIES_FILTERS[prop] = _filter

    if prop not in _SCOPE_PROPERTIES:
        _SCOPE_PROPERTIES.append(prop)


def scope_add_properties(properties: list):
    global _SCOPE_PROPERTIES
    for prop in properties:
        scope_add_property(prop)


def scope_add_book(book_label: str):
    global _SCOPE_BOOKS
    book_uri = _get_uri('jbo:Book', _escape(book_label))

    if book_uri not in _SCOPE_BOOKS:
        _SCOPE_BOOKS.append(book_uri)


def scope_add_section(section_label: str):
    global _SCOPE_SECTIONS
    if len(_SCOPE_BOOKS) != 1:
        raise Exception('A SINGLE book should be added to scope before adding a section.')

    book_uri = _SCOPE_BOOKS[0]
    section_uri = _get_section_uri(book_uri, _escape(section_label))

    if section_uri not in _SCOPE_SECTIONS:
        _SCOPE_SECTIONS.append(section_uri)


def scope_add_author(author_label: str):
    global _SCOPE_AUTHORS
    author_uri = _get_uri('jbo:Person', _escape(author_label))

    if author_uri not in _SCOPE_AUTHORS:
        _SCOPE_AUTHORS.append(author_uri)


def scope_set_type(class_type: str):
    """
...
def get_tanach_perushim(torah_only=False, section_heb=None) -> pd.DataFrame:
    ...
    scope_add_properties(properties)
...
def fetch(debug=False) -> pd.DataFrame:
    """
    Returns the subjects specified in the scope. The default type of the subjects is jbo:Text
    and may be overriden by scope_add_type().
    :return:
    """

    if not _SCOPE_PROPERTIES:  # is empty
        print("Warning: no properties in scope.")

    book_values = _get_as_values('book', _SCOPE_BOOKS)
    author_values = _get_as_values('author', _SCOPE_AUTHORS)
    section_values = _get_as_values('section', _SCOPE_SECTIONS)

    # should be refactored...
    props = ''
    columns = []
    for prop in _SCOPE_PROPERTIES:
        alias = None if prop not in _PROPERTIES_ALIASES else _PROPERTIES_ALIASES[prop]
        _filter = None if prop not in _PROPERTIES_FILTERS else _PROPERTIES_FILTERS[prop]
        ...
        if '#' not in prop:  # not a nested property
            ...
            prop_split = split_nested_property_string(prop)
    ...
    query = """SELECT ?uri %s WHERE {
        ...
        %s
        %s
        }""" % (columns_header, book_values if _SCOPE_BOOKS else '', author_values if _SCOPE_AUTHORS else '',
                section_values if _SCOPE_SECTIONS else '', _SCOPE_TYPE, '?uri jbo:book ?book.' if _SCOPE_BOOKS else '',
                '?book jbo:author ?author.' if _SCOPE_AUTHORS else '',
                '?uri jbo:within ?section.' if _SCOPE_SECTIONS else '', props)
    ...
    # print(query)
    return run_query(query, pretty=pretty, debug=debug)
...
def split_nested_property_string(nested_string: str):
    """
...
def run_query(query: str, pretty=[], set_limit: int = 0, set_offset: int = 0, debug=False) -> pd.DataFrame:
    """
    ...
    :param set_offset: results offset. In case of '0' the is no offset
    :return:
    """
    ...
    sparql_wrapper = SPARQLWrapper(_ENDPOINT_URI)
    sparql_wrapper.addDefaultGraph(_GRAPH_URI)
