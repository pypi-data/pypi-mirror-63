...
def get_field_beam(beam: Beam, acc: Accelerator, type: str,
    ...
    if type == 'E':
        ...
        return k_e*q*Ex/1e6, k_e*q*Ey/1e6, k_e*q*Ez/1e6     # MV/m
    if type == 'B':
        Bx, By, Bz = sum_field_particles(x, y, z, acc.z_start, acc.z_stop)
        return k_m*q*Bx, -k_m*q*By, 0*Bz                    # T
...
class Simulation:
    ...
    def track(self, *, n_files: int=20) -> None:
        ...
        P0 = m*1e3*c*1e2*c*1e2 / (e*1e6)
        E0 = m*1e3*c*1e2 / (q*dt) * 1e6 / (c*1e2)   # from MV/m
        B0 = m*1e3 / (q*dt) * 1e4 / (c*1e2) 
