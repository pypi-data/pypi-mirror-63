...
def get_field_beam(beam: Beam, acc: Accelerator, type: str,
    ...
    if type == 'E':
        ...
        return ke*q*Ex/1e6, ke*q*Ey/1e6, ke*q*Ez/1e6
    if type == 'B':
...
class Simulation:
    ...
    def track(self, *, n_files: int=20) -> None:
        ...
        P0 = m*1e3 * c*100 * c*100 / (e*(c*100/10)) / 1e6
        E0 = m*1e3 * c*100 / (q*(c*100/10) * dt) / 1e6 / (1e6/(c*100))
        B0 = m*1e3 * c*100 / (q * (c*100/10) * dt) / (1e4/(c*100))
