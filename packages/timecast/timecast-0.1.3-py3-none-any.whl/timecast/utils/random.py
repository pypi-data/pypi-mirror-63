...
def set_key(key=None):
    ...
    assert type(key) == int
    timecast.GLOBAL_RANDOM_KEY = jax.random.PRNGKey(key)
...
def generate_key():
    ...
    Returns:
        Random random key
    """
    key, subkey = jax.random.split(timecast.GLOBAL_RANDOM_KEY)
    timecast.GLOBAL_RANDOM_KEY = key
    return subkey
...
def get_global_key():
    ...
    return timecast.GLOBAL_RANDOM_KEY
