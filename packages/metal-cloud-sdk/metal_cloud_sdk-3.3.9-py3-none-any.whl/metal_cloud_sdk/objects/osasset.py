...
class OSAsset(object):
	...
	"""
	Names of the secrets used on this OSAsset. Secrets will only be used if the
	os_asset_mime_type is text/plain and the OSAsset contents are stored and not
	served from a remote URL.
	"""
	os_asset_secret_names = [];

	"""
	Names of the secrets used on this OSAsset. Variables will only be used if
	the os_asset_mime_type is text/plain and the OSAsset contents are stored and
	not served from a remote URL.
	"""
