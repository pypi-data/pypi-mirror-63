...
class Agent:
    ...
    def verbose_line(self,env,episodes,verbose,verbose_freq,silent_plot,save_figure,plot_freq):
        if verbose and ((isinstance(verbose_freq,float) and self.episode % round(verbose_freq*episodes) == 0) or (isinstance(verbose_freq,list) and self.episode in verbose_freq)):
            if self.method == 'tabular':
                Q = self.memory.Q
            ...
            if (isinstance(plot_freq,(float,int)) and self.episode > 0 and self.episode % round(plot_freq*episodes) == 0) or (isinstance(plot_freq,list) and self.episode in plot_freq):
