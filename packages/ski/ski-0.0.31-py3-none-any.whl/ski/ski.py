...
import warnings
from ski.util import parent_directory,create_folder
import cmocean
...
class Metrics:
    ...
    def total_regret(self,env,total_V,episodes):
        return episodes*env.memory.V[env.start_state]-total_V
    ...
    def plot_Q(self,env,Q_pred,name,episodes,silent_plot,save_figure):
        ...
        if not silent_plot:
            plt.show()
        if save_figure:
            path = parent_directory(0)+rf'\{name}\Q\episode_{episodes}'
            create_folder(path)
    ...
    def plot_V(self,env,Q_pred,name,episodes,silent_plot,save_figure):
        ...
        if not silent_plot:
            plt.show()
        if save_figure:
            path = parent_directory(0)+rf'\{name}\V\episode_{episodes}'
            create_folder(path)
            plt.savefig(path+r'\fig.svg',bbox_inches='tight',transparent=True)
        plt.close('all')
    ...
    def plot_A(self,env,Q_pred,name,episodes,silent_plot,save_figure):
        ...
        if not silent_plot:
            plt.show()
        if save_figure:
            path = parent_directory(0)+rf'\{name}\A\episode_{episodes}'
            create_folder(path)
            plt.savefig(path+r'\fig.svg',bbox_inches='tight',transparent=True)
        plt.close('all')
...
class Agent:
    ...
    def verbose_line(self,env,episodes,verbose,verbose_freq,silent_plot,save_figure,plot_freq):
        if verbose and self.episode % round(verbose_freq*episodes) == 0:
            ...
            total_regret = self.metrics.total_regret(env,self.performance_memory.total_V,self.episode)
            regret = total_regret/self.episode
            print(f'episode = {self.episode} {self.name} rmse_Q = {rmse_Q} rmse_V = {rmse_V} rmse_A = {rmse_A} total_regret = {total_regret} regret = {regret}')
    ...
    def fit(self,env,episodes,init=None,verbose=True,verbose_freq=.1,silent_plot=False,save_figure=False,plot_freq=1):
        ...
        for _ in range(episodes):
            ...
            for t in env.time_range:
                ...
                state = next_state
                action = next_action
            # G += power(self.memory.Q[state][action],env.N-env.start_state[0])
