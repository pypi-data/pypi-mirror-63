...
class MonteCarlo(Agent):
    def __init__(self,policy,lr,method='tabular',degree=2):
    ...
    def fit(self,env,episodes,init=None,verbose=True,verbose_freq=.1,silent_plot=False,save_figure=False,plot_freq=1):
        ...
        for _ in range(episodes):
            ...
            for t in reversed(env.time_range):
                ...
                if self.method == 'approx':
                    self.memory.W += self.lr*(G-self.memory.approxQ(env.normalize_state(state),env.normalize_action(action)))*self.memory.poly.fit_transform(self.memory.features(env.normalize_state(state),env.normalize_action(action))).flat
        ...
        for _ in range(episodes):
            ...
            for t in env.time_range:
                ...
                if rand:
                    ...
                    if self.method == 'approx':
                        self.memory2.W += self.lr*(U-self.memory2.approxQ(env.normalize_state(state),env.normalize_action(action)))*self.memory2.poly.fit_transform(self.memory2.features(env.normalize_state(state),env.normalize_action(action))).flat
                state = next_state
            G += power(env.gamma,env.N-env.start_state[0])*env.terminal_reward(state)
            self.performance_memory.G_dict[self.episode] = G
            self.verbose_line(env,episodes,verbose,verbose_freq,silent_plot,save_figure,plot_freq)
