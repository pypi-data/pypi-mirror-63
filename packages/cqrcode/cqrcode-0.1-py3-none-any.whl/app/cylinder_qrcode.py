...
@Version    :   1.0
@Desciption :
            内含了二维码图片处理的函数调用，包括了对于二维码编码以及图片二值化等调用。
...
from app.randomStr import random_bit
...
def expand_figure(multiplying_power=1.250, filePath=dataPath+'_128.png'):
    ...
    w1, h1 = INPUT_IMAGE.size[0], INPUT_IMAGE.size[1]
    if w1 != h1:
        print('扩展图片失败！')
        return None
    # print(w1, h1)  # 260 260
    ...
    for i in x:
        # 将移动的单位换算为像素
        y.append((math.asin(((r+l)/r)*math.sin(math.atan(i/l))) - math.atan(
            i/l))*r*10-i*10)
    y.reverse()
    # print(len(y), '\n', y)

    # 读入空白图片
    OUT_IMAGE = Image.open(dataPath+'二维码数据模板.png')
    if OUT_IMAGE.size[0] != pixel:
        print('数据模板与输入二维码尺寸不匹配，扩展图片失败!')
        return None
    # 扩展图片尺寸
    OUT_IMAGE = OUT_IMAGE.resize((2*round(y[0])+pixel, pixel), Image.ANTIALIAS)
    # print(OUT_IMAGE.size[0], OUT_IMAGE.size[1])  # 284 260
    w2, h2 = OUT_IMAGE.size[0], OUT_IMAGE.size[1]
    # 向左侧写入数据
    z = 0
    for i in range(0, w1 // 2, 1):
        for j in range(0, h1, 1):
            if img_array[i, j] == (0, 0, 0, 255):
                OUT_IMAGE.putpixel((i - round(y[z])+round(y[0]), j), (0, 0, 0))
        z += 1
    z = 0
    # 向右侧写入数据
    for i in range(w1-1, w1 // 2, -1):
        for j in range(0, h1, 1):
            if img_array[i, j] == (0, 0, 0, 255):
                OUT_IMAGE.putpixel((i + round(y[z])+round(y[0]), j), (0, 0, 0))
        z += 1
    OUT_IMAGE.save(dataPath + '扩展图片.png')
    time.sleep(2)
    OUT_IMAGE = Image.open(dataPath + '扩展图片.png')
    out_img_array = OUT_IMAGE.load()
    # 填补空白
    for i in range(w2):
        for j in range(h2):
            if out_img_array[i, j] == (0, 0, 0, 255) and out_img_array[i + 2, j] == (
                     0, 0, 0, 255):
                OUT_IMAGE.putpixel((i + 1, j), (0, 0, 0))

    OUT_IMAGE.save(dataPath+str(multiplying_power)+'_延展后二维码.png')
    os.remove(dataPath + '扩展图片.png')
    os.remove(filePath)

    # ac = (math.asin(((r+l)/r)*math.sin(math.atan(a/l))) - math.atan(a/l))*r
    # delta： 平面二维码上的某个位点向柱面二维码的水平偏移量
    # delta = ac - a
    # print(delta)


def encode_left(data='', filePath=dataPath + '二维码识别模板.png'):
    """
    向模板中（左侧）写入数据
    :return:
    """
    global data_list_left
    if data == '':
        data = random_bit(length=1345)
    index = 0
    figure = Image.open(filePath)
    figure = figure.convert('RGBA')
    for i in data:
        x = data_list_left[index][0]
        y = data_list_left[index][1]
        if i == '1':
        ...
        index += 1
    fileName = "柱体二维码预生成 " + \
        time.strftime('%Y_%m_%d_%H_%M_%S', time.localtime(time.time())) + ".png"
    figure.save(dataPath + fileName, "png")
    return dataPath + fileName


def encode_right(data='', filePath=dataPath + '二维码识别模板.png'):
    """
    向模板中（右侧）写入数据
    :return:
    """
    global data_list_right
    if data == '':
        data = random_bit(length=1417)
    index = 0
    figure = Image.open(filePath)
    figure = figure.convert('RGBA')
    for i in data:
        x = data_list_right[index][0]
        y = data_list_right[index][1]
        if i == '1':
...
def main():
    ...
    # img = Image.open(dataPath + '二维码识别模板.png')
    # img = img.convert('RGBA')
    # width, height = img.size  # 获取图像尺寸
    expand_figure()
    # stand_point2(dataPath+'result_figure.jpg')
    # deal_fig(threshold=20)
