...
class FactTestSuite(BaseTestSuite):
    ...
    def queue_tests(self) -> None:
        ...
        for name in names:
            self.TestSuite.addTest(FactTestCase(self.SparkUtility, self.TableDataFrame, self.FieldInfo, self.__linkedDimensions, self.DatabaseName, self.TableName, self.TableCount, self.CustomEvents, self.__excludeFromAggregation))
