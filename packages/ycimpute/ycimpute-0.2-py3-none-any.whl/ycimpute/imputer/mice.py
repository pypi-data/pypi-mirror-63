...
class MICE(Solver):
    ...
    def _imputation_round(self, X_filled, visit_indices,missing_mask):
        global imputed_values
        for col in visit_indices:
            x_obs, y_obs, x_mis = self.split(X_filled, col, missing_mask)
            model = self.model
            ...
            X_filled[missing_mask[:, col], col] = imputed_values
        return X_filled
    ...
    def solve(self, X, missing_mask):
        ...
        X_filled = np.array(X.copy())
        visit_idx = self.sort_col(missing_mask)
            ...
            if self.verbose:
                print(
                    "[MICE] Starting imputation round %d/%d, elapsed time %0.3f" % (
                        ...
                        total_rounds,
                        time() - start_t))
            X_filled = self._imputation_round(X_filled, visit_idx, missing_mask)

            if m >= self.n_burn_in:
                results_list.append(X_filled[missing_mask])

        imputed_arrays = np.asarray(results_list)

        # average the imputed values for each feature
        average_imputated_values = imputed_arrays.mean(axis=0)

        X[missing_mask] = average_imputated_values
