...
def get_factor_data(factor, price_data, periods=None, split=3,
    ...
    forward_returns = {delta: -prices.diff(-period) / prices
    ...
    if isinstance(split, int):
        factor_quantile = factor_data.groupby(level=0)['factor'].transform(
            lambda x: pd.qcut(x, split, labels=False, duplicates='drop')) + 1
    elif isinstance(split, (list, tuple, set)):
        factor_quantile = factor_data.groupby(level=0)['factor'].transform(
            lambda x: pd.cut(x, split, labels=False, duplicates='drop')) + 1
        split = len(split) - 1
    else:
        raise ValueError(
            f'Factor `{name}` split type {type(split)} is not supported.')
    factor_data['factor_quantile'] = factor_quantile
    quantiles = [1, split] if long_short else list(range(1, split + 1))
    factor_data['weights'] = factor_data[factor_data['factor_quantile'].isin(
        quantiles)].groupby(level=0)['factor'].transform(
