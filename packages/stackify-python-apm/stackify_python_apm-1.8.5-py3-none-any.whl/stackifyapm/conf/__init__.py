    ...
    def __init__(self, config_dict=None, env_dict=None, inline_dict=None):
        ...
        for field, config_value in self.__class__.__dict__.items():
            ...
            if config_value.env_key and config_value.env_key in env_dict:
                ...
                new_value = inline_dict.get(field) or inline_dict.get(field.upper())
...
def setup_stackifyapm_logging(client):
    ...
    filename = "{}{}#{}-1.log".format(client.config.log_path, host_name, process_id)

    def namer(name):
        num = int(name.split("/")[-1].split(".")[-1] or 0) + 1
        return "{}{}#{}-{}.log".format(client.config.log_path, host_name, process_id, num)
    ...
    try:
        ...
        print("No such file or directory: {}.".format(client.config.log_path))
