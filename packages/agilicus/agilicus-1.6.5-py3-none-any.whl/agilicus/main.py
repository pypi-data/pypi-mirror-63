...
def list_tokens(ctx, org_id, **kwargs):
    output_tokens_list(json.loads(tokens.query_tokens(ctx, org_id=org_id, **kwargs)))
...
def list_users(ctx, organisation, org_id, **kwargs):
    ...
    output_list_users(org_by_id, json.loads(users.query(ctx, org_id, **kwargs)))
...
def update_user(ctx, user_id, org_id, **kwargs):
    output_entry(users.update_user(ctx, user_id=user_id, org_id=org_id, **kwargs))
...
def list_groups(ctx, organisation, org_id, type, **kwargs):
    ...
    output_list_groups(org_by_id, json.loads(users.query(ctx, org_id, type, **kwargs)))
...
def list_sysgroups(ctx, organisation, org_id, **kwargs):
    ...
    output_list_groups(
        org_by_id, json.loads(users.query(ctx, org_id, type="sysgroup", **kwargs)),
    )
...
@click.pass_context
def update_org(
    ctx, org_id, auto_create, issuer, contact_id, subdomain, external_id, **kwargs,
):
    orgs.update(
        ctx,
        org_id,
...
@click.option("--subdomain", default=None)
@click.pass_context
def add_org(ctx, organisation, issuer, contact_id, auto_create, subdomain, **kwargs):
    output_entry(
        orgs.add(
            ctx,
            organisation,
...
def show_environment(ctx, app_id, env_name, org_id, **kwargs):
    output_environment_entries(apps.get_env(ctx, app_id, env_name, org_id, **kwargs))
...
@click.option("--hostname", default=None)
@click.pass_context
def rules_from_csv(ctx, application_id, role_name, file_name, org_id, hostname):
...
def _format_redirects(redirects):
    ...
    for redirect in redirects:
        result.append(redirect.redirect_url)
    return result
...
def list_issuers(ctx, **kwargs):
    ...
    table = PrettyTable(["id", "issuer", "client", "secret", "application", "redirects"])
    table.align = "l"
    for issuer in _issuers:
        if len(issuer.clients):
            ...
            table.add_row([issuer.id, issuer.issuer, "-", "-"])
    print(table)
...
@click.argument("issuer_id", default=None)
@click.argument("client_name", default=None)
