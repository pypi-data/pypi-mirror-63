from . import access, agilicus_api, context
...
def add(ctx, issuer, **kwargs):
    ...
    if not token:
        ...
        token = access_token.get()
    apiclient = context.get_apiclient(ctx, token)
    issuer_model = agilicus_api.Issuer(issuer=issuer)
...
def add_client(
    ...
    if not token:
        ...
        token = access_token.get()
    apiclient = context.get_apiclient(ctx, token)
    client_model = agilicus_api.IssuerClient(
...
def add_redirect(ctx, issuer_id, client_name, redirect_url, **kwargs):
    ...
    redirect_model = agilicus_api.IssuerClientRedirect(redirect_url=redirect_url)
