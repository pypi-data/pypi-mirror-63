...
from . import access, context, response
...
def get_env(ctx, id, env_name, org_id=None, **kwargs):
    ...
    return apiclient.application_api.get_environment(id, env_name, org_id).to_dict()
...
def get_application_services(ctx, org_id=None, **kwargs):
    ...
    if not org_id:
        org_id = context.get_org_id(ctx, token)
    apiclient = context.get_apiclient(ctx, token)
    return apiclient.app_services_api.get_application_services(org_id)
