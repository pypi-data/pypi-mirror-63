    ...
    def handle_verification(self, ctx, request):
        ...
        if score < config.blocking_score:
            ...
            if config.module_mode == px_constants.MODULE_MODE_BLOCKING or should_bypass_monitor:
                ...
                response_function = generate_blocking_response(config, data, headers, status)
            else:
                pass_request = True
        ...
        if config.custom_request_handler:
            ...
            if data and headers and status:
                return generate_blocking_response(self.config, data, headers, status)
...
def generate_blocking_response(config, data, headers, status):
    result = Response(data)
    if headers is not None:
        result.headers = headers
    if status is not None:
        result.status = status
    for _, func in config.response_functions:
        result = func(result)
    return result
