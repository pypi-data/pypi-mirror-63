...
class PxRequestVerifier(object):
    ...
    def verify_request(self, app, ctx, request):
        uri = ctx.uri
        ...
        if not px_cookie_validator.verify(ctx, self.config):
            ...
            if not px_api.verify(ctx, self.config):
                ...
                return True
        return self.handle_verification(app, ctx, request)

    def handle_verification(self, app, ctx, request):
        config = self.config
        ...
        if score < config.blocking_score:
            ...
            if config.module_mode == px_constants.MODULE_MODE_BLOCKING or should_bypass_monitor:
                ...
                response_function = generate_blocking_response(app, data, headers, status)
            else:
                pass_request = True
        ...
        if config.custom_request_handler:
            ...
            if data and headers and status:
                return generate_blocking_response(app, data, headers, status)
...
def generate_blocking_response(app, data, headers, status):
