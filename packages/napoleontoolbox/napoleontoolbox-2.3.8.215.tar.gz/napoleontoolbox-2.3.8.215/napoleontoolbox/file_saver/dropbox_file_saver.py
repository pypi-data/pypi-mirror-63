...
class NaPoleonDropboxConnector(object):
    ...
    def local_features_npy_ovverwrite_and_load_array(self, starting_date = None, ending_date = None, feature_type=None,
                                           ...
                                           features_saving_suffix='', features_names_saving_suffix='',shortcut_features_pkl_file_name = ''):
        ...
        shortcut_df = pd.read_pickle(shortcut_features_file_full_path)
        shortcut_df = shortcut_df[shortcut_df.index >= starting_date]
        shortcut_df = shortcut_df[shortcut_df.index <= ending_date]

        return features, features_names, shortcut_df.values
    ...
    def local_features_npy_save_and_upload(self, starting_date = None, ending_date = None, shortcut_features=None, features=None, features_names=None, feature_type=None, n_past_features = None, local_root_directory='', user='', shortcut_features_saving_suffix='',features_saving_suffix='', features_names_saving_suffix=''):
        features_file_name = starting_date.strftime('%d_%b_%Y')+'_'+ending_date.strftime('%d_%b_%Y')+'_'+user +'_' + feature_type.name + '_' + str(n_past_features) + features_saving_suffix
        local_root_directory = Path(local_root_directory)
        features_full_path = local_root_directory/features_file_name
        ...
        shortcut_features_file_name = starting_date.strftime('%d_%b_%Y')+'_'+ending_date.strftime('%d_%b_%Y')+'_'+user +'_'+ shortcut_features_saving_suffix
        local_root_directory = Path(local_root_directory)
