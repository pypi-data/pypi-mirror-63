...
def volume_weigthed_high_low_vol_perc(data = None, vol_threshold = 0.8, up_trend_threshold=0.8, low_trend_threshold=0.2, contravariant = -1., lag = 2, **kwargs):
    ...
    data['aggregated_high'] =  data['aggregated_high'].map(high)
    data['aggregated_low'] =  data['aggregated_low'].map(low)
    ...
    trend_rank = data['close_ret_pct_rank'][-1]
    vol_rank = data['volu_hi_low_pct_rank'][-1]
