...
class SignalParalleLauncher():
    def __init__(self,drop_token='', dropbox_backup=True, local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', triggers = [True], transaction_costs = [True], normalizations = [False], signal_idiosyncratic_paramaters=[None]):
        self.args = []
        self.counter = 1
        self.seed = 0
    ...
    def updateSignals(self, run, values):
        ...
        try:
            ...
            cursor = sqliteConnection.cursor()
            for i, v in values.iteritems():
                sqlite_update_query = """UPDATE 'parallel_signals' set '""" + run + """' = '""" + str(
    ...
    def saveAll(self,run, strat_results_df):
        ...
        self.saveSignals(run, strat_results_df['reconstituted_perf'])
