...
class FeaturesAssembler(AbstractAssembler):
    ...
    def assembleFeature(self, feature_type, n_past_features, tsfresh_lookback_window):
        ...
        print('Actual min shortcut feature date '+str(min(shortcut_features_df.index)))
        print('Actual max shortcut feature date '+str(max(shortcut_features_df.index)))

        shortcut_features_df = shortcut_features_df[shortcut_features_df.index >= self.starting_date]
        shortcut_features_df = shortcut_features_df[shortcut_features_df.index <= self.ending_date]
        print('Max shortcut date '+str(max(shortcut_features_df.index)))
        print('Min shortcut date '+str(min(shortcut_features_df.index)))
        ...
        self.dbx.local_features_npy_save_and_upload(starting_date = self.starting_date,ending_date = self.running_date, shortcut_features=shortcut_features, features = features,features_names= predictor_names,feature_type=feature_type, n_past_features = n_past_features , local_root_directory= self.local_root_directory, user = self.user, features_saving_suffix=self.features_saving_suffix, features_names_saving_suffix=self.features_names_saving_suffix)
