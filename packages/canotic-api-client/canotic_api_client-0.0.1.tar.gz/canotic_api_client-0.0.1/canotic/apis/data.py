    ...
    def upload_data(self, path: str, description: str, mimeType: str, file: BinaryIO) -> dict:
        ...
        try:
            ...
            if resp.status_code == 200 or resp.status_code == 201:
                ...
                raise CanoticStorageError(
                    f'File {str(file)} referenced by dataset {dataset["path"]} couldn\'t be uploaded to Canotic Storage')
        except Exception as e:
            raise CanoticStorageError(
                f'File {str(file)} referenced by dataset {dataset["path"]} couldn\'t be uploaded to Canotic Storage Error: {str(e)}')
