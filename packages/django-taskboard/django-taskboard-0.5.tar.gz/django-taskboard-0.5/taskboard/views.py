...
def read_admins_file():
    if not path.exists('./taskboard'):
        os.mkdir('./taskboard')

    if not path.exists('./taskboard/sessions'):
        os.mkdir('./taskboard/sessions')

    if not path.exists('./taskboard/sessions/admins.txt'):
        with open('./taskboard/sessions/admins.txt', 'w', encoding='UTF-8') as file:
            file.write('admin;admin;Administrator')

    if not settings.SESSION_ENGINE:
        settings.SESSION_ENGINE = 'django.contrib.sessions.backends.file'

    if not settings.SESSION_FILE_PATH:
        settings.SESSION_FILE_PATH = './taskboard/sessions'
    ...
    with open('./taskboard/sessions/admins.txt', encoding='UTF-8') as file:
        ...
        return {username: [password, name] for line in lines for username, password, name in [line.split(';')]}
