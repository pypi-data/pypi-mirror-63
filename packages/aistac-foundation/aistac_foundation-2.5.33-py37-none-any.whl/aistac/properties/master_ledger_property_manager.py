...
class MasterLedgerPropertyManager(AbstractPropertyManager):
    ...
    def get_ledger_pm(self, manager: str, task: str) -> dict:
        ...
        if self.is_key(self.join(self.KEY.component_key, manager, task)):
            uri = self.get(self.join(self.KEY.component_key, manager, task, 'uri'))
            module = self.get(self.join(self.KEY.component_key, manager, task, 'module_name'))
            handler = self.get(self.join(self.KEY.component_key, manager, task, 'handler'))
            version = self.get(self.join(self.KEY.component_key, manager, task, 'version'))
            kwargs = self.get(self.join(self.KEY.component_key, manager, task, 'kwargs'))
    ...
    def set_ledger_pm(self, property_manager: AbstractPropertyManager):
        ...
        self.set(self.join(self.KEY.component_key, manager, task, 'uri'), connector_contract.uri_raw)
        self.set(self.join(self.KEY.component_key, manager, task, 'module_name'), connector_contract.module_name)
        self.set(self.join(self.KEY.component_key, manager, task, 'handler'), connector_contract.handler)
        self.set(self.join(self.KEY.connectors_key, manager, task, 'version'), connector_contract.version)
        self.set(self.join(self.KEY.component_key, manager, task, 'kwargs'), connector_contract.kwargs)
