...
class ConnectorContractTest(unittest.TestCase):
    ...
    def test_connector_environ(self):
        os.environ['AISTAC_TESTER'] = 'work/tester'
        ...
        sc = ConnectorContract('s3://${USERNAME}:${PASSWORD}@${BUCKET}:8080/${AISTAC_TESTER}/uri?encoding=${ENCODING}',
                               '${MODULE_ROOT}.handlers.python_handlers',
                               '${HANDLER}',version='v${VERSION}', kwarg1='${AISTAC_TESTER}')
        # raw
        self.assertEqual(sc.raw_uri, 's3://${USERNAME}:${PASSWORD}@${BUCKET}:8080/${AISTAC_TESTER}/uri?encoding=${ENCODING}')
        ...
        self.assertDictEqual(sc.raw_kwargs, {'kwarg1': '${AISTAC_TESTER}'})
        # parsed
        self.assertEqual(sc.uri, 's3://dtester:pa22w0rd@aistac:8080/work/tester/uri?encoding=Latin1')
        ...
        self.assertDictEqual(sc.query, {'encoding': 'Latin1'})
        self.assertEqual(sc.schema, 's3')
        self.assertEqual(sc.username, 'dtester')
        os.environ.pop('AISTAC_TESTER')
