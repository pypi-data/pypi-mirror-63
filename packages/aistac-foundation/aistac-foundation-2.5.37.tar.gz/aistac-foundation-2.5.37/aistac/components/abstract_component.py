...
class AbstractComponent(ABC):
    ...
    def _add_templates(cls, property_manager: AbstractPropertyManager, save: bool=None, source_path: str=None,
        ...
        if isinstance(_source_path, str):
            source_kwargs = ConnectorContract.parse_query(_source_path)
            _source_path = ConnectorContract.parse_address(_source_path)
        if isinstance(_persist_path, str):
            persist_kwargs = ConnectorContract.parse_query(_persist_path)
            _persist_path = ConnectorContract.parse_address(_persist_path)
    ...
    def from_environ(cls, task_name: str, uri_pm_path: str, pm_file_type: str=None, pm_module: str=None,
        ...
        return cls.from_uri(task_name=task_name, uri_pm_path=uri_pm_path, pm_file_type=pm_file_type,
                            ...
                            template_source_kwargs=template_source_kwargs,
                            template_persist_kwargs=template_persist_kwargs)
    ...
    def from_env(cls, task_name: str, default_save=None, **kwargs):
        ...
        if pm_kwargs is None and isinstance(pm_uri, str):
            pm_kwargs = ConnectorContract.parse_query(pm_uri)
            pm_uri = ConnectorContract.parse_address(pm_uri)
        ...
        if isinstance(source_path, str):
            source_kwargs = ConnectorContract.parse_query(source_path)
            source_path = ConnectorContract.parse_address(source_path)
        persist_kwargs = None
        if isinstance(persist_path, str):
            persist_kwargs = ConnectorContract.parse_query(persist_path)
            persist_path = ConnectorContract.parse_address(persist_path)
