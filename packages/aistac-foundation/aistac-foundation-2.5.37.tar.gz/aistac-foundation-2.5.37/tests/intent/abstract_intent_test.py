...
class IntentModelTest(unittest.TestCase):
    ...
    def test_run_intent_pipeline_levels(self):
        ...
        result = model.run_intent_pipeline(canonical=data, inplace=False, levels=0)
        self.assertDictEqual({'A': [1, 4, 7]}, result)
        result = model.run_intent_pipeline(canonical=data, inplace=False, levels=1)
        self.assertDictEqual({'a': [1,4,7], 'b': [4,5,9]}, result)
        result = model.run_intent_pipeline(canonical=data, inplace=False, levels=[0,1,2])
        self.assertDictEqual({'a': [1,4,7]}, result)
