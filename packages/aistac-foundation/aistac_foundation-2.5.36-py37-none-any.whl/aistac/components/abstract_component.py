...
class AbstractComponent(ABC):
    ...
    literal blocks::
        ...
        def from_uri(cls, task_name: str, uri_pm_path: str, pm_file_type: str=None, pm_module: str=None,
                     ...
                     template_source_path: str=None, template_persist_path: str=None,
                     template_source_module: str=None, template_persist_module: str=None,
                     template_source_handler: str=None, template_persist_handler: str=None,
                     template_source_kwargs: dict=None, template_persist_kwargs: dict=None):
    ...
    _ENV_PM_KWARGS = 'AISTAC_PM_KWARGS'
    ...
    _ENV_DEFAULT_KWARGS = 'AISTAC_DEFAULT_MODULE'
    _ENV_DEFAULT_KWARGS_SOURCE = 'AISTAC_DEFAULT_SOURCE_KWARGS'
    _ENV_DEFAULT_KWARGS_PERSIST = 'AISTAC_DEFAULT_PERSIST_KWARGS'
    ...
    def from_uri(cls, task_name: str, uri_pm_path: str, pm_file_type: str=None, pm_module: str=None,
        ...
        See class inline docs for an example method
         ...
         :param template_source_kwargs: (optional) additional kwargs to pass to the source handler
         :param template_persist_kwargs: (optional) additional kwargs to pass to the persist handler
    ...
    def _add_templates(cls, property_manager: AbstractPropertyManager, save: bool=None, source_path: str=None,
                       ...
                       source_handler: str=None,  persist_handler: str=None, source_kwargs: dict=None,
        ...
        """ add a template Connector Contract to the properties
         ...
         :param source_kwargs: (optional) additional kwargs to pass to the source handler
         :param persist_kwargs: (optional) additional kwargs to pass to the persist handler
        """
        ...
        _source_kwargs = os.environ.get(f'AISTAC_{manager}_SOURCE_KWARGS', None)
        _persist_kwargs = os.environ.get(f'AISTAC_{manager}_PERSIST_KWARGS', None)
        ...
        if _source_kwargs is None:
            _source_kwargs = _source_kwargs if isinstance(_source_kwargs, dict) else {}
        ...
        if _persist_kwargs is None:
            _persist_kwargs = _source_kwargs if isinstance(_source_kwargs, dict) else {}
    ...
    def from_environ(cls, task_name: str, uri_pm_path: str, pm_file_type: str=None, pm_module: str=None,
        ...
        This method calls the Factory Method 'from_uri(...)' returning the initialised class instance
         ...
         :param pm_kwargs: (optional) a dictionary of kwargs to pass to the properties
         :param default_save: (optional) if the configuration should be persisted. default to 'True'
         :param template_source_path: (optional) a default source root path for the source canonicals
         :param template_persist_path: (optional) a default source root path for the persisted canonicals
         :param template_source_module: (optional) a default module package path for the source handlers
        ...
        for label in re.findall(pattern, " ".join([uri_pm_path, str(pm_module), str(pm_handler), str(pm_kwargs),
                                                   str(template_source_path), str(template_persist_path),
                                                   str(template_source_module), str(template_persist_module),
                                                   str(template_source_handler), str(template_persist_handler),
                                                   str(template_source_kwargs), str(template_persist_kwargs)])):
            if label not in os.environ.keys():
                raise EnvironmentError(f"The environment variable '{label}' has not been set")
        uri_pm_path = re.sub(pattern, lambda m: os.getenv(m.group(1), uri_pm_path), uri_pm_path)
        if isinstance(pm_module, str):
            pm_module = re.sub(pattern, lambda m: os.getenv(m.group(1), pm_module), pm_module)
        if isinstance(pm_handler, str):
            pm_handler = re.sub(pattern, lambda m: os.getenv(m.group(1), pm_handler), pm_handler)
        if isinstance(pm_kwargs, dict):
            for key, value in pm_kwargs.items():
                pm_kwargs.update({re.sub(pattern, lambda m: os.getenv(m.group(1), key), key):
                                  re.sub(pattern, lambda m: os.getenv(m.group(1), value), value)})
        ...
        if isinstance(template_persist_handler, str):
            template_persist_handler = re.sub(pattern, lambda m: os.getenv(m.group(1), template_persist_handler),
                                              template_persist_handler)
        if isinstance(template_source_kwargs, dict):
            for key, value in template_source_kwargs.items():
                template_source_kwargs.update({re.sub(pattern, lambda m: os.getenv(m.group(1), key), key):
                                               re.sub(pattern, lambda m: os.getenv(m.group(1), value), value)})
        if isinstance(template_persist_kwargs, dict):
            for key, value in template_persist_kwargs.items():
                template_persist_kwargs.update({re.sub(pattern, lambda m: os.getenv(m.group(1), key), key):
                                                re.sub(pattern, lambda m: os.getenv(m.group(1), value), value)})
    ...
    def from_env(cls, task_name: str, default_save=None, **kwargs):
        ...
        pm_kwargs = kwargs if isinstance(kwargs, dict) else os.environ.get(cls._ENV_PM_KWARGS, None)
        ...
        source_kwargs = os.environ.get(cls._ENV_DEFAULT_KWARGS_SOURCE, os.environ.get(cls._ENV_DEFAULT_KWARGS, None))
        persist_kwargs = os.environ.get(cls._ENV_DEFAULT_KWARGS_PERSIST, os.environ.get(cls._ENV_DEFAULT_KWARGS, None))
        return cls.from_environ(task_name=task_name, uri_pm_path=pm_uri, pm_file_type=pm_file_type, pm_module=pm_module,
                                ...
                                template_source_path=source_path, template_persist_path=persist_path,
                                template_source_module=source_module, template_persist_module=persist_module,
                                template_source_handler=source_handler, template_persist_handler=persist_handler,
                                template_source_kwargs=source_kwargs, template_persist_kwargs=persist_kwargs)
    ...
    def report_environ(self):
        ...
        for environ in [self._ENV_PM_PATH, self._ENV_PM_TYPE, self._ENV_PM_MODULE, self._ENV_PM_HANDLER,
                        ...
                        self._ENV_DEFAULT_HANDLER_SOURCE, self._ENV_DEFAULT_HANDLER_PERSIST,
                        self._ENV_DEFAULT_KWARGS, self._ENV_DEFAULT_KWARGS_SOURCE, self._ENV_DEFAULT_KWARGS_PERSIST,
                        f'AISTAC_{manager}_PATH', f'AISTAC_{manager}_SOURCE_PATH', f'AISTAC_{manager}_PERSIST_PATH',
                        f'AISTAC_{manager}_MODULE',
                        ...
                        f'AISTAC_{manager}_KWARGS',
                        f'AISTAC_{manager}_SOURCE_KWARGS', f'AISTAC_{manager}_PERSIST_KWARGS',
                        ]:
    ...
    def add_connector_from_template(self, connector_name: str, uri_file: str, template_name: str,  save: bool=None,
        ...
        uri = os.path.join(template.uri_raw, uri_file)
