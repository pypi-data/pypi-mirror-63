    ...
    def play(self, start_at=0, event_played_callback=None, key_event_catcher=None):
        ...
        for event in self._event_list.events[first_event:]:
            ...
            if self._playing:
                ...
                if key_event_catcher and not isinstance(event, Delay):
...
class SnapshotPlayer(QObject):
    ...
    def play(self):
        """ Reperforms all the recorded snapshots and performs the diff """
    ...
    def show_diff(self, snap_id):
        ...
        if orig and res:
            ...
            self._diff_win = Diff(orig.pixmap, res.pixmap, diff.pixmap if diff else None, None)
            self._diff_win.show()
        else:
            # something's wrong in the matrix
            message = self.tr("Some of the screenshot are missing.\n"
                "Don’t forget to click the play button after playing back the events.")
