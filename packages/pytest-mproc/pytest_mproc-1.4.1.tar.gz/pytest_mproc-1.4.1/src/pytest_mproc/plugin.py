...
def pytest_runtestloop(session):
    ...
    if not session.config.getvalue("collectonly") and worker is None:
        try:
            context_managers = [cm for cm, args in _GlobalFixtures.session_contexts if not args or
                                [a for a in args if any([a in item._fixtureinfo.argnames for item in session.items])]]
