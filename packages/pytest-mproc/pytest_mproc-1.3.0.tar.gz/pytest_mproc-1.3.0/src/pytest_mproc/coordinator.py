...
class Coordinator:
    ...
    def _populate_test_q(self):
        ...
        for test in self._tests:
            ...
            if isinstance(test, list):
                self._test_q.put([t.nodeid for t in test], timeout=TIMEOUT_QUEUE_OP)
            else:
                self._test_q.put(test.nodeid, timeout=TIMEOUT_QUEUE_OP)
        for _ in self._processes:
            self._test_q.put(None, TIMEOUT_QUEUE_OP)  # signals end;  close() doesn't actually seem to work like it should :-/
        self._test_q.close()
    ...
    def run(self, session):
        ...
        while any(self._result_qs):
            ...
            for input in inputs:
                items = reader_mapping[input].get(timeout=TIMEOUT_QUEUE_OP)
