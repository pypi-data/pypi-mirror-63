...
class WorkerSession:
    ...
    def _flush(self):
        ...
        if self._buffered_results:
            self._result_q.put(self._buffered_results, timeout=TIMEOUT_QUEUE_OP)
            self._buffered_results = []
            self._timestamp = datetime.utcnow()
...
def main(index, test_q, result_q, num_processes):
    ...
    try:
        def generator():
            try:
                test = test_q.get(timeout=TIMEOUT_QUEUE_OP)
                while test:
                    yield test
                    test = test_q.get(timeout=TIMEOUT_QUEUE_OP)
            except Empty:
        ...
        try:
            ...
            result_q.put(('exception', e), timeout=TIMEOUT_QUEUE_OP)
        ...
        result_q.put(('exception', e), timeout=TIMEOUT_QUEUE_OP)
        result_q.put(('exit', (index, -1, -1)), timeout=TIMEOUT_QUEUE_OP)
