...
def get_secret_value(name: str, key: str = None):
  ...
  if response.get('SecretString'):
    ...
    raise Exception("The secret doesn't have a binary or string value. Something is wrong here.")

def create_secret(name: str, description: str = None, binary: bytes = None, string: str = None, kvp: dict = None, tags: dict = None):
  """
...
def delete_secret(name: str, recovery_window: int = None, perma_delete: bool = False):
  ...
  :param name: The name of the secret you're deleing. If the secret is in your account, this can be the 'friendly' name or ARN. Otherwise, it must be the ARN.
